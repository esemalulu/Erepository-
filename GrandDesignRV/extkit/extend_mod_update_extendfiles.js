	...
	function (error, recordModule, search, file) {
		...
		const updateExtendFilesRecord = (args) => {
			...
			if (args.extendFileId) {
				...
				throw error.create({
					name: 'eXtendFiles Record Id Not Found',
					message: `eXtendFiles record Id not found.`
				});
			}

			let filesRecordObject = recordModule.load({ type: CONSTANTS.extendFile.recordType, id: filesRecordId, isDynamic: true });
