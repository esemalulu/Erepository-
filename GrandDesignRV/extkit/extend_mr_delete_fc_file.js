    ...
    function _reduce(context) {
      ...
      try {
        ...
        fileObjectList.forEach(filesRecordObject => {
          try {
            // filesRecordObject = JSON.parse(filesRecordObject);
            ...
            filesRecordObject.values?.custrecord_extfile_file_cabinet?.[0];
      ...
      // log.debug({ title: 'Map Data', details: data });

      // let fileCabinetFieldId = getScriptLevelParameter('custscript_extkit_del_fc_file_cabinet_fi');
      // let fileCabinetFileId = data.values[fileCabinetFieldId]?.value;

      // // Support for text field
      // if (!fileCabinetFileId) {
      //   fileCabinetFileId = data.values[fileCabinetFieldId];
      // }

      // if (fileCabinetFileId) {
      //   file.delete({
      //     id: fileCabinetFileId
      //   });
      // }
    }
