 ...
 *
 */
function ReturnAuthViewModel(data) {
    ...
    self.searchOptions = ko.observableArray([
        ...
        new searchOption(2, 'Return Auth #', 'tranid'),
        new searchOption(3, 'Order #', 'orderNumber')
    ]);
    ...
    self.filterOptions = ko.observableArray([
        ...
        new filterOption('pendingReceipt', 'Pending Receipt'),
        new filterOption('refunded', 'Refunded')
    ]);
...
//    self.ascending = "fa fa-arrow-up";
    ...
    self.columns = ko.observableArray([
        ...
        { property: "tranid", header: "Return Auth # ", type: "number", state: ko.observable(self.descending), width: "cell-md" },
        { property: "trandate", header: "Date ", type: "date", state: ko.observable(""), width: "cell-md" },
        { property: "orderNumber", header: "Order # ", type: "number", state: ko.observable(""), width: "cell-sm" },
    ...
    self.newReturnAuth = function () {
        var context = nlapiGetContext();
        top.window.location.href =  context.getSetting('SCRIPT', 'custscriptwebsitemyaccountdomainurl') + 
                                    "/app/accounting/transactions/rtnauth.nl?compid=" + context.getCompany();
    ...
    self.reloadPage = function () {
        window.location.href = nlapiResolveURL('SUITELET', 'customscriptgd_website2_returnauth_suite', 'customdeploygd_website2_returnauth_suite') + '&showclosed=' + self.showClosed();
    };
...
$(document).ready(function() {
    var data = nlapiGetFieldValue('custpage_returnauthdata');
    var raVM = new ReturnAuthViewModel(data);
    ko.applyBindings(raVM);
    window.getVM = function() {return raVM;};
});
