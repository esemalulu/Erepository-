 ...
 * 1.00       25 Aug 2016     brians
 *
 */
function WarrantyClaimViewModel(data) {
    ...
    self.searchOptions = ko.observableArray([
        ...
        new searchOption(2, 'Claim #', 'id'),
        new searchOption(3, 'Work Order #', 'workOrder'),
        new searchOption(4, 'Pre Auth #', 'preauth'),
        new searchOption(5, 'Customer', 'customer'),
    ]);
    
    self.filterOptions = ko.observableArray([
        new filterOption(1, 'Open'),
        new filterOption(3, 'Approved'),
        new filterOption(4, 'Denied'),
        new filterOption(6, 'Feedback Required'),
        new filterOption(7, 'Pending Part Return'),
        new filterOption(8, 'Submitted'),
        new filterOption(10, 'Pending VCB')
    ]);

    //If the user wants to show all items, then add extra filterOptions
    self.showClosed = ko.observable(parsedData.showClosed);
    if(self.showClosed()){
        self.filterOptions.push(new filterOption(2, 'Closed'));
        self.filterOptions.push(new filterOption(5, 'Paid'));
    }
...
//    self.ascending = "fa fa-arrow-up";
    ...
    self.columns = ko.observableArray([
        ...
        { property: "id", header: "Claim", type: "number", state: ko.observable(self.descending), width: "cell-sm" },
        { property: "requestor", header: "Requestor ", type: "string", state: ko.observable(""), width: "cell-md" },
        { property: "preauth", header: "Pre Auth ", type: "number", state: ko.observable(""), width: "cell-sm" },
...
$(document).ready(function() {
    var data = nlapiGetFieldValue('custpage_warrantydata');
    var claimVM = new WarrantyClaimViewModel(data);
    ko.applyBindings(claimVM);
    window.getVM = function() {return claimVM;};
});
