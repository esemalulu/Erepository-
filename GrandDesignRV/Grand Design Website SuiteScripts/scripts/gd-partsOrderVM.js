 ...
 *
 */
function PartsOrderViewModel(data) {
    ...
    self.searchOptions = ko.observableArray([
        ...
        new searchOption(3, 'Invoice #', 'invoice'),
    ]);
    
    self.filterOptions = ko.observableArray([
        new filterOption('fullyBilled', 'Billed'),
        new filterOption('cancelled', 'Cancelled'),
        new filterOption('closed', 'Closed'),
        new filterOption('partiallyFulfilled', 'Partially Fulfilled'),
        new filterOption('pendingApproval', 'Pending Approval'),
        // new filterOption('pendingBilling', 'Pending Billing'),
        new filterOption('pendingFulfillment', 'Pending Fulfillment')
    ]);
...
//    self.ascending = "fa fa-arrow-up";
    ...
    self.columns = ko.observableArray([
        ...
        { property: "tranid", header: "Order # ", type: "number", state: ko.observable(self.descending), width: "cell-md" },
        ...
        { property: "method", header: "Method ", type: "string", state: ko.observable(""), width: "cell-sm" },
        { property: "tracking", header: "Tracking ", type: "string", state: ko.observable(""), width: "cell-sm" },
...
$(document).ready(function() {
    var data = nlapiGetFieldValue('custpage_partsordersdata');
    var partsOrderVM = new PartsOrderViewModel(data);
    ko.applyBindings(partsOrderVM);
    window.getVM = function() {return partsOrderVM;};
});
