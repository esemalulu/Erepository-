...
function PartsViewModel(data) {
    ...
    self.returnAuthsUrlString = parsedData.returnAuthsUrlString.replace(/&amp;/g, '&');
    self.creditMemosUrlString = parsedData.creditMemosUrlString.replace(/&amp;/g, '&');
...
//    self.ascending = "fa fa-arrow-up";
    ...
    self.columns = ko.observableArray([
        ...
        { property: "tranid", header: "Order # ", type: "number", state: ko.observable(self.decending), width: "cell-md" },
    ...
    self.newPartsOrder = function () {
        //Only redirect if the user has permission. Button will be disabled, but this prevents people from enabling it through the console.
        if(self.hasPermission())
        {
            top.window.location.href = self.urlString;
        }
        else
            alert('You do not have permission to create a parts order.');

    };
    
    self.viewReturnAuths = function () {
        //Only redirect if the user has permission. Button will be disabled, but this prevents people from enabling it through the console.
        if(self.hasPermission())
        {
            top.window.location.href = self.returnAuthsUrlString;
        }
        else
            alert('You do not have permission to view return authorizations.');

    };
    
    self.viewCreditMemos = function () {
        //Only redirect if the user has permission. Button will be disabled, but this prevents people from enabling it through the console.
        if(self.hasPermission())
        {
            top.window.location.href = self.creditMemosUrlString;
        }
        else
            alert('You do not have permission to view return credit memos.');
    ...
    self.reloadPage = function () {
        window.location.href = nlapiResolveURL('SUITELET', 'customscriptrvs_website2_partsord_suite', 'customdeployrvs_website2_partsord_suite') + '&showclosed=' + self.showClosed();
    };
...
$(document).ready(function() {
    var data = nlapiGetFieldValue('custpage_partsdata');
    var partsVM = new PartsViewModel(data);
    ko.applyBindings(partsVM);
    window.getVM = function() {return partsVM;};
});
