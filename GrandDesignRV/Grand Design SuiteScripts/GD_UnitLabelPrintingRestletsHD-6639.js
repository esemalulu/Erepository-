...
function GetLabelPrintingUnitInfoBySerialNumber(datain) {
    ...
    if (IsDefined(datain)) {
        ...
        if (filters.length > 0) //At least one filter is set, continue
        {
            ...
            columns.push(new nlobjSearchColumn('custrecordunit_axleconfiguration'));
            columns.push(new nlobjSearchColumn('custrecordunit_rimspare'));
            columns.push(new nlobjSearchColumn('custrecordunit_tirespare'));
            columns.push(new nlobjSearchColumn('custrecordunit_psispare'));
            columns.push(new nlobjSearchColumn('custrecordunit_tirehotfix'));
            ...
            */
            ...
            if (results != null && results.length > 0) {
                for (var i = 0; i < results.length; i++) {
                    ...
                    var isCanadianDealer = ((shipToCountry == 'CAN') || (shipToCountry == 'CA') || (shipToCountry == 'CANADA'));
                    ...
                    // HD-6639
                    var spareTireSize = ConvertNSFieldToString(resultObject.getValue('custrecordunit_tirespare'));
                    var spareTirePSI = ConvertNSFieldToInt(resultObject.getValue('custrecordunit_psispare'));
                    var spareRimSize = ConvertNSFieldToString(resultObject.getValue('custrecordunit_rimspare'));
                    var tireHotfixApplied = ConvertNSFieldToString(resultObject.getValue('custrecordunit_tirehotfix')) == 'T';
                    ...
                    var axleCount = ConvertNSFieldToInt(resultObject.getValue('custrecordunit_axleconfiguration'));
                    var singleOrDualAxle = 'SINGLE';
                    switch (axleCount) {
                        case 1:
                            singleOrDualAxle = 'SINGLE';
                            break;
                        case 2:
                            singleOrDualAxle = 'TANDEM';
                            break;
                        case 3:
                            singleOrDualAxle = 'TRIPLE';
                            break;
                    }
                    ...
                    var cargoCapacityLbCan = Math.round(gvwrLb - uvwLb - lpGasCapacityLb - freshWaterCapacityLb - waterHeaterCapacityLb);
                    var cargoCapacityKgCan = Math.round(ConvertLbToKG(gvwrLb)) - Math.round(ConvertLbToKG(uvwLb)) - Math.round(ConvertLbToKG(lpGasCapacityLb)) - Math.round(ConvertLbToKG(freshWaterCapacityLb)) - Math.round(ConvertLbToKG(waterHeaterCapacityLb));;
                    ...
                    // IF the unit has “axle configuration” as tandem or triple axles, then proceed how it is currently set up 
                    // (just copying the tire information into both the front and rear axles).
                    ...
                    unitObject['CanadaCargoCapacityLB'] = cargoCapacityLbCan;
                    unitObject['CanadaCargoCapacityKG'] = cargoCapacityKgCan;
