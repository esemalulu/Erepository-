...
function PrintVendorReturnAuthorizationSuitelet(request, response)
{
	...
	if (vendorReturnAuthId != null && vendorReturnAuthId != '')
	{
		...
		var plugin = new VendorReturnAuthPrintoutPlugin();
		var vendorReturnAuthHTML =  plugin.PrintVendorReturnAuthorization(vendorReturnAuthId);
...
function PrintVendorReturnAuthorization(vendorReturnAuthId)
{
	...
	var totalAmount = nlapiFormatCurrency(returnAuth.getFieldValue('usertotal'));
	...
	var itemHTML = 
			...
			'<tr>' + 
				'<td style="font-weight:bold; border-bottom: 1px solid black; padding-bottom:10px;width:15%;">' +
					'Item' +
				'</td>' +
                '<td style="font-weight:bold; border-bottom: 1px solid black; padding-bottom:10px;width:20%; text-align:center;">' +
					'Vendor<br />Part #' +
				'</td>' +
                '<td style="font-weight:bold; border-bottom: 1px solid black; padding-bottom:10px;width:13%; text-align:center;">' +
				...
				'<td style="font-weight:bold; border-bottom: 1px solid black; padding-bottom:10px;width:40%;">' +
					'&nbsp;&nbsp;&nbsp;Description' +
				'</td>' +
                '<td style="font-weight:bold; border-bottom: 1px solid black; padding-bottom:10px;width:5%;">' +
					'Qty' +
				'</td>' +
                '<td style="font-weight:bold; border-bottom: 1px solid black; padding-bottom:10px;width:7%;">' +
                	'Rate' +
                '</td>' +
                '<td style="font-weight:bold; border-bottom: 1px solid black; padding-bottom:10px;width:5%;">' +
					'UoM' +
				'</td>' +
				'<td style="font-weight:bold; border-bottom: 1px solid black; padding-bottom:10px;width:10%;" align="right">' +
					'Amount' +
				'</td>' +
			'</tr>' + 
			'<tr>' + 
	...
	{	
		...
		var itemPrice = ConvertNSFieldToString(returnAuth.getCurrentLineItemValue('item', 'rate'));
		...
		var lineAmount = ConvertNSFieldToString(returnAuth.getLineItemValue('item', 'amount', i));
        
        lineAmount = (lineAmount == '' ? '' : nlapiFormatCurrency(lineAmount));
		itemHTML += 
	...
	'<tr>' + 
		...
		'<td style="font-weight:bold; padding-top:10px;">' +
			'Total' +
		'</td>' +
		'<td style="font-weight:bold; padding-top:10px;" align="right">' +
			'$' + totalAmount +
		'</td>' +
	'</tr>';
