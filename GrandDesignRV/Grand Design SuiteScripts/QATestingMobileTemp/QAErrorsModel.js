...
function QATestViewModel() {
    ...
    self.isErrorMode = ko.observable(false); // flag indicating if the page is in error remediation mode
    ...
    self.filterUnits = ko.computed(function() {
    	if(!self.selectedLocation()) {
            ...
            return ko.utils.arrayFilter(self.units(), function(unit) {
                return unit.location == self.selectedLocation().locationId;
    ...
    self.selectedLocation.subscribe(function(newValue){
    	self.clearTest();
    	self.getTestFailures();
    });
    
    // Action from notes popup button that sets the question to bind to and opens the popup 
    ...
    self.goToTest = function(unit, test){
    	self.getTest(test.testId);
    	self.selectedVIN(unit);
  ...
  // ***** REST Calls
  self.getTest = function(testId) {
	   	 // Get Test
	  showOrHideSpinner(true, 'Loading Test...');
	  var proxy = new ProxyObject(GetAuthToken(), 
	  			"/app/site/hosting/restlet.nl?script=customscriptrvs_qatest_testrest&deploy=customdeployrvs_qatest_testrest", {testId: testId}, 'GET');
		  	proxy.makeRequest({
		  		successCallback: function (data) {
		  			 var lines = [];
		    	        ...
		    	        // filter out non-failure lines since this is just for error remediation
		    	        self.qaLines( ko.utils.arrayFilter(lines, function(line) {
		    	               return !line.value();}));
    ...
    self.getTestFailures = function() {
    
    	showOrHideSpinner(true, 'Loading Tests...');
    	
      // Get Existing Tests with Failures
  	  var proxy = new ProxyObject(GetAuthToken(), 
    			"/app/site/hosting/restlet.nl?script=customscriptqatest_failuresrest&deploy=customdeployqatest_testfailuresdeploy", {'location': (self.selectedLocation() ? self.selectedLocation().locationId : '')}, 'GET');
  	  	proxy.makeRequest({
  	  		successCallback: function (data) {
	  	  		 var tempUnits = [];
		    	       
	 	        $.each(data, function(i, item) {
	 	        	tempUnits.push(new QAUnit(item)); 	            
	 	        });
	 	        self.units(tempUnits);
  	    	        
	         },
	         failureCallback: function() {
	        	 alert("Error Retrieving Unit Failures.\n\n1.Make sure the tablet is connected to the internet.\n2.Open the Settings panel on the right to check login status.");
    	         //self.netSuiteLogin();
	         },
	         completeCallback: function() {
	        	 showOrHideSpinner(false);
   ...
   self.saveTest = function(callback){
	  ...
	  var failureLines =  ko.utils.arrayFilter(self.qaLines(), function(qaLine) {
		    return !qaLine.fixed();
      		});
	  
	  // if there are no more failed lines then mark the test complete
	  if(failureLines == 0)
	...
	proxy.makeRequest({
		successCallback: function (data) {
	         ...
	         failureCallback: function() {
	        	 alert("Error Saving Test. Please re-load the test and try submitting it again.\n\n1.Make sure the tablet is connected to the internet.\n2.Open the Settings panel on the right to check login status.");
	        	 //self.netSuiteLogin();
	         },
	         completeCallback: function() {
	        	 setTimeout(function(){
   ...
   self.markCompleted = function(){
		   ...
		   if(invalidLines.length > 0)
		   {
			   ...
			   invalidLines =  ko.utils.arrayFilter(self.qaLines(), function(qaLine) {
				    return !qaLine.value();
	            });
			   // check if test should go to Completed or Rework Status
			   if(invalidLines.length >0)
				   self.test().status(3);
			   else
				   self.test().status(2);
			   self.test().isCompleted(true);
...
function QATest(data) {
    ...
    self.typeId = ko.observableArray(data.typeId);
    self.typeName = ko.observableArray(data.typeName);
