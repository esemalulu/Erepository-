/**
 * Code for printing the DRO report/portlet.
 * 
 * Version    Date            Author           Remarks
 * 1.00       26 Feb 2013     nathanah
 *
 */
 ...
 */
function DROPrintSuitelet(request, response)
{
	if (request.getMethod() == 'GET') {
		...
		if (custparamReportDate != '' ) {
			...
			var emptyRow = 
				'<tr>' + 
					'<td colspan="' + ((droMonthlyObjArray.length*2) + 1) + '">&nbsp;</td>' + 
				'</tr>';
			
			// build the data HTML
			var dataHTML = 
				'<table cellpadding="5" width="100%" style="padding-top:9px;" cellborder="1" cellmargin="0">' + 
					GetDRODailyHTML(droObjectArray) +
					emptyRow + GetDROMonthlyHTML(droMonthlyObjArray) + emptyRow + GetDRORegHTML(droRegObjArray) +
				'</table>';
			...
			var html = 
				'<body padding=".1in .1in .1in .1in" style="font-size:7pt; font-family:sans-serif; ' + portraitOrLandscape + '">' + 
					headerHTML + 
					'<br />' + 
					dataHTML +
				'</body>';
			
			PrintPDFInSuiteLet(request, response, 'DROByPlant.pdf', html);
		} else {
		...
		nlapiSetRedirectURL('SUITELET', 'customscriptgddropbyplantprintsuitelet', 'customdeploy1', false, parameters);
	}
}
 ...
 * @returns {String}
 */
function GetDRODailyHTML(droObjectArray)
{
	...
	dailyDetailHTML += GetDROHTMLDataByName(droObjectArray, 'Units Produced (Offline)', DAILY_UNITSOFFLINE, DAILY_UNITSOFFLINEAMOUNT);
	
	dailyDetailHTML += GetDROHTMLDataByName(droObjectArray, 'Shipments', DAILY_SHIPMENTS, DAILY_SHIPMENTSAMOUNT);
	...
	dailyDetailHTML += GetDROHTMLDataByName(droObjectArray, 'Orders Received', DAILY_SALESORDERS, DAILY_SALESORDERSAMOUNT);	
 ...
 * @returns {String}
 */
function GetDROMonthlyHTML(droObjectArray)
{
	droObjectArray.sort(function(a, b){
		return (a[LOCATIONTEXT] < b[LOCATIONTEXT]) ? -1 : 1;
	});
	...
	var emptyRow = 
		'<tr>' + 
			'<td colspan="' + ((droObjectArray.length*2) + 1) + '">&nbsp;</td>' + 
		'</tr>';
	
	// build the detail html data for the daily part of the html	
	monthlyDetailHTML += GetDROHTMLDataByName(droObjectArray, 'Beginning Inventory (Total)', MONTHLY_BEGINNINGINVENTORY, MONTHLY_BEGINNINGINVENTORYAMOUNT);
	monthlyDetailHTML += GetDROHTMLDataByName(droObjectArray, 'Units Produced (Total)', MONTHLY_UNITSOFFLINE, MONTHLY_UNITSOFFLINEAMOUNT);
	...
	monthlyDetailHTML += GetDROHTMLDataByName(droObjectArray, 'Shipments', MONTHLY_SHIPMENTS, MONTHLY_SHIPMENTSAMOUNT);
	monthlyDetailHTML += GetDROHTMLDataByName(droObjectArray, 'Ending Inventory (Total)', MONTHLY_ENDINGINVENTORY, MONTHLY_ENDINGINVENTORYAMOUNT);
	monthlyDetailHTML += GetDROHTMLDataByName(droObjectArray, 'Units Not Complete in Yard', UNITS_INCOMPLETE_IN_YARD, UNITS_INCOMPLETE_IN_YARD_AMOUNT);
	monthlyDetailHTML += emptyRow;
	
	monthlyDetailHTML += GetDROHTMLDataByName(droObjectArray, 'Orders Received (current month)', MONTHLY_SALESORDERS, MONTHLY_SALESORDERSAMOUNT);	
	monthlyDetailHTML += GetDROHTMLDataByName(droObjectArray, 'Orders Received Last MTD (@ month end)', MONTHLY_LAST_SALESORDERS, MONTHLY_LAST_SALESORDERSAMOUNT);	
	monthlyDetailHTML += GetDROHTMLDataByName(droObjectArray, 'Orders Received (YTD)', MONTHLY_YTD_SALESORDERS, MONTHLY_YTD_SALESORDERSAMOUNT);	
	monthlyDetailHTML += GetDROHTMLDataByName(droObjectArray, 'Orders Received Last YTD (@ month end)', MONTHLY_LASTYTD_SALESORDERS, MONTHLY_LASTYTD_SALESORDERSAMOUNT);	
	//Shipments area
	monthlyDetailHTML += emptyRow;
	monthlyDetailHTML += GetDROHTMLDataByName(droObjectArray, 'Total Available to Ship', MONTHLY_TOTAL_TO_SHIP, MONTHLY_TOTAL_TO_SHIPAMOUNT);	
	monthlyDetailHTML += GetDROHTMLDataByName(droObjectArray, 'Shipments MTD', MONTHLY_SHIPMENTS, MONTHLY_SHIPMENTSAMOUNT);	
	monthlyDetailHTML += GetDROHTMLDataByName(droObjectArray, 'Total Left to Ship for Month', MONTHLY_LEFT_TO_SHIP, MONTHLY_LEFT_TO_SHIPAMOUNT);	
	monthlyDetailHTML += GetDROHTMLDataByName(droObjectArray, 'Total Open Units for Month', MONTHLY_OPEN_UNITS, MONTHLY_OPEN_UNITSAMOUNT);
	monthlyDetailHTML += emptyRow;
	// Scheduled totals area
	monthlyDetailHTML += GetDROHTMLDataByName(droObjectArray, 'Scheduled Opens', MONTHLY_SCHEDULED_OPENS, MONTHLY_SCHEDULED_OPENSAMOUNT);
	monthlyDetailHTML += GetDROHTMLDataByName(droObjectArray, 'Scheduled Sold', MONTHLY_SCHEDULED_SOLD, MONTHLY_SCHEDULED_SOLDAMOUNT);
	monthlyDetailHTML += GetDROHTMLDataByName(droObjectArray, 'Backlog', MONTHLY_BACKLOG, MONTHLY_BACKLOGAMOUNT);
 ...
 * @returns {String}
 */
function GetDRORegHTML(droObjectArray)
{
	droObjectArray.sort(function(a, b){
		return (a[LOCATIONTEXT] < b[LOCATIONTEXT]) ? -1 : 1;
	});
	...
	// build the detail html data for the daily part of the html	
	regDetailHTML += GetDROHTMLDataByName(droObjectArray, 'Daily (yesterday)', DAILY_REGISTRATIONS);
	regDetailHTML += GetDROHTMLDataByName(droObjectArray, 'Month To Date', MONTHLY_REGISTRATIONS);
	regDetailHTML += GetDROHTMLDataByName(droObjectArray, 'Last Year MTD', MONTHLY_LAST_YEAR_REGISTRATIONS);
	regDetailHTML += GetDROHTMLDataByName(droObjectArray, 'Year To Date', YEARLY_REGISTRATIONS);
	regDetailHTML += GetDROHTMLDataByName(droObjectArray, 'Last Year To Date', LAST_YEARLY_REGISTRATIONS);	
...
//		                       	  [ 'custrecordunit_datecompleted', 'before', 'yesterday' ], 
//		                       	  'AND', 
//		                       	  [
//		                       	   		[ 'custrecordunit_shipdate', 'isempty', null ],
//		                       	   		'OR',
//		                       	   		[ 'custrecordunit_shipdate', 'onorafter', 'yesterday' ]
//		                       	  ]
//	                       	  ];
//	
//	var results = nlapiSearchRecord('customrecordrvsunit', 'customsearchrvs_drobyplantdailycount', filterExpression);
...
//	filterExpression =	  [ 
//		                       	  [ 'custrecordunit_datecompleted', 'on', 'yesterday' ], 
//                       	  ];	
//	
//	results = nlapiSearchRecord('customrecordrvsunit', 'customsearchrvs_drobyplantdailycount', filterExpression);	
...
//	
	...
	filterExpression =	  [ 
		                       	  ...
		                       	  'AND', ['custrecordunit_location.custrecordrvs_location_isproduction', 'is', 'T']
                       	  ];	
	...
	results = nlapiSearchRecord('customrecordrvsunit', 'customsearchrvs_drobyplantoffline', filterExpression);	
	...
	results = nlapiSearchRecord('customrecordrvsunit', 'customsearchrvsdrobyplantsocount', filterExpression);
...
{	
	...
	var locationCol = new nlobjSearchColumn('custrecordunit_location', null,'group');
	locationCol.setSort();
	var results = nlapiSearchRecord('customrecordrvsunit', 'customsearchrvs_drobyplantdailycount', filterExpression, [locationCol]);
...
//	ProcessDROSearchResults(results, MONTHLY_UNITSPRODUCED, objectArray, TYPE_MONTHLY, MONTHLY_UNITSPRODUCEDAMOUNT);
	...
	filterExpression =	  [ 
		                       	  ...
		                       	  'AND', ['custrecordunit_location.custrecordrvs_location_isproduction', 'is', 'T']
                       	  ];	// beginning of the current fiscal month until the report date.
	
//	var locationCol = new nlobjSearchColumn('location', null,'group');
//	locationCol.setSort();
	results = nlapiSearchRecord('customrecordrvsunit', 'customsearchrvs_drobyplantdailycount', filterExpression, [locationCol]);	
	...
	results = nlapiSearchRecord('customrecordrvsunit', 'customsearchrvs_drobyplantoffline', filterExpression, [locationCol]);	
	...
	results = nlapiSearchRecord('customrecordrvsunit', 'customsearchrvsdrobyplantsocount', filterExpression, [locationCol]);	
...
//	var thisYearBeginning = '1/1/' + (nlapiAddDays(new Date(), -1)).getFullYear();
	var d = new Date(custparamReportDate);
	d.setFullYear(d.getFullYear() - 1);
	var custparamReportDateLastYear = nlapiDateToString(d);
//	var lastYearBeginning = '1/1/' + ((nlapiAddDays(new Date(), -1)).getFullYear()-1);
	...
	filterExpression =	  [ 
		                       	  ...
		                       	  'AND',
		                       	  [ 'custrecordunit_salesorder.custbodyrvsdealerorderdate', 'onorbefore', custparamReportDateLastYear ],
		                       	  'AND',
		                       	  [ 'custrecordunit_dealer', 'noneof', GetRVSOpenLotDealer()],
		                       	  'AND', ['custrecordunit_location.custrecordrvs_location_isproduction', 'is', 'T']
                       	  ];	
...
{	
	...
	results = nlapiSearchRecord('customrecordrvsunit', 'customsearchrvs_drobyplantregistrations', filterExpression);	
	...
	filterExpression =	  [ 
		                       	  [ 'custrecordunitretailcustomer_unit.created', 'onorafter', lastYearThisMonthAccountingPeriod[0].getValue('startdate') ],
		                       	  'AND',
		                       	  [ 'custrecordunitretailcustomer_unit.created', 'onorbefore', lastYearThisMonthAccountingPeriod[0].getValue('enddate') ],
		                       	  'AND', ['custrecordunit_location.custrecordrvs_location_isproduction', 'is', 'T']
                       	  ];	
	...
	// Registrations YTD
	filterExpression =	  [ 
		                       	  [ 'custrecordunitretailcustomer_unit.created', 'onorafter', thisYearAccountingPeriod[0].getValue('startdate') ], 
		                       	  'AND',
		                       	  [ 'custrecordunitretailcustomer_unit.created', 'onorbefore', custparamReportDate ],
		                       	  'AND', ['custrecordunit_location.custrecordrvs_location_isproduction', 'is', 'T']
                       	  ];	
...
function ProcessDROSearchResults(results, fieldName, objectArray, type, amountFieldName)
{
	...
	if (results != null)
	{
		for (var i=0; i<results.length; i++)
		{
			...
			var unitCount = results[i].getValue('name', null, 'count');
			var locationId = results[i].getValue('custrecordunit_location', null, 'group');
			var locationText = results[i].getText('custrecordunit_location', null, 'group');
			var amount = results[i].getValue('amount', 'custrecordunit_salesorder', 'sum');
			var object;
			switch(type)
			{
			case TYPE_DAILY:
				object = GetObjectFromArray(objectArray, locationId, locationText);
				break;
			case TYPE_MONTHLY:
				object = GetMonthlyObjectFromArray(objectArray, locationId, locationText);
				break;
			case TYPE_REG:
				object = GetRegObjectFromArray(objectArray, locationId, locationText);
				break;	
			default:
				object = null;
			}
...
/**
 * Returns the Daily DRO object based on the locationId and the array being passed in.
 * If no object exists for the series already, then create one.
 * 
 * @param objectArray
 * @param locationId
 */
function GetObjectFromArray(objectArray, locationId, locationText)
{
	for (var j=0; j<objectArray.length; j++)
	{
		var object = objectArray[j];
		
		if (object[LOCATIONID] == locationId)
		{
			return object;
		}
	}
	...
	object[LOCATIONTEXT] = nlapiEscapeXML(locationText);
...
/**
 * Returns the monthly dro object based on the locationId and the array being passed in.
 * If no object exists for the series already, then create one.
 * 
 * @param objectArray
 * @param locationId
 */
function GetMonthlyObjectFromArray(objectArray, locationId, locationText)
{
	for (var j=0; j<objectArray.length; j++)
	{
		var object = objectArray[j];
...
/**
 * Returns the registraitions dro object based on the locationId and the array being passed in.
 * If no object exists for the series already, then create one.
 * 
 * @param objectArray
 * @param locationId
 */
function GetRegObjectFromArray(objectArray, locationId, locationText)
{
	for (var j=0; j<objectArray.length; j++)
	{
		var object = objectArray[j];
