...
function processMerge() {
	...
	if (!caseobj.hasmsgs) {
		...
		/**
		 * Since 2014v1, Modifying Existing Email Message has been Disabled.
		 * Due to this Issue, Message MUST be copied and original one deleted.
		 */
		for (var m=0; armsg && m < armsg.length; m++) {
			...
			if (msginternalId) {
				...
				//Disabled Message Editing since 2014v1
				//load original message to grab values from
				var origmsg = nlapiLoadRecord('message',msginternalId);
				//Copy existing message
				//2014v1 & 2014v2 update.
				//- Copy original message record and set values from original.
				var msg = nlapiCopyRecord('message', msginternalId);
				
				var authEmail = origmsg.getFieldValue('authoremail');//ELI - Nov 5, 2016 Correcting to prevent "Please enter value(s) for: From Email Address" Error
				var recEmails = origmsg.getFieldValue('recipientemail');
				
				var arRecEmails = new Array();
				var ccString = '';

				if (recEmails != null && recEmails.indexOf(',') > -1 ) 
				{  // GAMA edit - Jan 7, 2012  - Added check for null recipients
					arRecEmails = recEmails.split(',');
				} 
				else if (recEmails != null && recEmails.indexOf(';') > -1 ) 
				{ // GAMA edit - Jan 7, 2012  - Added check for null recipients
					arRecEmails = recEmails.split(';');
				} 
				else if (recEmails == null) 
				{   // GAMA addition - Jan 7, 2012  - Added check for null recipients
					msg.setFieldValue('recipientemail', 'Adam.Harte-Maxwell@javelin-tech.com'); // GAMA addition - Jan 7, 2012  - Added putting Adam as recipient if blank
					log('DEBUG','processMerge', 'Adding Adam email address to fix blank recipient'); // GAMA addition - Jan 7, 2012  - Added putting Adam as recipient if blank
				} // GAMA addition - Jan 7, 2012  - Added check for null recipients
				...
				if (origmsg.getFieldValue('subject') == null) { // GAMA addition - Add a subject for internal notes if message subject blank
					msg.setFieldValue('subject', 'Internal Memo'); // GAMA addition - Add a subject for internal notes if message subject blank
				}  // GAMA addition - Add a subject for internal notes if message subject blank
				
				msg.setFieldValue('authoremail', authEmail); //ELI - Nov 5, 2016 Correcting to prevent "Please enter value(s) for: From Email Address" Error				
				...
				nlapiDeleteRecord('message', msginternalId);
...
function displayLinkedSourceRecords() {
	...
	if (armsg && armsg.length > 0) {
		...
		for (var m=0; m < armsg.length; m++) {
			...
			if (armsg[m].getValue('subject','messages') != null && armsg[m].getValue('subject','messages') != '') { // GAMA addition - Jan 7, 2012
				msglist.setLineItemValue('msglist_msg_msbj', mline, armsg[m].getValue('subject','messages'));
			} // GAMA addition - Jan 7, 2012
			else {  // GAMA addition - Jan 7, 2012
				msglist.setLineItemValue('msglist_msg_msbj', mline, 'Internal Note'); // GAMA addition - Jan 7, 2012
			} // GAMA addition - Jan 7, 2012
...
function initForm(isResult) {
	...
	if (caseobj.haserror) {
		...
		if (!isResult) {
			...
			markInternalOnly.setDefaultValue('T');  //  GAMA added as per request by Adam 2012-10-26
...
function initReqVals(req) {
	...
	if (caseobj.target) {
        nlapiSubmitField('supportcase',caseobj.target,'status','2'); // Set the case to work in progress
	...
	//if source and target are the same case  // GAMA addition - Jan 23, 2012
	if (caseobj.source == caseobj.target) {  // GAMA addition - Jan 23, 2012
		caseobj.haserror = true;  // GAMA addition - Jan 23, 2012
		caseobj.action = 'ERROR';  // GAMA addition - Jan 23, 2012
		caseobj.errortext = 'That was a close one, you almost deleted your source case!  Please try again and select a different target case.';  // GAMA addition - Jan 23, 2012
	}  // GAMA addition - Jan 23, 2012
