...
function ValidateLine(type)
{
	...
	if (itemType != "Discount" && itemType != "Markup" && itemType != "Description" && itemType != "Other Charge" && itemType !="Subtotal" && itemType !="Item Group" && itemType !="Kit/Package" && itemType !="Item Group" && itemType != "")
	{
...
function FieldChanged(type,name)
{
	...
	if ((name == 'porate') || (name == 'custcol_purchaseprice') || (name == 'povendor'))
	{
	if (debug)
	 	alert("type: " + type + "\nname:" + name);
		...
		filters[0] = new nlobjSearchFilter('internalid', null,'anyof', vendor, null);
		...
		if (debug)
			alert("About to search for exchange rate");

		var exchangerate = nlapiLookupField('currency', vendor_currency, 'exchangerate');
		...
		else if (name == "porate")
		{
			...
			nlapiSetCurrentLineItemValue('item', 'custcol_purchaseprice', converted_rate.toFixed(2), false);
		}
	}
