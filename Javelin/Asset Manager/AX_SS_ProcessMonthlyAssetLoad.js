...
function ProcessMonthlySwxAssetLoad(type) {
	...
	try {
		...
		var pendingCol = [new nlobjSearchColumn('internalid').setSort(true),
		                  ...
		                  new nlobjSearchColumn('custrecord_ax_swxa_orig_order'), new nlobjSearchColumn('custrecord_ax_swxa_endcust_contact_nm'),
		                  ...
		                  new nlobjSearchColumn('custrecord_ax_swxa_endcust_site'), new nlobjSearchColumn('custrecord_ax_swxa_endcust_ctry'),
		...
		for (var i=0; pendingRslt && i < pendingRslt.length; i++) {
			...
			if (acctCustRslt) {
				...
				//Use Global Search 
				//mod 3/32014 - remove multiple periods in the account name. 
				var globalAcctName = strGlobalReplace(pobj.accountname, '\\.', '');
				var globalSearchKeyword = 'cust:'+globalAcctName;
			...
			if (execContactSearch && pobj.contactemail) {
				//Bug 2/15/2014-Make sure parent of contact is CUSTOMER
			...
			if (pobj.customerid && pobj.itemid)  {
				...
				var custAssetCol = [new nlobjSearchColumn('internalid'),
				                    ...
				                    new nlobjSearchColumn('custrecord_ax_cswxa_assetstatus')];
				...
				if (custAssetRslt) {
					...
					//Mod 3/9/2015 - Grab current asset status value
					pobj.customerorigassetstatus = custAssetRslt[0].getValue('custrecord_ax_cswxa_assetstatus');
				}
			...
			//3/9/2015 -
			//Check to see if Asset Status MUST be set to 5 or 3 depending on entitlement end date
			var isOnOrBeforeLastDayPrev = false;
			if (pobj.entitleenddate) {
				//Grab Objectized Entitlement End Date
				var dtObjEntEndDate = nlapiStringToDate(pobj.entitleenddate);
				var currDate = new Date();
				var firstOfCurrDate = nlapiStringToDate((currDate.getMonth()+1)+'/1/'+currDate.getFullYear());
				var dtObjLastDayPrevMonth = nlapiAddDays(firstOfCurrDate, -1);
				
				//Compare and see isOnOrBeforeLastDayPrev
				log('debug','Ent Date compare','Ent. End Date: '+dtObjEntEndDate+' // Last day of Prev Month: '+dtObjLastDayPrevMonth);
				if (dtObjEntEndDate <= dtObjLastDayPrevMonth) {
					isOnOrBeforeLastDayPrev = true;
				}
			}
			log('debug','isOnOrBeforeLastDayPrev for Stage rec '+pobj.id,isOnOrBeforeLastDayPrev);
			...
			if (pobj.stagestatus == paramValidStatusId) {
				...
				if (!pobj.customerassetid) {
					...
					//NEED TO Make sure it checks for MASs UPDATE Replacement
					//Entitlement date on or before last day of prevoius month based on current date
					//AND
					//status is NOT "Not To Be Renewed" and "Not Renewing"
					//SET The Status to "Not Renewing"
					//status value of Not Renewing: 3
					//status value of Not To be Renewed: 5
					if (isOnOrBeforeLastDayPrev && assetStatus != '5' && assetStatus != '3') {
						assetStatus = '3';
					}
					...
					if (pobj.customerassetentdate && pobj.customerassetentdate !=pobj.entitleenddate) {
						...
						if (!itemsNotToRenew.contains(pobj.itemid)) {
							...
							//status is NOT "Not To Be Renewed" and "Not Renewing"
							//SET The Status to "Not Renewing"
							//pobj.customerorigassetstatus
							
							assetUpdFlds.push('custrecord_ax_cswxa_assetstatus');
							
							if (isOnOrBeforeLastDayPrev && pobj.customerorigassetstatus != '5' && pobj.customerorigassetstatus != '3') {
								assetUpdVals.push('3');
							} else {
								assetUpdVals.push('4');
							}
		...
		var errmsg = curProcessAssetLoadText+'<br/>'+
					 'Scheduled script terminated with following error:<br/>'+
					 getErrText(assetStageError);
