...
function PardotLead(msg) {
	...
	this.pardotPhoneAreaCodeCountryName = false;
	this.pardotPhoneAreaCodeRegionID = false;
	this.pardotPhoneAreaCodeRegionName = false;
	this.pardotPhoneAreaCodeCountryID = false;
	...
	this.northAmericanPhoneNumber = getPardotPhoneAreaCodeRegion(this.areaCode);
	this.validCanadianPhoneNumber = (this.pardotPhoneAreaCodeCountryName === 'Canada') ? true : false;
...
function createOpportunity() {
	...
	if (isOpportunityReady()) {
		...
		// Disposition based on valid email, phone and location ( TBD )
		log('isOpportunityReady', 'lead.pardotPhoneAreaCodeCountryName =' + lead.pardotPhoneAreaCodeCountryName + ', lead.hasPardotHardBounce = ' +lead.hasPardotHardBounce);
		if (lead.pardotPhoneAreaCodeCountryName !== 'Canada' && lead.hasPardotHardBounce === true) {
		...
		else if (lead.pardotPhoneAreaCodeCountryName !== 'Canada' && lead.hasPardotHardBounce === false) {
			log('createOpportunity - BAD OPP', 'EMAIL RESPONSE ONLY OPPORTUNITY - NOT VALID NORTH AMERICAN PHONE NUMBER');
			lead.badDataOpportunity = true;
			lead.emailResponseOnly = true;  // TBD - Use later to send email response from Netsuite
		}
		...
		newOpp.setFieldValue( 'custbody_pardotbadphonenumber',(lead.validCanadianPhoneNumber === false) ? 'T' : null);
		newOpp.setFieldValue( 'custbody_pardotareacoderegionname', (lead.pardotPhoneAreaCodeRegionID === false) ? null : lead.pardotPhoneAreaCodeRegionID);
		newOpp.setFieldValue( 'custbody_pardotareacoderegioncountry', (lead.pardotPhoneAreaCodeCountryID === false) ? null : lead.pardotPhoneAreaCodeCountryID);
		...
		if (lead.webProduct === 'Training') {  // Get the training rep if there is one logged in the queue
			var trainingRepLoggedInQueue = getProductSpecificRepOverides(2);  // Rapid Prototype	1, Training	2, Subscription	3, Simulation	4
			if (trainingRepLoggedInQueue !== false) {
				lead.insideSalesRepID = trainingRepLoggedInQueue;
			}
		}
...
function getPardotPhoneAreaCodeRegion(areaCode) {

	var foundPhoneAreaCodeRegion = false;
	var filters = [];
	var columns = [];

	if (areaCode !== false) {
		filters[0] = new nlobjSearchFilter('custrecord_areacodelist', null, 'contains', areaCode);
		columns[0] = new nlobjSearchColumn('internalid');
		columns[1] = new nlobjSearchColumn('custrecord_areacodelist');
		columns[2] = new nlobjSearchColumn('custrecord_areacoderegioncountry');
		columns[3] = new nlobjSearchColumn('name');


		log('getPardotPhoneAreaCodeRegion', 'Attempting to find the phone area code region by area code = ' + areaCode);
		var phoneAreaCodeRegions = nlapiSearchRecord('customrecord_areacoderegion', null, filters, columns);
		for (var i = 0 ; phoneAreaCodeRegions !== null && i < phoneAreaCodeRegions.length ; i++) {
			var phoneAreaCodeRegion = phoneAreaCodeRegions[i];
			foundPhoneAreaCodeRegion = true;
			lead.pardotPhoneAreaCodeRegionID = phoneAreaCodeRegion.getValue('internalid');
			lead.pardotPhoneAreaCodeRegionName = phoneAreaCodeRegion.getValue('name');
			lead.pardotPhoneAreaCodeCountryID = phoneAreaCodeRegion.getValue('custrecord_areacoderegioncountry');
			lead.pardotPhoneAreaCodeCountryName = phoneAreaCodeRegion.getText('custrecord_areacoderegioncountry');
			log('getPardotPhoneAreaCodeRegion', 'Success! Found phone area code region for ' + areaCode + '.  lead.pardotPhoneAreaCodeRegionID = ' + lead.pardotPhoneAreaCodeRegionName + ', lead.pardotPhoneAreaCodeCountry = ' + lead.pardotPhoneAreaCodeCountryName);
		}
	}
	if (foundPhoneAreaCodeRegion) {
		return true;
	}
	else {
		log('getPardotPhoneAreaCodeRegion', 'Did not find a phone area code region for area code = ' + areaCode );
		return false;
	}
}
...
function getProductSpecificRepOverides(oppProduct) {
	...
	Simulation	4	 
	*/
	var oppProductList = ['Rapid Prototype', 'Training', 'Subscription', 'Simulation'];
...
function getInsideSalesRepByAreaCode(areaCode) {
	...
	columns[0] = new nlobjSearchColumn('internalid');
	columns[1] = new nlobjSearchColumn('custentity_salesareacodes').setSort();
	columns[2] = new nlobjSearchColumn('entityid');
