//This scirpt is similar to 'kana_ss_update_end_date_from_so.js' but for updating Subscription End date
...
var functionName;
//Saved Search: Yesterdays Sales Order Approvals For Subscription
var SEARCH_CRITERIA = 'customsearch_yesterday_so_approvals_4_3';
	...
	try {
		...
		if(null != custSearchResults && custSearchResults.length > 0){
			...
			for(soCount = 0; soCount < custSearchResults.length; soCount++){
				...
				if(!kana_IsNull(dtRevRecEndDate)){
					dtRenewalOrRevRecDate = dtRevRecEndDate;
				} else {
					if(!kana_IsNull(dtRenewalEndDate))
						dtRenewalOrRevRecDate = dtRenewalEndDate;
				}
				
				if(kana_IsNull(dtUseEndDate)){
				...
				if((tmpdtRenewalEndDate > tmpdtRevRecEndDate) && (!kana_IsNull(tmpdtRenewalEndDate)) && (!kana_IsNull(tmpdtRevRecEndDate))){
					dtRenewalOrRevRecDate = dtRenewalEndDate;
				}
				...
				if((tmpdtRenewalOrRevRecDate > tmpdtUseEndDate) || (kana_IsNull(tmpdtUseEndDate))){
					dtUseEndDate = dtRenewalOrRevRecDate;
				}
		...
		if (exception instanceof nlobjError) {
			errorText = 'UNEXPECTED ERROR: Updating Customer Subscription Date from SO ' + '\n\n' +
						'Script Name : kana_ss_update_sub_end_date_from_so.js' + '\n' +
...
function updateCustomerRecord(ilocalcustid)
{
	...
	try {
		...
		icustenddate = custCIR.getFieldValue('custentity_subscription_end_date');
		
		var tmpicustenddate = new Date(icustenddate);
		var tmpdtUseEndDate = new Date(dtUseEndDate);
		
		//if(icustenddate < dtUseEndDate){
		if((tmpicustenddate < tmpdtUseEndDate ) && (!kana_IsNull(tmpdtUseEndDate))){
			custCIR.setFieldValue('custentity_subscription_end_date', dtUseEndDate);
