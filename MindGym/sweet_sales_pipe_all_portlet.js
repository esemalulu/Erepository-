...
function sweet_main(portlet, column) {
  ...
  var financial_year = nlapiGetContext().getSetting('SCRIPT', 'custscriptfinancial_year');
  var subsidiary = nlapiGetContext().getSetting('SCRIPT', 'custscriptsubsidiary');
  
  var title = 'Sales Pipeline Consolidated ' + financial_year;
...
function getRowData(searchType, searchId, searchIdNum, fieldName, rowLabel, dateField, year, subsidiaryField, subsidiary, version) {
  ...
  for (i; i < n; i++) {
    ...
    var subsidiaryFilter= ['2', '3', '4', '5'];
    filters.push(new nlobjSearchFilter(subsidiaryField, null, 'anyof', subsidiaryFilter));
    if (version != 0) {
