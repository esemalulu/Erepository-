var WEBSERVICE_URL = 'http://mail-mangler-russell.dev.themindgym.com/mailjob.json';
...
* @param Object type
*/
function scheduleMailManager(type) {

  nlapiLogExecution('DEBUG', 'Start', 'scheduleMailManager');

  // Get a list of email campaigns to be sent today
  var campaign_list = getEmailCampaigns();
...
/*
*  Perform search to get email campaigns that need to be sent today
*
*  @return array
*/
function getEmailCampaigns() {

  var emailCampaigns = new Array();

  var filters = new Array();
  filters[0] = new nlobjSearchFilter('custrecord_ecam_queuedinmailmanager', null, 'is', 'F');
  filters[1] = new nlobjSearchFilter('custrecord_ecam_emailcampaignstatus', null, 'is', 1);
  //filters[2] = new nlobjSearchFilter('custrecord_ecam_senddate', null, 'is', 'today');

  var columns = new Array();
  columns[0] = new nlobjSearchColumn('internalId');

  // Get all the email campaigns
  var emailCampaigns = nlapiSearchRecord('customrecord_emailcampaign', null, filters, columns);
...
function mailManagerJob(email_campaign) {
  ...
  // Schedule date
  var jobDate = nlapiStringToDate(email_campaign.getFieldValue('custrecord_ecam_senddate'));
  // Convert timestamp to seconds from milliseconds
  this.notBefore = (jobDate.getTime() / 1000);
