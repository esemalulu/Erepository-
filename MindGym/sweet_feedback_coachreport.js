/**
 * Display UI for coach feedback report
 *
 * @method coachReport
 * @param {Object} request
 * @param {Object} response
 */
function coachReport(request, response)
{
  echo(getHeader());
  echo(getBody());
  echo(getFooter()); 
...
{ 
  ...
  html.push('<html>');
  html.push('<head>');
  html.push('<title>Coach feedback report</title>');
  ...
  html.push('<!-- coach_feedback_report.js --><script type="text/javascript" src="https://system.netsuite.com/core/media/media.nl?id=304&c=720154&h=b76943d4719ab64c8751&_xt=.js"></script>');    
...
function getBody() 
{
  ...
  html.push('    </div>');
  html.push('    <h1>Coach feedback report</h1>');
  html.push('    ');
  ...
  html.push('    	  ');
  html.push('    	  <div class="search-filters-row">  ');
  html.push('          <button id="run-report-button" class="submit-disabled" onclick="runCoachFeedbackReport()" disabled="true">Run report</button>');
  html.push('        </div>');
  html.push('      ');
  ...
  html.push('          <input type="hidden" id="export-label" name="export-label" value="coach_feedback_report" />          ');
