...
function(search) {
    ...
    function validateLine(scriptContext) {
        ...
        if (sublistId === 'timeitem') {
            ...
            var taskId = currentRecord.getCurrentSublistValue({ sublistId: sublistId, fieldId: 'casetaskevent' });

            if(projectId){
                var taskCount = search.create({
                    type: "projecttask",
                    filters: [ ["project","anyof",projectId] ]
                    }).runPaged().count;
                
                log.debug("Project " + projectId, "Number of tasks: " + taskCount);
                    
                if (taskCount > 0 && !taskId) {
                    alert('Please select a task when a project with tasks is selected.');
