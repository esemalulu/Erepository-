...
{
	var salesOrderStatus = nlapiGetFieldValue('orderstatus')
	//log('DEBUG', 'SALES ORDER STATUS', nlapiGetFieldValue('orderstatus'));
	if(salesOrderStatus == 'A')
	{
		...
		for(var i = 0; i < numOfLines; i+=1)
		{
			...
			try
			{
				switch (itemType)
				{
					case "NonInvtPart":
						...
						var isTraining = itemRec.getFieldValue('custitem_is_training_course');
						if(isTraining == 'T')
						{
							for(var x = 0; x < numOfSeats; x+=1)
							{
								var seat = nlapiCreateRecord('customrecord_trgseat')
									...
									seat.setFieldText('custrecord_seat_trainingitem', item);
			...
			{
				nlapiLogExecution('DEBUG', 'ERROR GENERATING SEAT FULFILLMENT', getErrText(err));
