 ...
 * Date: 2/25/2012
 * Description:
 * - Goes through list of ALL invoices to fill in each lines' dept and class from its' item.
...
var SCRIPT_PARAM_PROC_INTERNALID='custscript_invlast_processed_id';
var lastProcessedId = '';
var EXIT_COUNT=1000;
var MAIN_SEARCH_ID='customsearch_all_inv_search'; //Saved Search only visible to administrators
var ctx = nlapiGetContext();
var iobj;

var csvHeader = 'Skipped Invoice InternalID, Skipped Invoice #, Skipped Error Msg\n';
var csvBody = '';
var csvSendTo = 'joe.son@audaxium.com';

//main function
function CorrectInvoices() {
...
function processInvoice() {
	var inv = nlapiLoadRecord('invoice',iobj.id);
	try {
		//load the invoice record for processing.
		
		var itmcnt = inv.getLineItemCount('item'); //get total count of this Invoices' line items
		var updInv = false;
		log('DEBUG','invoice: '+iobj.id,'Item Count: '+itmcnt);
		for (var i=1; i<=itmcnt; i++) {
			//objectize each line item for each of use 
			var itmobj = new Object();
			itmobj.dept = inv.getLineItemValue('item','department',i);
			itmobj.liclass = inv.getLineItemValue('item','class',i);
			itmobj.item = inv.getLineItemValue('item','item',i);
			itmobj.itmtype = inv.getLineItemValue('item','itemtype',i);
			itmobj.linenum = i;
			itmobj.inv = inv;
		...
		//check to see if we need to update the invoice (submit the record)
		if (updInv) {
		...
		csvBody+= inv.getId()+','+inv.getFieldValue('tranid')+','+getErrText(e)+'\n';
...
function modifyItemLine(itmobj) {
	log('DEBUG','Exec modifyItemLine(itmobj) Inv Item/dept/class/itemType',itmobj.item+'//'+itmobj.dept+'//'+itmobj.liclass+'//'+itmobj.itmtype);
...
function setLineValues(itmobj) {
	...
	//if invoice item is missing dept AND replacement value is FOUND
	if (!itmobj.dept && deptVal) {
		log('DEBUG','Setting Dept','At line #'+itmobj.linenum);
		itmobj.inv.setLineItemValue('item','department',itmobj.linenum,deptVal);
	}
	
	if (!itmobj.liclass && classVal) {
		log('DEBUG','Setting Class','At line #'+itmobj.linenum);
		itmobj.inv.setLineItemValue('item','class',itmobj.linenum,classVal);
	}
	
	//log('DEBUG','Get Line item Values: ',itmobj.inv.getLineItemValue('item','department',itmobj.linenum));
}
...
function getMainSearch() {
	...
	return nlapiSearchRecord('invoice', MAIN_SEARCH_ID, flt,null);
}
