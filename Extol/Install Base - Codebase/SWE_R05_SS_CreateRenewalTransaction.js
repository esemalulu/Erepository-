...
//Audaxium Mod: 6/13/2014
//Optional script parameter that gets a date to execute as.
//When value is passed in, script will assume todays' date is date passed in
//	When no value is passed in, uses current date as Today
var AX_EXEC_AS_DATE			= nlapiGetContext().getSetting('SCRIPT', 'custscript_axr05_executeasdate');
...
function main() {
	...
	//  	When no value is passed in, uses current date as Today
    if (AX_EXEC_AS_DATE) {
    	dtToday = new Date(AX_EXEC_AS_DATE);
    }
...
//                        }
                        ...
                        if(arrProductResults != null && arrProductResults != undefined && arrProductResults != '') {
                            ...
                            for (var iProdIdx = 0; iProdIdx < arrProductResults.length; iProdIdx++){
                                ...
                                logger.debug(MSG_TITLE, 'stProdId:[' + stProdId + ']  Index [' + iProdIdx + '/' + arrProductResults.length + '] // stCoTermDate [ '+stCoTermDate+' ].');
...
//                                         );
                                        ...
                                        if(stItemUsedToReplace != null && stItemUsedToReplace != undefined && stItemUsedToReplace != '') {
                                            stItemUsedToRenew   = nlapiLookupField('item',stItemUsedToReplace,'custitem_renew_with');
                                        }
                                ...
                                if (PRICING_MODEL == PRICING_MODEL_CURRENT_LIST) {
                                    try {
                                        ...
                                        for (var i = 1; i <= iLineCount; i++) {
                                            ...
                                            //recTran.setLineItemValue('item', 'price', i, '-1'); //COMMENTED: Fix for Issue 191573
                                            /* ===== Inline Discounting START ===== */
                                            // Calculate Discount
