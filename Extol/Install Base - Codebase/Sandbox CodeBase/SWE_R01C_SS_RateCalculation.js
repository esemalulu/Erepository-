 ...
 *
 * @author Michael Franz V. Sumulong, Ruel Dizon
 * @version 1.0
 */
function afterSubmit_rateCalculation(stType)
{
    var logger = new Logger(false);
    var MSG_TITLE = 'afterSubmit_rateCalculation';
    ...
    recTran = nlapiLoadRecord(stRecType, stRecordId);
    ...
    var stTranType = recTran.getFieldValue('type');
    ...
    var stTranStartDt = recTran.getFieldValue('startdate');
    var stTranEndDt = recTran.getFieldValue('enddate');
    ...
    logger.debug(MSG_TITLE, 'Line Item Count:' + recTran.getLineItemCount('item'));
    var bIsItemGroup = false;
    var iItemGroupLineCnt = 0;
    for (var idx = 1; idx <= recTran.getLineItemCount('item'); idx++)
    {
        
        var stItemListRate = recTran.getLineItemValue('item','custcol_list_rate',idx);
        if (isEmpty(stItemListRate)) 
        {
            resetListPrice(idx); // reset list price
            validateLine_CalculateRate('item',idx); // run validate Line function            
        }

        // recalc function begin
        recTran.setLineItemValue('item','custcol_reset_renewal_data',idx,'F');
        var flAmount = recTran.getLineItemValue('item','amount',idx);
        var bDisplayAmt = ('T' == recTran.getLineItemValue('item','custcol_display_printed_amount', idx));
        if(bDisplayAmt)
        {
            recTran.setLineItemValue('item','custcol_display_amount',idx,flAmount);
        }
        ...
        //        var stItemType = nlapiLookupField('item', recTran.getLineItemValue('item', 'item', idx), 'recordType');
        var stItemType = recTran.getLineItemValue('item', 'itemtype', idx);
        ...
        var stItemCat = recTran.getLineItemValue('item', 'custcol_item_category', idx);
        var bIsProjItem = (recTran.getLineItemValue('item', FLD_IS_PROJECT_ITEM, idx) == 'T');
        ...
        if (bIsItemGroup || bIsProjItem)
        {
            ...
            {
                logger.debug(MSG_TITLE,'Alert: You are saving a transaction with an Item Group that has ' + (iItemGroupLineCnt - ITEM_COMMIT_LIMIT) + ' Group Line Items remaining to process, which is more than the ' + ITEM_COMMIT_LIMIT + ' Line Items limit. The next ' + ITEM_COMMIT_LIMIT + ' Group Line Items will be processed when you click the "Process Group" button.');
                return false;
            }

            var flCurListRate = recTran.getLineItemValue('item','custcol_list_rate',idx);
            var bIsOutOfSynch = (recTran.getLineItemValue('item','custcol_out_of_synch',idx) == 'T');
            if(flCurListRate == null || flCurListRate == '' || flCurListRate == undefined || bIsOutOfSynch)
            {
                ...
                {
                    var stMSPercent = recTran.getLineItemValue('item', 'custcol_mtce_support_percent', idx);
                    var stMSType = recTran.getLineItemValue('item', 'custcol_mtce_support_type', idx);
                    ...
                    if (stMSPercent == null || stMSPercent == undefined || stMSPercent == '')
                    {
                        if (stMSType != null && stMSType != undefined && stMSType != '')
                        {
                            ...
                            if (stMSPercent != null && stMSPercent != undefined && stMSPercent != '')
                            {
                                ...
                                recTran.setLineItemValue('item', 'custcol_mtce_support_percent',idx, stMSPercent);
                            }
                        }
                    }
                }
        ...
        if(MS_MODEL == MS_MODEL_PERCENTAGE || MS_MODEL == MS_MODEL_PERCENTAGE_NET)
        {
            ...
            {
                //nlapiSelectLineItem('item', idx);
                logger.debug(MSG_TITLE,'Alert: You have edited a transaction line that effects a percentage Maintenance/Support calculation in another transaction line.\nPlease edit and commit each Maintenance/Support transaction line effected by your change.');
                return false;
            }
        }
        
        var flListRate = recTran.getLineItemValue('item', 'custcol_list_rate', idx);
        if (flListRate == null || flListRate == undefined || flListRate == '')
        {
            logger.debug(MSG_TITLE,'Alert: The list rate seems to be missing for transaction line ' + idx + '. Please review that transaction line first.');
            return false;
        }

        if (searchInList(arrTermCat, stItemCat) || searchInList(arrMaintCat, stItemCat) || searchInList(arrSupportCat, stItemCat))
        {
            var iTermInMonths = recTran.getLineItemValue('item', 'revrecterminmonths', idx);
            var stStartDt = recTran.getLineItemValue('item', 'revrecstartdate', idx);
            var stEndDt = recTran.getLineItemValue('item', 'revrecenddate', idx);
            if (iTermInMonths == null || iTermInMonths == undefined || iTermInMonths == '')
            {
                logger.debug(MSG_TITLE,'Alert: The term in months seems to be missing for transaction line ' + idx + '. Please review that transaction line first.');
                return false;
            }
            if(stTranType != 'opprtnty' && stTranType != 'estimate'){
                if (stStartDt == null || stStartDt == undefined || stStartDt == '')
                {
                    logger.debug(MSG_TITLE,'Alert: The rev. rec. start date seems to be missing for transaction line ' + idx + '. Please review that transaction line first.');
                    return false;
                }
                if (stEndDt == null || stEndDt == undefined || stEndDt == '')
                {
                    logger.debug(MSG_TITLE,'Alert: The rev. rec. end date seems to be missing for transaction line ' + idx + '. Please review that transaction line first.');
                    return false;
                }
    ...
    // update record
    var updatedRecId = nlapiSubmitRecord(recTran,true);
    logger.debug(MSG_TITLE,'Updated Record Id: '+updatedRecId);    
 ...
 * @version 1.0
 */
function resetListPrice(iLineIndex)
{
    var logger = new Logger(false);
    var MSG_TITLE = 'resetListPrice';
    logger.enableDebug(); //comment this line to disable debug
    ...
    var stListName = 'item';
    var stFieldName = 'price';
    /* Clear the List Rate */
    //nlapiSetCurrentLineItemValue(stListName, 'custcol_list_rate', '', false,true);
    recTran.setLineItemValue(stListName, 'custcol_list_rate',iLineIndex, '');

    /* Reset the List Price field. */
    var stItemCat = recTran.getLineItemValue(stListName, 'custcol_item_category', iLineIndex);
    var stPriceLevel = recTran.getLineItemValue(stListName, stFieldName, iLineIndex);
    ...
    if (stPriceLevel != null && stPriceLevel != undefined && stPriceLevel != '')
    {
        ...
        if (stPriceLevel == -1)
        {
            var isCustomBlocked = (recTran.getFieldValue('custbody_block_custom_price_level') == YES);
            var isCustomMSAllowed = (recTran.getFieldValue('custbody_allow_custom_price_lvl_for_ms') == YES);
                ...
                if (isCustomBlocked)
                {
                    if (!isCustomMSAllowed)
                    {
                        if(!CUSTOM_PRICE_CHECK_DONE)
                        {
                            ...
                            recTran.setLineItemValue(stListName, 'price',iLineIndex, 1);
                        ...
                        else
                        {
                            ...
                            if(CUSTOM_PRICE_CHECK_ITERATION <= 10)
                            {
                                ...
                                recTran.setLineItemValue(stListName, 'price',iLineIndex,CUSTOM_PRICE_CHECK_ITERATION);
                            }
                            else
                            {
                                return;
    ...
    */
    ...
    var stItemCat = recTran.getLineItemValue(stListName, 'custcol_item_category', iLineIndex);
    var bIsAnnualRate = (recTran.getFieldValue('custbody_custom_price_is_annual_rate') == 'T');
    //var stPriceLevel = nlapiGetCurrentLineItemValue(stListName, 'price');
    var stPriceLevel = recTran.getLineItemValue(stListName, 'price',iLineIndex);
    ...
    {
        //var stItemType = nlapiGetCurrentLineItemValue('item', 'itemtype');
        var stItemType = recTran.getLineItemValue('item', 'itemtype',iLineIndex);
        if (stItemType != 'Discount'
                && stItemType != 'Markup'
                && stItemType != 'Description'
                && stItemType != 'Subtotal'
                ...
                )
        {
            if(bIsTerm || bIsMaint || bIsSupport)
            {
                ...
                var flCurListRate = recTran.getLineItemValue('item','custcol_list_rate',iLineIndex);
                if(flCurListRate!=null && flCurListRate != '' && flCurListRate != undefined && flCurListRate != 0)
                {
                    ...
                    recTran.setLineItemValue('item','custcol_list_rate',iLineIndex,Math.round(flCurListRate/12*10000)/10000);
                }
            }
        }
    }
    ...
    {
        var stTranId = recTran.getFieldValue('id');
        ...
        {
            logger.debug(MSG_TITLE,'Alert: Please make sure to reset each transaction line.\nYou can also do this by clicking on the \"Reset Lines\" button.');
        }
    }
    */
 ...
 * @version 1.1
 */
function validateLine_CalculateRate(stListName,iCurIdx)
{
    var logger = new Logger(false);
    ...
    */
    var stItemType = recTran.getLineItemValue('item', 'itemtype', iCurIdx);
    var stPriceLevel = recTran.getLineItemValue(stListName, 'price', iCurIdx);

    var stClassOfSale = recTran.getFieldValue('custbody_order_type');
    var stTranType = recTran.getFieldValue('baserecordtype');
    ...
    var stItemCat = recTran.getLineItemValue(stListName, 'custcol_item_category', iCurIdx);
    var bIsTerm = searchInList(arrTermCat, stItemCat);
    var bIsMaint = searchInList(arrMaintCat, stItemCat);
    var bIsSupport = searchInList(arrSupportCat, stItemCat);
    ...
    {
        var iCurTranLineCnt = recTran.getLineItemCount('item');
        //var stProdLine = nlapiGetCurrentLineItemValue('item', 'custcol_product_line');
        var stProdLine = recTran.getLineItemValue('item', 'custcol_product_line',iCurIdx);
        ...
        for(var iTLIdx = 1; iTLIdx <= iCurTranLineCnt; iTLIdx++)
        {
            ...
            var stCurProdLine = recTran.getLineItemValue('item','custcol_product_line',iTLIdx);
            var stCurItemCat = recTran.getLineItemValue('item','custcol_item_category',iTLIdx);
            ...
            {
                recTran.setLineItemValue('item','custcol_out_of_synch',iTLIdx,'T');
            }
        }
    }
    ...
    var bResetLine = (recTran.getLineItemValue('item','custcol_reset_renewal_data',iCurIdx)=='T');

    if(bResetLine)
    {
        //var stResetData = nlapiGetCurrentLineItemValue('item','custcol_renewal_reset_data');
        var stResetData = recTran.getLineItemValue('item','custcol_renewal_reset_data',iCurIdx);
        if(stResetData != null && stResetData != '' && stResetData != undefined)
        {
            ...
            for(i in arrResetData)
            {
                ...
                recTran.setLineItemValue('item',arrValuePair[0],iCurIdx,arrValuePair[1]);
            }
            return true;
        }
        else
    ...
    if (bIsRenewalSO) 
    {
        logger.debug(MSG_TITLE,'Alert: This is an auto-generated renewal transaction. Editing the transaction line is not allowed.\nPlease switch the order type to "Renewal - Manual" to make manual changes.');
        return false;
    }

    /* Move the rate to the list rate */
    //var stItemId = nlapiGetCurrentLineItemValue(stListName, 'item', iCurIdx);
    var stItemId = recTran.getLineItemValue(stListName, 'item', iCurIdx);
    ...
    if (stPriceLevel != null && stPriceLevel != undefined && stPriceLevel != '')
    {
        ...
        */
        var stOldPrice = recTran.getLineItemValue('item','price',iCurIdx);
        recTran.setLineItemValue('item','price',iCurIdx,'-1');
        recTran.setLineItemValue('item','price',iCurIdx,stOldPrice);
        ...
        if (bIsSupport || bIsTerm || bIsMaint) 
        {
            ...
            var flTermInMonths = recTran.getLineItemValue('item', "revrecterminmonths",iCurIdx);
            ...
            if (flTermInMonths == '' || flTermInMonths == null || flTermInMonths == undefined)
            {
                ...
                */
                var dtStartDate = recTran.getLineItemValue('item', "revrecstartdate",iCurIdx);
                var dtEndDate = recTran.getLineItemValue('item', "revrecenddate",iCurIdx);
        ...
        if (stPriceLevel != -1)
        {
            ...
            flRate = recTran.getLineItemValue(stListName, 'rate', iCurIdx);
            if (flTermInMonths == '' || flTermInMonths == null || flTermInMonths == undefined)
            {
                //nlapiSetCurrentLineItemValue(stListName, 'custcol_list_rate', flRate, false,true);
                recTran.setLineItemValue(stListName, 'custcol_list_rate',iCurIdx, flRate);
            }else
            {
                //nlapiSetCurrentLineItemValue(stListName, 'custcol_list_rate', (Math.round((flRate * flTermInMonths)*10000) / 10000), false,true);
                recTran.setLineItemValue(stListName, 'custcol_list_rate',iCurIdx, (Math.round((flRate * flTermInMonths)*10000) / 10000));
            }
        }
        else
        {
            //flRate = nlapiGetCurrentLineItemValue(stListName, 'custcol_list_rate', iCurIdx);
            flRate = recTran.getLineItemValue(stListName, 'custcol_list_rate', iCurIdx);
            if (flTermInMonths == '' || flTermInMonths == null || flTermInMonths == undefined)
            {
                //nlapiSetCurrentLineItemValue(stListName, 'rate', Math.round(flRate*100)/100, false,true);
                recTran.setLineItemValue(stListName, 'rate',iCurIdx, Math.round(flRate*100)/100);
            }else
            {
                //nlapiSetCurrentLineItemValue(stListName, 'rate', (Math.round((flRate * flTermInMonths)*100) / 100), false,true);
                recTran.setLineItemValue(stListName, 'rate',iCurIdx, (Math.round((flRate * flTermInMonths)*100) / 100));
            }
        }
    ...
    if (bIsSupport || bIsTerm || bIsMaint)
    {
        if (stTranType == 'salesorder' || stTranType == 'invoice' || stTranType == 'creditmemo' || stTranType == 'returnauthorization')
        {
            ...
            {
                logger.debug(MSG_TITLE,"Alert: Please make sure that Rev. Rec. Start Date of transaction line " + iCurIdx + " is provided.");
                return false;
            }
        }
    }
...
function processMaintenance(arrMaintBasisCat, iLineIndex)
{
    ...
    */
    var stProdLine = recTran.getLineItemValue('item', 'custcol_product_line', iLineIndex);
    var stPriceLevel = recTran.getLineItemValue('item', "price", iLineIndex);
    var stItemId = recTran.getLineItemValue('item', "item", iLineIndex);
    ...
    var flTermInMonths = recTran.getLineItemValue('item', "revrecterminmonths", iLineIndex);
     // If term in months is set, use it. Else, compute for it!
    if (flTermInMonths == '' || flTermInMonths == null || flTermInMonths == undefined)
    {
        /*
        ...
        */
        var dtStartDate = recTran.getLineItemValue('item', "revrecstartdate", iLineIndex);
        var dtEndDate = recTran.getLineItemValue('item', "revrecenddate", iLineIndex);
         // Make sure that the start date is provided.
        if (dtStartDate == '' || dtStartDate == null || dtStartDate == undefined)
        {
            logger.debug(MSG_TITLE,"Alert: Please make sure that Rev. Rec. Term in Months of transaction line " + iLineIndex + " is provided.");
            return false;
        }
    ...
    recTran.setLineItemValue('item','custcol_out_of_synch',iLineIndex,'F');
    ...
    if (stPriceLevel == -1)
    {
        ...
        */
        var flCustomRate = recTran.getLineItemValue('item', 'custcol_list_rate', iLineIndex);
        recTran.setLineItemValue('item', "rate",iLineIndex, (Math.round((flCustomRate * flTermInMonths)*100) / 100));
        return true;
    }
    ...
    switch (MS_MODEL) {
		...
		case MS_MODEL_PERCENTAGE_NET:
            ...
            var flPercentage = recTran.getLineItemValue('item', 'custcol_mtce_support_percent', iLineIndex);
            ...
            else
            {
                ...
                var stMSType = recTran.getLineItemValue('item', 'custcol_mtce_support_type',iLineIndex);
                ...
                if (stMSType != null && stMSType != undefined && stMSType != '')
                {
                    ...
                    if (stMSPercent != null && stMSPercent != undefined && stMSPercent != '')
                    {
                        logger.debug(MSG_TITLE, 'Setting M/S Percentage');
                        //nlapiSetCurrentLineItemValue('item', 'custcol_mtce_support_percent', stMSPercent, false,true);
                        recTran.setLineItemValue('item', 'custcol_mtce_support_percent',iLineIndex, stMSPercent);
	        ...
	        //nlapiSetCurrentLineItemValue('item', 'rate', Math.round(flPercentageAmt*100)/100, false,true);
            recTran.setLineItemValue('item', 'rate',iLineIndex, Math.round(flPercentageAmt*100)/100);
            ...
            recTran.setLineItemValue('item', 'custcol_list_rate',iLineIndex, Math.round(flListRate*10000)/10000);
		...
		case MS_MODEL_ITEMIZED:
	        ...
	        //        logger.debug(MSG_TITLE,'Alert: Please make sure that there is a defined Maintenance/Support % for this item.');
	        //        return false;
            ...
            if(stPriceLevel == null){
                ...
                if (stPriceLevel != -1)
                {
                    ...
                    */
                    var stOldPrice = recTran.getLineItemValue('item','price',iLineIndex);
                    recTran.setLineItemValue('item','price',iLineIndex,'-1');
                    recTran.setLineItemValue('item','price',iLineIndex,stOldPrice);
                    var flListRate = recTran.getLineItemValue('item', 'rate',iLIneIndex);

                }
                else
                {
                    //flListRate = nlapiGetCurrentLineItemValue(stListName, 'rate');
                    flListRate = recTran.getLineItemValue(stListName, 'rate',iLineIndex);
                }
            }
            ...
            var stItemPricingType = recTran.getLineItemValue('item', 'custcol_item_pricing_type',iLineIndex);
            ...
            /*	
	        nlapiSetCurrentLineItemValue('item', 'rate', Math.round(flTermAmt*100)/100, false,true);
	        nlapiSetCurrentLineItemValue('item', 'custcol_list_rate', Math.round(flListRate*10000)/10000, false,true);
            */

	        recTran.setLineItemValue('item', 'rate',iLineIndex, Math.round(flTermAmt*100)/100);
	        recTran.setLineItemValue('item', 'custcol_list_rate',iLineIndex, Math.round(flListRate*10000)/10000);
			break;
...
*/
    var dtStartDate = recTran.getLineItemValue(stListName, "revrecstartdate", iLineIndex);
    var dtEndDate = recTran.getLineItemValue(stListName, "revrecenddate", iLineIndex);
    var stPriceLevel = recTran.getLineItemValue(stListName, "price", iLineIndex);
    var stItemId = recTran.getLineItemValue(stListName, "item", iLineIndex);
    var stCurrency = recTran.getFieldValue('currency');
    ...
    if(stPriceLevel == null){
        ...
        if (stPriceLevel != -1)
        {
            ...
            */
            ...
            recTran.setLineItemValue('item','price',iLineIndex,'-1');
            recTran.setLineItemValue('item','price',iLineIndex,stOldPrice);
            var flOldRate = recTran.getLineItemValue('item', 'rate',iLineIndex);
        }
    }
    /* If Item Pricing Type is annual and not custom, convert to monthly rate. */
    //var stItemPricingType = nlapiGetCurrentLineItemValue('item', 'custcol_item_pricing_type');
    ...
    var flTermInMonths = recTran.getLineItemValue(stListName, "revrecterminmonths", iLineIndex);
     // If term in months is set, use it. Else, compute for it!
    if (flTermInMonths == '' || flTermInMonths == null || flTermInMonths == undefined)
    {
        // Make sure that the start date is provided.
    ...
    if (stPriceLevel == -1)
    {
        ...
        */
        var flCustomRate = recTran.getLineItemValue(stListName, 'custcol_list_rate', iLineIndex);
        recTran.setLineItemValue(stListName, "rate",iLineIndex, Math.round(flCustomRate * flTermInMonths *100)/100);
        return true;
    }
    ...
    */
    recTran.setLineItemValue(stListName, "custcol_list_rate",iLineIndex, Math.round(flNewRate / flTermInMonths * 10000)/10000);
    recTran.setLineItemValue(stListName, "rate",iLineIndex, Math.round(flNewRate*100)/100);
    return true;
}
...
function recalc_PopulateDisplayAmount(stType)
{
    if(stType == 'item')
    {
        ...
        {
            recTran.setLineItemValue('item','custcol_reset_renewal_data',iLineIdx,'F');

            var flAmount = recTran.getLineItemValue('item','amount',iLineIdx);
            var bDisplayAmt = ('T' == recTran.getLineItemValue('item','custcol_display_printed_amount', iLineIdx));
            if(bDisplayAmt)
            {
                recTran.setLineItemValue('item','custcol_display_amount',iLineIdx,flAmount);
            }
        }
    }
}
...
function CS_CommitItemGroupLines()
{
    ...
    {
        logger.debug(MSG_TITLE,'Alert: There are no Group Line Items below to process.');        
    }
    else
    {
...
function CS_CountItemGroupLines(iStartIdx)
{
    ...
    // added by ruel
    if (recTran.getLineItemCount('item') > 0) 
    {
        bIsProjItem = (recTran.getLineItemValue('item', FLD_IS_PROJECT_ITEM, iStartIdx) == 'T');    
    }

    for(var x=iStartIdx; x<=recTran.getLineItemCount('item'); x++)
    {
        var stItemType = recTran.getLineItemValue('item', 'itemtype', x);
        if (stItemType == 'Discount'
                || stItemType == 'Markup'
                || stItemType == 'Description'
                || stItemType == 'Subtotal'
        ...
        {
            var flCurListRate = recTran.getLineItemValue('item','custcol_list_rate',x);
            var bIsOutOfSynch = (recTran.getLineItemValue('item','custcol_out_of_synch',x) == 'T');
            if (flCurListRate == null || flCurListRate == '' || flCurListRate == undefined || bIsOutOfSynch) 
            {
                iCount++;
            }
