	...
	function (search, record, log, runtime, format, email, encode, file, task, sftp, error) {
					...
					if (curSegmentDelimeter && curElementDelimeter) {
						...
						for (var sodataLen = 0; sodataLen < sodataArray.length; sodataLen++) {
							...
							/*else if(curLineHeader == curDateTimeHeader){
								...
								log.debug('curLineElements is  : ', curLineElements);
								if (curLineElements[1] == 'ST') {
									...
									for (var stTotalLinesLen = 0; stTotalLinesLen < stTotalLines; stTotalLinesLen++) {
										...
										if (stNextSegmentLineHeader == curAdditionalNameInformationHeader) {
											if (stNextSegmentLineElements[1]) sovalues['attention'] = stNextSegmentLineElements[1];
											sodataLen++;
									...
									log.debug('ADDRESS INFO: ', { shipaddressee: sovalues['shipaddressee'], attention: sovalues['attention'], shiptoaddr1: sovalues['shiptoaddr1'], shiptoaddr2: sovalues['shiptoaddr2'], shiptocity: sovalues['shiptocity'], shiptostate: sovalues['shiptostate'], shiptozip: sovalues['shiptozip'], shipcountry: sovalues['shipcountry'] });
		...
		function getInputData() {
			try {
				...
				// return [
				// 	{
				// 		"edifilename": "Restockit-Acme.850_20240729T180039_638578728395989437.EDI",
				// 		"filedata": [
				// 			"ST~850~0004",
				// 			"BEG~00~DS~90620~~20240724",
				// 			"REF~CO~RSI125625",
				// 			"TD5~~~~T",
				// 			"N1~ST~JOE HICKEY",
				// 			"N2~ELVE'S CANDY CO. LLC",
				// 			"N3~6428 SH 108",
				// 			"N4~TEXARKANA~AR~71854",
				// 			"PER~DC~JOE HICKEY~TE~(903) 733-5956",
				// 			"PO1~1~4~CT~64.41~PE~VN~353035~BP~49399",
				// 			"CTT~1~4",
				// 			"SE~12~0004"
				// 		]
				// 	}
