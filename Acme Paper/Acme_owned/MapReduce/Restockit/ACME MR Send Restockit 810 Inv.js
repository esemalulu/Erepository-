 ...
 * Script Name      : ACME MR Send Restockit 810 Invoice
 * Version               : 2.0
 * Description        : This script will run a search to create a 810 Invoice EDI file in NetSuite and uploads the file to the Restockit SFTP server. 
 */
...
var Restockit_Inv810_SearchId_ScriptField = 'custscript_restockit_inv_810_search_id';

var SFTPPort = 22;
var Restockit_SFTPInteg_Username_ScriptField = 'custscript_restckit_sftpintg810_username';
var Restockit_SFTPInteg_Pwd_GUID_ScriptField = 'custscript_restckit_sftpintg810_pwd_guid';
var Restockit_SFTPInteg_URL_ScriptField = 'custscript_restckit_sftpintg810_sftp_url';
var Restockit_SFTPInteg_Hostkey_ScriptField = 'custscript_restckit_sftpintg810_host_key';
var Restockit_SFTPInteg_SFTPDir_ScriptField = 'custscript_restckit_sftpintg810_sftp_dir';
			...
			if(edi810FileStaticInformation){
				...
				if(!isEmpty(currentTransactionType)){
					if(!isEmpty(curTransactionTypeCode)){
						...
						else if(currentTransactionType == 'creditmemo'){
				...
				// BIG Segment
				invString += curBeginningSegmentHeader+curElementDelimeter+invShipDateFormatted+curElementDelimeter+invTranNumber+curElementDelimeter;  // Beginning of BEG Segment
				...
				var vendorPONumber = headerinfo.values.custbody_edi_network_po_no;
				...
				invString += curNameHeader+curElementDelimeter+entitySTIdentifierCode+curElementDelimeter+curShipAddresse+curSegmentDelimeter;  // ST N1 Segment
				...
				if(curShipCity || curShipState || curShipCountry || curShipZip ){
					invString += curGeographicLocationHeader+curElementDelimeter+curShipCity+curElementDelimeter+curShipState+curElementDelimeter+curShipZip;    // Beginning of ST N4 Segment
				...
				log.debug('curBillAttention is '+ curBillAttention, ' curBillCountry is '+ curBillCountry);
				invString += curNameHeader+curElementDelimeter+entityBTIdentifierCode+curElementDelimeter+curBillAddresse+curSegmentDelimeter;  // BT N1 Segment

				if(curBillAttention){
					invString += curAdditionalNameInformationHeader+curElementDelimeter+curBillAttention+curSegmentDelimeter;  // BT N2 Segment
				}
				...
				if(curBillAddress1){
					...
					invString += curAddressInformationHeader+curElementDelimeter+curBillAddress1;  // BT N3 Segment
					if( curBillAddress2 ){
						...
						var additionalSTAddressInfo = curBillAddress2 ;
						if(curBillAddress3 ){
							...
							additionalSTAddressInfo += ', '+curBillAddress3;
						}
						invString += curElementDelimeter+additionalSTAddressInfo;
					}
					invString += curSegmentDelimeter;
				...
				if(curBillCity || curBillState || curBillCountry || curBillZip ){
					invString += curGeographicLocationHeader+curElementDelimeter+curBillCity+curElementDelimeter+curBillState+curElementDelimeter+curBillZip;    // Beginning of BT N4 Segment

					if(curBillCountry ){
						invString += curElementDelimeter+curBillCountry;
					}
					else{
						invString += curElementDelimeter;
					}
				...
				log.debug('invString after BT N4 Segment is ', invString);
...
function getInputData() {
	try{
		...
		var restockit_Inv810_Search_ID =  runtime.getCurrentScript().getParameter({name: Restockit_Inv810_SearchId_ScriptField});
		log.debug('Restockit Invoice 810 Search ID is ', restockit_Inv810_Search_ID);

		if(restockit_Inv810_Search_ID){
			var restockit_Inv810_Search_Obj = search.load({
				id: restockit_Inv810_Search_ID
			});
...
function updateInvoice_EDIStatus(invoiceId){
	try{
		log.debug('updatePO_EDIStatus function: invoiceId is ', invoiceId);
