...
function scheduled(type) {
	if(type=='scheduled' || type=='userinterface')
	{
		try
		{
			 var arrSearchResult = largeSavedSearch('customsearchitems_to_del');
			 ...
			 if ( arrSearchResult != null )
			    {
			    	for (var i = 0; i < arrSearchResult.length; i++)
			    	{
			    		try
			    		{
                            ...
                            var displayname = arrSearchResult[i].getValue('displayname');
                          
                         
				    		if(recordInternalId)
				    		{
				   	            nlapiLogExecution('DEBUG', 'recordInternalId', 'Internalid = '+recordInternalId + 'displayname: ' +displayname );
                 	    			nlapiDeleteRecord('inventoryitem', recordInternalId);
				    		}
			    		}
			    		catch(error)
			   		 {                     
...
function largeSavedSearch(id)
{
	...
	var recSearch = nlapiLoadSearch('item', 'customsearchitems_to_del');
