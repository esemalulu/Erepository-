...
function orderGuideSuitelet(req, res){
 if(req.getMethod() == "POST"){
  ...
  try {
    ...
    if(mode == "create"){
      ...
      if(guide.copy_items_from){
        var q_script_id = "customscript_copy_guide_items_q_sched"
        var q_deployment_id = "customdeploy_copy_guide_items_q_sched"

        var q_payload = {copy_from: guide.copy_items_from, copy_to: new_guide_id}

        var new_item = addQueueItem(JSON.stringify(q_payload), q_deployment_id, q_script_id)

        var task_id = initiateQueueScript(q_deployment_id, q_script_id, null)
      }
