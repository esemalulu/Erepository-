...
function guidesSalesOrdersSuitelet(req, res){
 if(req.getMethod() == "POST"){
  ...
  try {
    ...
    if(mode == "get_item_prices"){
      ...
      if(!item_ids){
        throw new Error("Item ids not provided")
      }
      if(!item_ids.length || item_ids.length < 1){
        throw new Error("Item ids not provided")
      }
      nlapiLogExecution("ERROR", "items", JSON.stringify(item_ids))
      ...
      if(module_settings){
        ...
        if(module_settings.columns.custrecord_rebates_module){
          ...
          nlapiLogExecution("ERROR", "rebates resposne", JSON.stringify(response))
          if(response.body){
            try {
              ...
              nlapiLogExecution("ERROR", "rebate res err", err)
              throw new Error("Error parsing response from rebates module")
            }
          }
        }
        ...
        if(module_settings.columns.custrecord_advanced_pricing_module){
          ...
          nlapiLogExecution("ERROR", "ap record reponse", JSON.stringify(response))
          if(response.body){
      ...
      return res.write(JSON.stringify(response_body))
