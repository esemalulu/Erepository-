 ...
 * Module Description
 * 
 * Version Date Author Remarks 1.00 01 April 2021 Suite Professionals Copyright 2021, Suite Professionals, All rights reserved.
 * 
 * This software is confidential and proprietary information of Suite Professionals - ("Confidential Information"). You shall not disclose such
...
function suiteletGPExport(request, response) {
	...
	if (request.getMethod() == 'GET') {
		...
		list.addField('quantity', 'float', 'Quantity');
		list.addField('distprice', 'currency', 'Dist Price');
		list.addField('paprice', 'currency', 'PA Price');
		...
		filters[0] = new nlobjSearchFilter('custcol_rebate_vendor', null, 'anyof', vendor);
		...
		var searchResults = nlapiSearchRecord(null, '2186', filters);
		...
		//	var dunsNumber = 'DIXIEPAPER';
			...
			for(var v=0; v < searchResults.length; v++){
				...
				var qty = resultSet1.getValue(columns[10]);
				var billPrice = resultSet1.getValue(columns[16]);
				var contractCost = resultSet1.getValue(columns[13]);
			...
			headerResultText += "1" + "\t" + "1" + "\t" + "2" + "\t" +"            "+ distId + "\t" + "Acme Paper and Supply Co." + "\t"+"\t"
					+ "8229 Sandy Court" + "\t" +"            "+ "P.O. BOX 422" + "\t"+"        " + "Savage" + "\t"+"        " + "MD" + "\t" + "20763" + "\t" + "1" + "\t" + fromDate + "\t" + toDate
			...
			//	var warehouse = resultSet.getText(columns[1]);
				...
				var documentNumber = resultSet.getValue(columns[1]);
				var customerNumber = resultSet.getValue(columns[2]);
				var customerName = resultSet.getValue(columns[3]);
				var shipAddress1 = resultSet.getValue(columns[4]);
				var shipCity = resultSet.getValue(columns[5]);
				var shipState = resultSet.getValue(columns[6]);
				var shipZip = resultSet.getValue(columns[7]);

				var vendor = resultSet.getValue(columns[11]);
				var rebateId = resultSet.getText(columns[12]);
				var endUserNum = resultSet.getValue(columns[15]);
				var dixieItemNumber = resultSet.getText(columns[8]);
				var mpn = resultSet.getValue(columns[17]);

				var description = resultSet.getValue(columns[18]);
				var billPrice2 = resultSet.getValue(columns[16]);
			//	var contractCost = resultSet.getValue(columns[22]);
				var qty2 = resultSet.getValue(columns[10]);
				var uom = resultSet.getValue(columns[9]);
			//	throw new Error(uom);
			...
			//	rebateId = rebateId.replace(/\D/g,'');
				...
				list.setLineItemValue('quantity', itemlin, qty2);
				list.setLineItemValue('distprice', itemlin, billPrice2);
