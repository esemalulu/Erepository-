...
function findEmailAutomationRecords()
{
	...
	try
	{
		...
		for (var t=0; tsrrs && t < tsrrs.length; t+=1)
		{
		    ...
		    if (curDate.getMonth() > 6 || curDate.getDate() >= dayLookedFor.getDate())
		    {
		    	var procJson = {
			    ...
			    if (corers && corers.length > 0)
			    {
				    ...
				    */
				    		...
				    		else
				    		{
				    			...
				    			try
				    			{
				    				...
				    				catch(trxsaveerr)
				    				{
				    					throw nlapiCreateError(
				    							'TRX_SAVE_ERROR',
				    							'Emails and Followup Calls (If required) process but Failed to save transaction with processed saved search: '+ getErrText(trxsaveerr),
				    							true
				    						  );
				    				}
				    				
				    				//2. Add/Update to the Email Automation Processed Custom record with success
					    			if (!coreProcTrxJson[procTrxJson.traninternalid])
					    			{
					    				//Create New custom record
					    				var eapRec = nlapiCreateRecord('customrecord_email_automation_proc');
				    ...
				    //TODO: Generate failed trx email
