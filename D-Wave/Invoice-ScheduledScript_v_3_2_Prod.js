...
function scheduled(type) {
    ...
    if (environment == 'PRODUCTION') {
        ...
        var param_url = context.getSetting('SCRIPT', 'custscript_sandbox_beta_url');
        var param_APIKey = context.getSetting('SCRIPT', 'custscript_sandbox_beta_api');
    ...
    if (response.getCode() == '200') {
        ...
        for (var i = 0; i < invoiceHeaderNodes.length; i++) {
            ...
            nlapiLogExecution('AUDIT', 'Processing Coupa Invoice', 'Invoice Number = ' + tranid + ' Vendor = ' + nlapiSelectValue(invoiceHeaderNodes[i], 'supplier/number') + ' Coupa Invoice Id = ' + externalid);
            ...
            if (invoiceexists == 'false') {
                try {
                    ...
                    nlapiLogExecution('AUDIT', 'Cannot create Vendor Bill ', 'Invoice Number = ' + tranid + ' Vendor = ' + nlapiSelectValue(invoiceHeaderNodes[i], 'supplier/name') + ' Coupa Invoice Id = ' + externalid + ' Netsuite Vendor Bill id = ' + invoiceexists + '. Error Discription :' + e.toString());
                    if (environment == 'PRODUCTION') {
                        ...
                        /*nlapiSendEmail(1287, 1287, ' Invoice Integration:Processing Error - Cannot create Vendor Bill Number = ' + tranid +
                         ...
                         invoiceexists, '');*/
                        nlapiSendEmail(1287, 1287, ' Invoice Integration:Processing Error - Unable to create vendor bill', 'Invoice Number = ' + nlapiSelectValue(invoiceHeaderNodes[i], 'invoice-number') + ' Supplier Name = ' + nlapiSelectValue(invoiceHeaderNodes[i], 'name') + ' Error Description = ' + e.message);
                    }
...
function CreateVendorCredit(invoice) {
    ...
    try {
        ...
        nlapiSendEmail(1349, nlapiGetContext().getSetting('SCRIPT', 'custscript_emailaddr_notifications'), nlapiGetContext().getSetting('SCRIPT', 'custscript_acccountname') + ' Invoice Integration:Processing Error - Unable to create vendor credit', 'Invoice Number = ' + nlapiSelectValue(invoice, 'invoice-number') + ' Supplier Name = ' + nlapiSelectValue(supplierNode, 'name') + ' Error Description = ' + e.message);
        return;
    }
...
function CreateVendorBill(invoice) {
    ...
    //Changed to USA IN 07/21/2016
    var nsVendorSubsidiaryInternalId = 8;

    //var accountName = nlapiSelectValue(invoice, 'account-type/name');
    var accountName = nlapiSelectValue(supplierNode, 'primary-subsidiary');

    nlapiLogExecution('debug', 'Vendor accountName', accountName);

    if (accountName == 'D-Wave (Government) Inc.') {
        nsVendorSubsidiaryInternalId = 2;
    } else if (accountName == 'D-Wave (Commercial) Inc.') {
        nsVendorSubsidiaryInternalId = 4;
    } else if (accountName == 'D-Wave Quantum Solutions Inc.') {
        nsVendorSubsidiaryInternalId = 9;
    }  else if (accountName == 'D-Wave Europe Consolidation sub') {
        nsVendorSubsidiaryInternalId = 14;
    }  else if (accountName == 'D-Wave UK Ltd.') {
        nsVendorSubsidiaryInternalId = 15;
      }  else if (accountName == 'D-Wave Japan Co., Ltd.') {
        nsVendorSubsidiaryInternalId = 13;
    }  else if (accountName == 'D-Wave Quantum Services Europe Limited') {
        nsVendorSubsidiaryInternalId = 16;
    
   }  else if (accountName == 'Omni USD') {
        nsVendorSubsidiaryInternalId = 10;
  }
  
  else if (accountName == 'Omni Circuit Boards Ltd.') {
        nsVendorSubsidiaryInternalId = 11;
  }
    ...
    nlapiLogExecution('debug', 'Supplier Sub' + record.getFieldValue('nlsub'), record.getFieldValue('subsidiary') + ' Primiary from Coupa ' + accountName)
    ...
    /**
     * If Sub = D-Wave Systems (Delaware)=2 - Location = D-Wave Systems
     * (Delaware) - Dont populate project ( Customer column on the VB). - No tax
     * code - If Sub = D-Wave Systems (USA)= 4 - Location = D-Wave Systems (USA) -
     * Dont populate project ( Customer column on the VB). - No tax code - Else -
     * BURNABY_LOCATION
     * OLD Logic DONT USE  07/22/16
     */
    if (nsVendorSubsidiaryInternalId == 4) {
        DEFAULT_LOCATION = '10';
        populateObject.populateLineItemProject = true;
        populateObject.populateLineExpeneTax = false;
    }
    else if (nsVendorSubsidiaryInternalId == 2) {
        DEFAULT_LOCATION = '4';
        populateObject.populateLineItemProject = true;
        populateObject.populateLineExpeneTax = false;

    }
    else if (nsVendorSubsidiaryInternalId == 9) {
        DEFAULT_LOCATION = 33;
        DEFAULT_TAX_ITEM_ID = 26995;
    }
    else if (nsVendorSubsidiaryInternalId == 8) {
        DEFAULT_LOCATION = 22;
    }
  
    else if (nsVendorSubsidiaryInternalId == 15) {
        DEFAULT_LOCATION = 39;
        populateObject.populateLineItemProject = true;
        populateObject.populateLineExpeneTax = false;
    }
  
  else if (nsVendorSubsidiaryInternalId == 16) {
        DEFAULT_LOCATION = 37;
        populateObject.populateLineItemProject = true;
        populateObject.populateLineExpeneTax = false;
    }
  
  else if (nsVendorSubsidiaryInternalId == 13) {  //d-wave japan
        DEFAULT_LOCATION = 38;
        populateObject.populateLineItemProject = true;
        populateObject.populateLineExpeneTax = false;
    }
  
  else if (nsVendorSubsidiaryInternalId == 10) {  //omni
        DEFAULT_LOCATION = 36;
        populateObject.populateLineItemProject = true;
        populateObject.populateLineExpeneTax = false;
    }
  
   else if (nsVendorSubsidiaryInternalId == 11) {  //omni
        DEFAULT_LOCATION = 35;
        populateObject.populateLineItemProject = true;
        populateObject.populateLineExpeneTax = false;
    }
    ...
    for (var x = 0; x < invoiceLineNodes.length; x++) {
        ...
        if (!boolCreateItemSublist) {
            ...
            record.setCurrentLineItemValue('expense', 'memo', nlapiSelectValue(invoiceLineNodes[x], 'description'));
            //record.setCurrentLineItemValue('expense', 'isbillable', 'T'); COMMENTED OUT AFTER 2019.1 RELEASE caused problems importing TJ
            nlapiLogExecution('debug', 'is NS Item', isNetSuiteItem);
            ...
            if (nlapiSelectNode(invoiceLineNodes[x], 'item')) {
                ...
                record = createItemsSublist(record, itemNumber, nlapiSelectValue(invoiceLineNodes[x], 'quantity'), nlapiSelectValue(invoiceLineNodes[x], 'price'), departmentNumeber, projectID, NSUnit, populateObject, DEFAULT_LOCATION);
    ...
    try {
        ...
        if (environment == 'PRODUCTION') {
            try {
                nlapiSendEmail(1287, 1287, ' Invoice Integration:Processing Error - Unable to create vendor bill', 'Invoice Number = ' + nlapiSelectValue(invoice, 'invoice-number') + ' Supplier Name = ' + nlapiSelectValue(supplierNode, 'name') + ' Error Description = ' + e.message);
            } catch (eMail) {
...
function createItemsSublist(recordObj, itemName, itemQuantity, itemRate, departmentNumeber, projectID, NSUnit, populateObject, DEFAULT_LOCATION) {
...
/**
 * Creates a saved search object based on provided type, filters and columns.
 * Runs the search to obtain the master results set. Then iterates through the
 * results set to obtain all results.
 *
 * @param {string}
 *        [required] type : The record internal ID of the record type you are
 *        searching
 * @param {nlobjSearchFilter |
 *        nlobjSearchFilter[] | Object[] } [optional] filters : A single
 *        nlobjSearchFilter object - or - an array of nlobjSearchFilter objects -
 *        or - a search filter expression.
 * @param {nlobjSearchColumn
 *        or nlobjSearchColumn[]} [optional] columns : A single
 *        nlobjSearchColumn object - or - an array of nlobjSearchColumn objects
 *
 * @return {nlobjSearchResult[]} results : concatenated array of all search
 *         results
 */
...
function VoidVendorBill(invoice, id) {
    ...
    try {
        ...
        nlapiSendEmail(1287, 1287, ' Invoice Integration:Processing Error - Unable to void vendor bill', 'Invoice Number = ' + nlapiSelectValue(invoice, 'invoice-number') + ' Supplier Name = ' + nlapiSelectValue(invoice, 'supplier/name') + ' Error Description = ' + e.message);
    }
...
function Setexportedtotrue(id) {
    ...
    var headers = new Array();
    headers['Accept'] = 'text/xml';
    headers['X-COUPA-API-KEY'] = context.getSetting('SCRIPT', 'custscript_apikey');
...
/**
 * This function will check if the a given date is within a closed or open
 * period
 * @param {Object}
 *        tranDateOnVB : Date to check period for.
 */
