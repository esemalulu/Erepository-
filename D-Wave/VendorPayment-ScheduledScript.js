 ...
 *              one email
 * 1.20      03 Dec 2018     pries            Add checkbox filter field for the VP search, 
 *                                            set the field to T on successful integration
 * 
 */
...
function scheduled(type) {
	...
	// v1.2 - added / updated - start
	filters[2] = new nlobjSearchFilter('custbody_dwave_vp_synced_to_coupa', null, 'is', 'F');
	filters[3] = new nlobjSearchFilter('postingperiod', null, 'rel', 'TP');

	if (fromdate && todate) {
		//nlapiLogExecution('DEBUG', 'condition if fromdate && todate', 'from = ' + fromdate + ' to = ' + todate);
		filters[4] = new nlobjSearchFilter('trandate', null, 'onOrAfter', fromdate);
		filters[5] = new nlobjSearchFilter('trandate', null, 'onorbefore', todate);
	}
	...
	if (!fromdate && todate) {
		...
		filters[4] = new nlobjSearchFilter('trandate', null, 'onOrAfter', todate);
	}

	if (!fromdate && !todate) {
		//nlapiLogExecution('DEBUG', 'condition if !fromdate && !todate');
		// v1.2 - deprecated --  filters[2] = new nlobjSearchFilter('trandate', null, 'onOrAfter', 'daysAgo1');
	}
	// v1.2 - added / updated - end
	...
	if (searchresults) {
		...
		for (var i = initialNbr; i < Math.min(maxNbr, InvpayMaxRecords); i++) {
			...
			try {
				...
				// 1.2 - added
				nlapiSubmitField('vendorpayment', record.getId(), 'custbody_dwave_vp_synced_to_coupa', 'T');
				nlapiLogExecution('DEBUG', 'Synced to Coupa field set to T on VP ' + record, getId());
			}
			catch (error) {
				if (error instanceof nlobjError) {
					...
					nlapiSendEmail(-5, nlapiGetContext().getSetting('SCRIPT', 'custscript_pay_emailaddr_notifications'), nlapiGetContext().getSetting('SCRIPT', 'custscript_pay_accountname') + ' Invoice Payment Integration:Processing Error - Exception',
...
function getCoupaInvoiceId(invoicenum, suppliernumber, tranid, topayamount, topaydate) {
	...
	var url = nlapiGetContext().getSetting('SCRIPT', 'custscript_payment_url') + '/api/invoices?invoice-number=' + encoded_invoicenum + '&&supplier[number]=' + suppliernumber + '&&status=approved';
	...
	for (var i = 0; i < paymentnode.length; i++) {
		...
		if (nlapiSelectValue(paymentnode[i], 'amount-paid') && nlapiSelectValue(paymentnode[i], 'payment-date')) {
			...
			if ((paidamount == parseFloat(topayamount)) && (tranid == checknumber) && (paiddate == topaydate)) { return 'DUPLICATE_PAYMENT'; }
		}
	}
...
function netsuitedatetoCoupadate(netsuitedate) {
	...
	//nlapiLogExecution('DEBUG', 'dateformat', dateformat);
	var datesplit;

	if (dateformat == 'DD-Mon-YYYY') {

		datesplit = netsuitedate.split("-");
		nlapiLogExecution('DEBUG', 'within dataformat == DD-Mon-YYYY', datesplit[2] + "-" + getMonthNum(datesplit[1]) + "-" + datesplit[0] + "T00:00:00-08:00");
		return datesplit[2] + "-" + getMonthNum(datesplit[1]) + "-" + datesplit[0] + "T00:00:00-08:00";
		//nlapiLogExecution('DEBUG', 'within dataformat == DD-Mon-YYYY', datesplit[2]+"-"+ datesplit[1]+"-"+datesplit[0]+"T00:00:00-08:00");
...
function getMonthNum(monthname) {
	nlapiLogExecution('DEBUG', 'within getMonthNum', 'monthname = ' + monthname);
	var months = {
...
function getMonthName(monthnum) {
	nlapiLogExecution('DEBUG', 'within getMonthName', 'monthnum = ' + monthnum);
	var months = {
...
function ConvertCoupaDateToNetSuiteDate(CoupaDate) {
	...
	if (dateformat == 'DD-Mon-YYYY') {
		...
		if (datesplit[2].charAt(0) == '0')
			...
			Nday = datesplit[2];
		return Nday + "-" + getMonthName(datesplit[1]) + "-" + datesplit[0];
	}
