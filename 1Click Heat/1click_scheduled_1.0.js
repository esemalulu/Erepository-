 ...
 * 1.00       03 Feb 2025     e.semalulu@1clickheat.com
 *
 */
...
*/    
	  ...
	  var searchResults = nlapiSearchRecord(null, 'customsearch3217');
...
//-------------------------------------------------------------------------------------------------------------------------------
            ...
            rec.setFieldValue('customform', '151');
        
            nlapiSubmitRecord(rec);
            //nlapiDeleteRecord('customrecord_cmms_equipment_service',searchResults[i].getId());
              ...
              if ((i+1)==100 || ((i+1) < searchResults.length && nlapiGetContext().getRemainingUsage() < 200))
              {
                //reschedule
...
*/    
	  ...
	  var searchResults = nlapiSearchRecord(null, 'customsearch2843');
...
//-------------------------------------------------------------------------------------------------------------------------------
            ...
            var userEmail = rec.getFieldValue('email')
            var passCode = rec.getFieldValue('custentity_pass_code')
            ...
            var template = 9;
		     ...
		     emailMerger.setEntity('employee', searchResults[i].getId());; // Set the ID of the employee where you are going to fetch the values to populate the variables on the template
		     ...
		     var emailSubject = mergeResult.getSubject(); // Get the subject for the email
		      var emailBody = mergeResult.getBody(); // Get the body for the email
		       ...
		       nlapiSendEmail(479006, userEmail, emailSubject, emailBody, null, null, null, null); // Send the email with merged template
...
/*
            for (var i=0; lineCnt && i < lineCnt; i+=1)
            {
                ...
                if(dept ){rec.setCurrentLineItemValue('item','department', dept)};
                if(clss ){rec.setCurrentLineItemValue('item', 'class', clss)};
                if(loctn ){rec.setCurrentLineItemValue('item','location', loctn)}; 
              
				rec.commitLineItem('item');         
         
            } 

            rec.setFieldValue('status', 'SalesOrd:B')
*/
            nlapiSubmitRecord(rec);
...
function deleteShepherdPhotos()
{
	...
	try
	{
        ...
        var searchResults = nlapiSearchRecord(null, 'customsearch2891');
