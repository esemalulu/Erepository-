...
function validateResetRequest(request, response){
	...
	try {
		if (request.getMethod() == 'POST') {
			...
			if(requestOriginatingFrom != toURL+'/c.663271/r7LicenseReset%2520Public/r7LicenseReset.html' || requestOriginatingFrom != toURL+'/c.663271/r7LicenseReset%2520Public/r7LicenseReset.html'){
...
function globalSearchLicense(license){
	...
	if (searchResults !=null) {
		for (var i = 0; i < searchResults.length; i++) {
			nlapiLogExecution('DEBUG','test','test'+i);
			...
			var dateExpiration = nlapiStringToDate(nlapiLookupField(recType, recId, expirationFieldId));
			nlapiLogExecution('DEBUG', 'expiration date', dateExpiration);
			var today = new Date();
			nlapiLogExecution('DEBUG', 'today', today);
			nlapiLogExecution('DEBUG', 'exp?', dateExpiration >= today);
