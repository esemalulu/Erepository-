...
function getAPIHeader(contentType, scope) {
    ...
    // noinspection JSVoidFunctionReturnValueUsed,JSIncompatibleTypesComparison
    var environment = context.getEnvironment() === 'PRODUCTION' ? 'prod' : 'sb';

    var clientURL = context.getSetting('SCRIPT', 'custscript_coupa_oidc_client_url_' + environment);
    var clientID = context.getSetting('SCRIPT', 'custscript_coupa_oidc_client_id_' + environment);
    var clientSecret = context.getSetting('SCRIPT', 'custscript_coupa_oidc_client_secret_' + environment);
