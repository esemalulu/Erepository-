/*
This script is used for posting calls from Shoretel DB to NetSuite.

GET-method is used for getting the last ShoretelCallId from NetSuite.
This ID will be used by Java-service for limiting the data volume got from Shoretel DB.

POST-method accepts Shotretel calls and submits them to NetSuite.
*/
...
function get(dataIn) {
	...
	if (dataIn.lastKnownId != null) {
		filter = new nlobjSearchFilter( 'custrecordr7phoneshoretelcallid', null, 'greaterthan',  dataIn.lastKnownId)
	} else {
		filter = new nlobjSearchFilter( 'custrecordr7phoneshoretelcallid', null, 'greaterthan',  lastId);// 9516000) //9850629
	}
	
	var columns = new Array();
	columns[0] = new nlobjSearchColumn( 'custrecordr7phoneshoretelcallid', null, 'max' );
...
function getEmployee(extension) {
	...
	var filters = new Array();
	filters.push(new nlobjSearchFilter( 'custentityr7employeeextension', null, 'equalto',  phoneNumber));
	filters.push(new nlobjSearchFilter( 'isinactive', null, 'is',  'F'));
...
function getContact(phone) {
	...
	var filter = new nlobjSearchFilter('custentityr7normalizedphone', null, 'equalto', phone);
...
function createRecord(dataIn) {
	...
	try {
		...
		result.shoretelCallId = dataIn.shoretelCallId;
	...
	if (employee != null) {
		...
		if (employee.inactive != 'F') {
			...
			result.statusDescription += 'Inactive employee found by given extension (' + dataIn.extension + ')' + '. ';
			result.status = 2;
			//ret.push(result);
			if (trackInactives != 1) {
				return result;
		...
		result.contactId = contactId;
		result.customerId = customerId;
		result.statusDescription += 'Didn\'t find employee by given extension (' + dataIn.extension + ')' + '. ';
		result.status = 2;
	}
	...
	record.setFieldValue('custrecordr7phoneshoretelcallid', dataIn.shoretelCallId);
	...
	record.setFieldValue('custrecordr7phoneextension', definedNull(extension));
	...
	if (definedNull(dataIn.phoneNumber) != null) {
		...
		return result;
		*/
		if ((dataIn.phoneNumber.length == 4) && (contact != null)) {
