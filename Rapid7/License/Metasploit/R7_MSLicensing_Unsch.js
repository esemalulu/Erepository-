 ...
 *  Scheduled (sc)<br/>
 * 
 *  _warning: R7_MSLicensing_Unsch.js is not registered as a script and is not deployed DEADCODE
 *  _warning: email sent from 2<br/>
 *  
 * existing:<br/>
 * filename: ./Metasploit Licensing/R7_MSLicensing_Unsch.js  ;<br/>
 * script id:<br/>
 * deploy id:<br/>
 * 
 * <br/>
 * proposed:<br/>
 * filename: ./Metasploit Licensing/r7_sc_mslicensingunsch.js<br/>
 * script id: customscript_r7_sc_mslicensingunsch<br/>
 * deploy id: customdeploy_r7_sc_mslicensingunsch<br/>
 * 
 * 
 * @class r7_sc_MSLicensingUnsch_DEAD
 * 
 */
...
function createLicenseAndNotifyContact(){
		...
		while (searchResults != null && nlapiGetContext().getRemainingUsage() > 200) {
			...
			if(searchResults!=null && searchResults.length>=2){
					...
					nlapiSendEmail(adminUser,adminUser,'Alert from Autocreate METASPLOIT License Script',errorText);
			}
		}
...
function processLicenseRecord(internalId){
			...
			if (nlapiGetContext().getRemainingUsage() > minimumCost) {
				...
				var nxposeLicense = "MSL"+internalId;
