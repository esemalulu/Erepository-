...
function beforeLoad(type, form, request){
	...
	if ((type == 'edit' || type == 'create') && context.getExecutionContext() == 'userinterface' && nlapiGetFieldValue('customform') == 48) {
		...
		try {
			...
			nlapiSendEmail(adminUser, adminUser, 'Error on Temp Partner Field - EVENTS', 'Error: ' + e);
		}

	}
	
	if (type == 'view' && userId == nlapiLoadConfiguration('companypreferences').getFieldValue('custscriptr7_system_info_email_sender')) {
...
function beforeSubmit(type){
	...
	if (type == 'delete') {
		...
		if (nlapiGetFieldValue('custeventr7lmeventtemplate') == 'T') {
			throw nlapiCreateError('NODELETE', 'Unable to delete. This event is used as a template for Integration Events. Uncheck the Integration Event Template field to allow deletion.', true);
		}
		else {
	...
	if ((type == 'edit' || type == 'create') && context.getExecutionContext() == 'userinterface' && nlapiGetFieldValue('customform') == 48) {
		...
		try {
			...
			var adminUser =  nlapiLoadConfiguration('companypreferences').getFieldValue('custscriptr7_system_info_email_sender');
			nlapiSendEmail(adminUser, adminUser, 'Error on Temp Partner Field - EVENTS BeforeSubmit', 'Error: ' + e);
		}
	}
	...
	if (type == 'edit' || type == 'create') {
		if (nlapiGetFieldValue('custeventr7eventconfcalltype') == 3) { //none
			...
			if (nlapiGetFieldValue('custeventr7eventconfcalltype') != null && nlapiGetFieldValue('custeventr7eventconfcalltype') != '') {
...
function runWebConferenceIntegration(){
	...
	var currentMeetingId = nlapiGetFieldValue('custeventr7gtmmeetingid');
	var recurring = (nlapiGetFieldValue('frequency') != 'NONE') ? true : false;
	var startDate = nlapiGetFieldValue('startdate');
	var endDate = (recurring && nlapiGetFieldValue('endbydate') != null && nlapiGetFieldValue('endbydate') != '') ? nlapiGetFieldValue('endbydate') : nlapiGetFieldValue('enddate');
	
	var dtStart = new Date(startDate + ' ' + nlapiGetFieldValue('starttime'));
	var dtEnd = new Date(endDate + ' ' + nlapiGetFieldValue('endtime'));

	var title = myEscapeXML(nlapiGetFieldValue('title'));
	var allDay = nlapiGetFieldValue('alldayevent');
	
	
	this.objMeeting = new meetingObject(gtm_Organizer, title, dtStart, dtEnd, currentMeetingId, recurring, allDay);
	...
	if (nlapiGetFieldValue('status') != 'CANCELLED') {
		...
		nlapiGetFieldValue('custeventr7eventconfcalltype') == 5) {
	...
	if (audienceInviteText != null && sendToCustomer == 'T') {
        for (var i = 0; audienceList != null && i < audienceList.length; i++) {
			...
			if (sent) {
				...
				integrationSystemNotes += logTimeStamp + ': ERROR sending cancellation email to ' + audienceList[i] + '<br>';
			}
        }
    }
    ...
    if (presenterInviteText != null) {
        for (var i = 0; presenterList != null && i < presenterList.length; i++) {
			...
			if (sent) {
				...
				integrationSystemNotes += logTimeStamp + ': ERROR sending cancellation email to ' + presenterList[i] + '<br>';
			}
		}
    }
}
...
function createICSInviteFileCancel(inviteText, attendees, salesRepICS){
	...
	var alarmTrigger = 'TRIGGER;RELATED=START:-PT15M';
	if (nlapiGetFieldValue('alldayevent') == 'T') {
		alarmTrigger = 'TRIGGER;RELATED=START:-PT18H';
	}
	
	var freeBusy = 'TRANSP:OPAQUE';
	if (nlapiGetFieldValue('custeventr7eventshowasfreetime') == 'T') {
		freeBusy = 'TRANSP:TRANSPARENT' +
		'\n' +
	...
	'\n' +
	'UID:' +
	nlapiGetFieldValue('custeventr7apimeetingid') +
	'@administration.rapid7.com' +
	'\n';
	
	contents += 'ATTENDEE;CUTYPE=INDIVIDUAL;ROLE=REQ-PARTICIPANT;PARTSTAT=ACCEPTED;CN=' + organizerName + ':mailto:' + organizerEmail + '\n';
	
	if (attendees != null && nlapiGetFieldValue('custeventr7eventinvitehideattendees') != 'T') {
	...
	'\n' +
	'SUMMARY:' +
	subject + //descriptionText + 
	'\n' +
	'DTSTART:' +
	objMeeting.icsstarttime +
	'\n' +
	'DTEND:' +
	objMeeting.icsendtime +
	'\n' +
	'DTSTAMP:' +
	nowTimeStamp +
	'\n' +
...
function createAPIEvent(){
	...
	if (nlapiGetFieldValue('custeventr7eventconfcalltype') == 1) {
		...
		if (objResponse == null) {
			...
			var adminUser =  nlapiLoadConfiguration('companypreferences').getFieldValue('custscriptr7_system_info_email_sender');
			nlapiSendEmail(adminUser, adminUser, 'GTM response body is null', nlapiGetFieldValue('organizer') + ': GTM probably did not get sent out. \n\n' + 'Event ID: ' + nlapiGetRecordId());
			integrationSystemNotes += logTimeStamp + ': Response body from server is null<br>';
			return;
		}
		...
		if (nlapiGetFieldValue('custeventr7eventconfcalltype') == 2) {
			try {
				...
				var adminUser =  nlapiLoadConfiguration('companypreferences').getFieldValue('custscriptr7_system_info_email_sender');
				nlapiSendEmail(adminUser, adminUser, 'Could not grab available bridge', 'location: ' + nlapiGetFieldValue('custeventr7eventbridgeloc') + '\nuser: ' + nlapiGetUser() + '\n\nError: ' + e);
				throw nlapiCreateError('ERROR', 'Could not find any available conference bridges.');
			}
			...
			if (nlapiGetFieldValue('custeventr7eventconfcalltype') == 4) { //GTM w/ Global Conferencing
				try {
					...
					nlapiLogExecution('DEBUG', 'audienceCallInfo', myEscapeXML(audienceCallInfo));
				} 
				catch (e) {
					var adminUser =  nlapiLoadConfiguration('companypreferences').getFieldValue('custscriptr7_system_info_email_sender');
					nlapiSendEmail(adminUser, adminUser, 'Could not grab available bridge', 'location: ' + nlapiGetFieldValue('custeventr7eventbridgeloc') + '\nuser: ' + nlapiGetUser() + '\n\nError: ' + e);
				...
				if (nlapiGetFieldValue('custeventr7eventconfcalltype') == 5) {
					deleteExistingMeetingFromEvent(true, true);
				}
				else {
					deleteExistingMeetingFromEvent(true, true);
	...
	if (nlapiGetFieldValue('custeventr7eventconfcalltype') == 1 || nlapiGetFieldValue('custeventr7eventconfcalltype') == 2 || nlapiGetFieldValue('custeventr7eventconfcalltype') == 4) { //call event
	...
	if (audienceInviteText != null) {
		for (var i = 0; audienceList != null && i < audienceList.length; i++) {
			...
			if (sent) {
				...
				integrationSystemNotes += logTimeStamp + ': ERROR sending invite email to ' + audienceList[i] + '<br>';
			}
		}
	}
	...
	if (presenterInviteText != null) {
		for (var i = 0; presenterList != null && i < presenterList.length; i++) {
			...
			if (sent) {
				...
				integrationSystemNotes += logTimeStamp + ': ERROR sending invite email to ' + presenterList[i] + '<br>';
			}
		}
	}
}
	...
	if (removedPresenterList != null) {
		...
		for (var i = 0; i < removedPresenterList.length; i++) {
			...
			if (sent) {
				...
				integrationSystemNotes += logTimeStamp + ': ERROR sending cancellation email to ' + removedPresenterList[i] + '<br>';
			}
		}
	}
	...
	if (audienceInviteText != null) {
		for (var i = 0; icsAudienceList != null && i < icsAudienceList.length; i++) {
			...
			if (sent) {
				...
				integrationSystemNotes += logTimeStamp + ': ERROR sending invite email to ' + icsAudienceList[i] + '<br>';
			}
		}
	}
	...
	if (presenterInviteText != null) {
		for (var i = 0; icsPresenterList != null && i < icsPresenterList.length; i++) {
			...
			if (sent) {
				...
				integrationSystemNotes += logTimeStamp + ': ERROR sending invite email to ' + icsPresenterList[i] + '<br>';
			}
		}
	}
}
...
function createICSInviteFile(inviteText, attendees, salesRepICS, locationDialInfo){
	...
	contents += 'SEQUENCE:' + nlapiGetFieldValue('custeventr7icssequenceid') +
	'\n' +
	'LOCATION:' +
	locationTextICS +
	'\n';
...
function parseAudienceList(idOnly, oldList){
	...
	var psoResources = nlapiGetFieldValues('custeventr7eventpsoresourceinvolv');
	...
	if (oldList) {
		...
		psoResources = recOld.getFieldValues('custeventr7eventpsoresourceinvolv');
	}
	
	for (var i = 0; customerContacts != null && i < customerContacts.length; i++) {
		audienceList[audienceList.length] = customerContacts[i];
	}
	
	for (var i = 0; partnerContacts != null && i < partnerContacts.length; i++) {
		audienceList[audienceList.length] = partnerContacts[i];
	}
	
	for (var i = 0; psoResources != null && i < psoResources.length; i++) {
		resourceList[resourceList.length] = psoResources[i];
	}
	
	if (idOnly) {
		resourceList = swapForActive('customrecordr7psoresources', resourceList, 'internalid');
		audienceList = swapForActive('contact', audienceList, 'internalid');
	}
	else {
		resourceList = swapForActive('customrecordr7psoresources', resourceList, 'custrecordr7psoresourcevendoremail');
		audienceList = swapForActive('contact', audienceList, 'email');
	}
	
	return audienceList.concat(resourceList);
}
...
function isThisOld(){
	...
	if (recurring && nlapiGetFieldValue('custeventr7eventconfcalltype') == 5) {
		var stEndDate = (recurring && nlapiGetFieldValue('endbydate') != null && nlapiGetFieldValue('endbydate') != '') ? nlapiGetFieldValue('endbydate') : nlapiGetFieldValue('enddate');
...
function getFieldChangeGroups(){
	...
	if (nlapiGetFieldValue('custeventr7resendmeetinginvites') == 'T') {
		objChangeGroups.nonAudienceChange = true;
		objChangeGroups.editFlag = true;
		nlapiSetFieldValue('custeventr7resendmeetinginvites', 'F');
	}
	...
	arrPresenterChangeFields[arrPresenterChangeFields.length] = 'custeventr7eventpsoresourceinvolv';
