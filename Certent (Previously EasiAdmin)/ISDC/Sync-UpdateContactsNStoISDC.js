 ...
 * 1.00       30 Mar 2016     WORK-rehanlakhani
 *
 */
...
function scheduled(type) {
	...
	var searchFilter = [
	                    	...
	                    	new nlobjSearchFilter('lastmodifieddate', null, 'onorafter', nlapiDateToString(twlvHoursAgoDate, 'datetime'), null),
	...
	var searchColumn = [
		                    ...
		                    new nlobjSearchColumn('custentity_aux_isdc_record_id')
	                   ];
		...
		for(var i = 0; i<resultSet.length; i+=1)
		{
			...
			var contact = {};
				...
				if(contacts.length == 50)
				{
					...
					{
						if(getErrText(upconerr).indexOf('RCRD_HAS_BEEN_CHANGED') > -1)
						{
							var lastUpdated = nlapiDateToString(today,'datetimetz');
							var rec = nlapiLoadRecord('contact', ids[y]);
								rec.setFieldValue('custentity_aux_isdc_lastupdated', lastUpdated);
						...
						{
							var eSbj = 'ISDC Error: Update Contact to ISDC Error' + ids[y],
