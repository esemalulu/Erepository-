 ...
 * 1.00       17 Mar 2016     WORK-rehanlakhani
 *
 */
...
function scheduled(type) {
	...
	var searchFilter = [
	                    	...
	                    	new nlobjSearchFilter('lastmodifieddate', null, 'onorafter', null, today)
	                    ];
	...
	top:do
	{
		...
		for(var i = 0; i<resultSet.length; i+=1)
		{
			...
			var account = {};
				account['id']                  = resultSet[i].getValue('custentity_aux_isdc_record_id');
		...
		if(accounts.length == 50)
		{
			try
			{
				...
				var updateAcc = {};
					updateAcc["operation"] = "updateAccounts";
					updateAcc["parameters"] = [accounts];
					
				var isdcRequest = nlapiRequestURL(URL, JSON.stringify(updateAcc), header, null, "POST");
				var isdcBody = JSON.parse(isdcRequest.body);
				...
				if(isdcBody != null)
				{
					for(var i = 0; i<isdcBody.length; i+=1)
					{
						...
						var cust = nlapiLoadRecord('customer', NSId);
							cust.setFieldValue('custentity_aux_isdc_lastupdated', lastUpdated);
			...
			catch (updaccterr)
			{
				...
				var eSbj = 'ISDC Error: Update Account to ISDC Error',
				eMsg = 'Error while processing SYNC-UpdateAccountsNStoISDC.js<br/>'+
					   '<b>ERROR: </b><br/>'+
					   getErrText(updaccterr) +'<br/><br/>' +
					   '<b>NS to ISDC JSON:</b><br/>'+
					   JSON.stringify(updateAcc);
	...
	if(resultSet.length > 50)
	{
		try
		{
			...
			if(isdcBody != null)
			{
				top:for(var i = 0; i<isdcBody.length; i+=1)
				{
					var lastUpdated = nlapiDateToString(date,'datetimetz');
					var cust = nlapiLoadRecord('customer', resultSet[i].getValue('internalid'));
						cust.setFieldValue('custentity_aux_isdc_lastupdated', lastUpdated);
					nlapiSubmitRecord(cust, false, true);
					if(nlapiGetContext().getRemainingUsage() <= 50)
					{
