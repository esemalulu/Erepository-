    	...
    	//try{
	    	for(var j=0;j<recIds.length;j++){
		    	...
		    	total_bill_price = dbt_active_price + dbt_upfront_price + dbt_repl_price + dbt_earlyterm_price + 
		    						...
		    						pd_active_price + pd_earlyterm_price + dbt_wp_price + htn_wp_price + pd_wp_price +
		    						bh2_active_price + bh2_pepm_price;// + 
		    						(dbt2_active_quantity*dbt2_active_rate) + (dbt2_wp_quantity*dbt2_wp_rate) +
		    						(htn2_active_quantity*htn2_active_rate) + (htn2_wp_quantity*htn2_wp_rate) +
		    						(wm2_active_quantity*wm2_active_rate) + 
		    						(pd2_active_quantity*pd2_active_rate) + (pd2_wp_quantity*pd2_wp_rate);
