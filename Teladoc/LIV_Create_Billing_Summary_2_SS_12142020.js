...
function(email, record, runtime, search, format, task) {
	...
	var dbt_wp_upfront_quantity = 0;
	var dbt_wp_upfront_rate = 0.0;
	var dbt_wp_upfront_price = 0.0;
	...
	var htn_wp_upfront_quantity = 0;
	var htn_wp_upfront_rate = 0.0;
	var htn_wp_upfront_price = 0.0;
	...
	var wm_upfront_quantity = 0;
	var wm_upfront_rate = 0.0;
	var wm_upfront_price = 0.0;
	...
	var pd_upfront_quantity = 0;
	var pd_upfront_rate = 0.0;
	var pd_upfront_price = 0.0;
	...
	var pd_wp_rate = 0.0;
	var pd_wp_price = 0.0;
	var pd_wp_upfront_quantity = 0;
	var pd_wp_upfront_rate = 0.0;
	var pd_wp_upfront_price = 0.0;
	...
	var bh2_earlyterm_quantity = 0;
	var bh2_earlyterm_rate = 0.0; 
	var bh2_earlyterm_price = 0.0; 
	...
	var total_upfront_amt = 0.0;
	var total_earlyterm_amt = 0.0;
	...
	var comb_wm2_htn_active_quantity = 0;
	var comb_wm2_htn_dbt_active_quantity = 0;
    ...
    function calculateData(scriptContext, recIds){  	
    	...
    	//try{
	    	for(var j=0;j<recIds.length;j++){
		    	...
		    	//Whole Person Diabetes Upfront
		    	if(rs_sku == 'wp_dbt_upfront'){
		    		//log.debug('dbtptm_pppm');
		    		dbt_wp_upfront_quantity = dbt_wp_upfront_quantity + rs_quantity;
		    		dbt_wp_upfront_rate = rs_rate;
		    		dbt_wp_upfront_price = dbt_wp_upfront_price + (rs_quantity * rs_rate);
		    		isRowProcessed = true;
		    	}
		    	...
		    	//Whole Person Hypertension Upfront
		    	if(rs_sku == 'wp_htn_upfront'){
		    		//log.debug('dbtptm_pppm');
		    		htn_wp_upfront_quantity = htn_wp_upfront_quantity + rs_quantity;
		    		htn_wp_upfront_rate = rs_rate;
		    		htn_wp_upfront_price = htn_wp_upfront_price + (rs_quantity * rs_rate);
		    		isRowProcessed = true;
		    	}
		    	...
		    	//WeightManagment upfront
		    	if(rs_sku == 'wm_upfront'){
		    		//log.debug('wm_upfront');
		    		wm_upfront_quantity = wm_upfront_quantity + rs_quantity;
		    		wm_upfront_rate = rs_rate;
		    		wm_upfront_price = wm_upfront_price + rs_price;
					isRowProcessed = true;
		    	}	
		    	...
		    	//Prediabetes upfront
		    	if(rs_sku == 'pd_upfront'){
		    		//log.debug('htn_upfront');
		    		pd_upfront_quantity = pd_upfront_quantity + rs_quantity;
					pd_upfront_rate = rs_rate;
					pd_upfront_price = pd_upfront_price + rs_price;
					isRowProcessed = true;
		    	}	
		    	...
		    	//Whole Person Prediabetes Upfront
		    	if(rs_sku == 'wp_htn_upfront'){
		    		//log.debug('dbtptm_pppm');
		    		pd_wp_upfront_quantity = pd_wp_upfront_quantity + rs_quantity;
		    		pd_wp_upfront_rate = rs_rate;
		    		pd_wp_upfront_price = pd_wp_upfront_price + (rs_quantity * rs_rate);
		    		isRowProcessed = true;
		    	}
		    	...
		    	//Behavioral Health Early terms
		    	if(rs_sku == 'bhptm_pppm'){
		    		//log.debug('bh_pepm');
		    		bh2_earlyterm_quantity = bh2_earlyterm_quantity + rs_quantity;
					bh2_earlyterm_rate = rs_rate;
					bh2_earlyterm_price = bh2_earlyterm_quantity * rs_rate;
					isRowProcessed = true;
		    	}	
		    	...
		    	if(rs_sku == 'wm_phase2_htn_pppm' || rs_sku == 'wm_htn_phase2_pppm' || rs_sku == 'wm_phase2_htn_phase2_pppm'){
					...
					if(rs_sku == 'wm_phase2_htn_pppm'){
						...
						comb_wm2_htn_active_quantity = comb_wm2_htn_active_quantity + rs_quantity;
					}
		    	...
		    	if(rs_sku.match(/wm/g) && rs_sku.match(/htn/g) && rs_sku.match(/dbt/g) && rs_sku.match(/pppm/g)){
			    	...
			    	if(rs_sku == 'wm_phase2_htn_dbt_pppm' || rs_sku == 'wm_htn_phase2_dbt_pppm' || rs_sku == 'wm_htn_dbt_phase2_pppm'
						...
						if(rs_sku == 'wm_phase2_htn_dbt_pppm'){
			    			...
			    			comb_wm2_htn_dbt_active_quantity = comb_wm2_htn_dbt_active_quantity + rs_quantity;
						}
		    	...
		    	*/
		    	...
		    	total_upfront_amt = dbt_upfront_price + dbt_wp_upfront_price + htn_upfront_price + htn_wp_upfront_price +
		    	wm_upfront_price + pd_upfront_price + pd_wp_upfront_price;
		    	log.debug('total_upfront_amt = ' + total_upfront_amt);
		    	 
		    	total_earlyterm_amt = dbt_earlyterm_price + dbt_earlyterm_wp_price + htn_earlyterm_price + htn_earlyterm_wp_price +
		    	wm_earlyterm_price + pd_earlyterm_price + pd_earlyterm_wp_price + bh2_earlyterm_price;
		    	log.debug('total_earlyterm_amt = ' + total_earlyterm_amt);
    ...
    function createBillingSummary2Record(){
    	...
    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_wp_upfront_meter_qty', value: parseInt(dbt_wp_upfront_quantity)});//WP UPFRONT METER QTY
    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_ns_wp_upfront_price', value: parseFloat(dbt_wp_upfront_rate)});//NS WP UPFRONT PRICE
    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_ns_wp_upfront_amt', value: parseFloat(dbt_wp_upfront_price)});//NS WP UPFRONT AMT
    	...
    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_ns_htn_wp_upfront_meter_q', value: parseInt(htn_wp_upfront_quantity)});//HTN WP UPFRONT METER QTY
    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_ns_htn_wp_upfront_price', value: parseFloat(htn_wp_upfront_rate)});//NS HTN WP UPFRONT PRICE
    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_ns_htn_wp_upfront_amt', value: parseFloat(htn_wp_upfront_price)});//NS HTN WP UPFRONT AMT
    	...
    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_wm_upfront_meter_qty', value: parseInt(wm_upfront_quantity)});//WM UPFRONT METER QTY
    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_ns_wm_upfront_price', value: parseFloat(wm_upfront_rate)});//NS WM UPFRONT PRICE
    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_wm_upfront_amt', value: parseFloat(wm_upfront_price)});//NS WM UPFRONT AMT
    	...
    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_dpp_upfront_meter_qty', value: pd_upfront_quantity});//WP UPFRONT METER QTY
    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_ns_dpp_upfront_price', value: pd_upfront_rate});//NS WP UPFRONT PRICE
    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_dpp_upfront_amt', value: pd_upfront_price});//NS WP UPFRONT AMT
    	...
    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_ns_dpp_wp_upfront_meter_q', value: pd_wp_upfront_quantity});//DPP WP UPFRONT METER QTY
    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_ns_dpp_wp_upfront_price', value: pd_wp_upfront_rate});//NS DPP WP UPFRONT PRICE
    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_ns_dpp_wp_upfront_amt', value: pd_wp_upfront_price});//NS DPP WP UPFRONT AMT
    	...
    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_bh2_earlyterm_qty', value: parseInt(bh2_earlyterm_quantity)});//BH2 Early Term Qty
    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_bh2_term_price', value: parseFloat(bh2_earlyterm_rate)});//BH2 Termination Price
    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_bh2_term_amt', value: parseFloat(bh2_earlyterm_price)});//BH2 Termination Amt
    	...
    	billlingSummary2Record.setValue({ fieldId: 'custrecord_upfront_total_amt', value: total_upfront_amt});//Upfront Total Amount
    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bundled_total_amt', value: total_earlyterm_amt});//Bundled Total Amount
 	   ...
 	   pccListSearchfilters.push(search.createFilter({ //create new filter
         ...
         start: 0,
 	  	...
 	  	if(pccListSearchRsultRange.length == 1){
 	  		clientCodeId = pccListSearchRsultRange[0].id;
 	  	}
 	  	else if(pccListSearchRsultRange.length == 0){
 	  		log.audit({    
	            title: 'createBillingSummary2Record', 
	            details: 'Client code: ' + clientCodeId + 'not present in the PCCode list.'
	        });
 	  	}
 	  	else{
 	  		...
 	  		log.audit({    
 	            title: 'createBillingSummary2Record', 
 	            details: 'More than 1 client code: ' + clientCodeId + 'present in the PCCode list.'
 	        });
 	  	}
 	  	...
 	  	if(clientCodeId){
	    	 var contratsSearch = search.create({
	    	        ...
	    	        columns: ['custrecord_fetch_partner_adm_fee_from_rs',
	    	                  'custrecord_fetch_partner_ref_fee_from_rs',
	    	                  'custrecord_liv_cm_admin_fee',
	    	                  'custrecord_liv_cm_htn_admin_fee',
	    	                  'custrecord_liv_cm_wm_admin_fee',
	    	                  'custrecord_liv_cm_dpp_admin_fee',
	    	                  'custrecord_liv_cm_partner_fee',
	    	                  'custrecord_liv_cm_contract_status',
	    	                  'custrecord_liv_cm_contract_end_date'
	    	                  ],
	    	        filters: [
		    ...
		    if(contratsSearchResultRange.length == 1){
		    	...
		    	if(fetchPartnerAdminFeeFromRS == true){
		    		//Get the Admin Fee from RS Table
		    		billlingSummary2Record.setValue({ fieldId: 'custrecord_get_partner_admin_fee_from_rs', value: true});// Get Partner Admin Fee From RS 
		    		...
		    		//Get the Admin Fee from Contract
		    		var dbtAdminFee = contratsSearchResultRange[0].getValue('custrecord_liv_cm_admin_fee');
		    		var htnAdminFee = contratsSearchResultRange[0].getValue('custrecord_liv_cm_htn_admin_fee');
		    		var wmAdminFee = contratsSearchResultRange[0].getValue('custrecord_liv_cm_wm_admin_fee');
		    		var dppAdminFee = contratsSearchResultRange[0].getValue('custrecord_liv_cm_dpp_admin_fee');
		    		var bundleAdminFee = contratsSearchResultRange[0].getValue('custrecord_liv_cm_bundled_admin_fee');
		    		
		    		var contractStatus = contratsSearchResultRange[0].getValue('custrecord_liv_cm_contract_status');
		    		var contractEndDate = contratsSearchResultRange[0].getValue('custrecord_liv_cm_contract_end_date');
		    		var monthEndDate = runtime.getCurrentScript().getParameter("custscript_process_rs_end_date");
		    		var formattedMonthEndDate = format.format({
		                value: monthEndDate,
		                type: format.Type.DATE
		            });
		    		/*log.debug({    
			            title: 'createBillingSummary2Record', 
			            details: 'formattedMonthEndDate: ' + formattedMonthEndDate
			        });*/
		    		
		    		if(contractStatus == 'Expired' && formattedMonthEndDate != contractEndDate){
		    			log.debug({    
				            title: 'createBillingSummary2Record', 
				            details: 'contractStatus: ' + contractStatus
				        });
		    			log.debug({    
				            title: 'createBillingSummary2Record', 
				            details: 'formattedMonthEndDate: ' + formattedMonthEndDate + ' :: contractEndDate: '+contractEndDate
				        });
		    			log.debug({    
				            title: 'createBillingSummary2Record', 
				            details: 'Skipping setting Admin Fee...' 
				        });
		    			//continue;
		    		}
		    		else{
		    			billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_ns_referral_fee', value: parseFloat(dbtAdminFee)});//NS ADMIN FEE
				    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_ns_dbt_admin_members', value: parseInt(dbt_active_quantity)});//NS DBT ADMIN MEMBER QTY
				    
				    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_ns_htn_admin_fee', value: parseFloat(htnAdminFee)});//NS HTN ADMIN FEE
				    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_ns_htn_admin_members', value: parseInt(htn_active_quantity)});//NS HTN ADMIN MEMBER QTY
			    	
				    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_ns_wm_admin_fee', value: parseFloat(wmAdminFee)});//NS WM ADMIN FEE
				    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_ns_wm_admin_members', value: parseInt(wm_active_quantity)});//NS WM ADMIN MEMBER QTY
			    	
				    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_ns_dpp_admin_fee', value: parseFloat(dppAdminFee)});//NS DPP ADMIN FEE
				    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_ns_dpp_admin_members', value: parseInt(pd_active_quantity)});//NS DPP ADMIN MEMBER QTY
				    	
				    	//Bundles
				    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_ns_htn_dbt_admin_fee', value: parseFloat(bundleAdminFee)});//NS HTN DBT ADMIN FEE
				    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_ns_htn_dbt_admin_members', value: parseInt(comb_htn_dbt_quantity)});//NS HTN DBT ADMIN MEMBER QTY
			    	
				    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_ns_wm_dbt_admin_fee', value: parseFloat(bundleAdminFee)});//NS WM DBT ADMIN FEE
				    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_ns_wm_dbt_admin_members', value: parseInt(comb_wm_dbt_quantity)});//NS WM DBT ADMIN MEMBER QTY
			    	
				    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_ns_wm_htn_admin_fee', value: parseFloat(bundleAdminFee)});//NS WM HTN ADMIN FEE
				    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_ns_wm_htn_admin_members', value: parseInt(comb_wm_htn_quantity)});//NS WM HTN ADMIN MEMBER QTY
			    	
				    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_ns_wm_htn_dbt_admin_fee', value: parseFloat(bundleAdminFee)});//NS WM HTN DBT ADMIN FEE
				    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_ns_wm_htn_dbt_admin_membe', value: parseInt(comb_wm_htn_dbt_quantity)});//NS WM HTN DBT ADMIN MEMBER QTY
			    	
				    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_ns_wm2_htn_admin_fee', value: parseFloat(bundleAdminFee)});//NS WM2 HTN ADMIN FEE
				    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_ns_wm2_htn_admin_members', value: parseInt(comb_wm2_htn_active_quantity)});//NS WM2 HTN ADMIN MEMBER QTY
			    	
				    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_ns_wm2_htn_dbt_admin_fee', value: parseFloat(bundleAdminFee)});//NS WM2 HTN DBT ADMIN FEE
				    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_ns_wm2_htn_dbt_admin_memb', value: parseInt(comb_wm2_htn_dbt_active_quantity)});//NS WM2 HTN DBT ADMIN MEMBER QTY
		    		}
		    	}
		    ...
		    else if(contratsSearchResultRange.length == 0){
		    	log.debug({    
		            title: 'createBillingSummary2Record', 
		            details: 'No Active Cobtarcts found for the the client code: ' + client_code
		        });
		    }
		    else{
		    	log.debug({    
 	  		...
 	  		log.debug({    
	            title: 'createBillingSummary2Record', 
	            details: 'Client code: ' + client_code + ' does not exist in PCCList.'
	        });
 	  	}
    ...
    function resetVariables(){
    	...
    	dbt_wp_rate = 0.0;
    	dbt_wp_price = 0.0;
    	dbt_wp_upfront_quantity = 0;
    	dbt_wp_upfront_rate = 0.0;
    	dbt_wp_upfront_price = 0.0;
    	...
    	htn_wp_rate = 0.0;
    	htn_wp_price = 0.0;
    	htn_wp_upfront_quantity = 0;
    	htn_wp_upfront_rate = 0.0;
    	htn_wp_upfront_price = 0.0;
    	...
    	wm_active_rate = 0.0;  
    	wm_active_price = 0.0; 
    	wm_upfront_quantity = 0;
    	wm_upfront_rate = 0.0;
    	wm_upfront_price = 0.0;
    	wm_repl_quantity = 0;
    	wm_repl_rate = 0.0;
    	wm_repl_price = 0.0;
    	...
    	pd_active_rate = 0.0;  
    	pd_active_price = 0.0; 
    	pd_upfront_quantity = 0;
    	pd_upfront_rate = 0.0;
    	pd_upfront_price = 0.0;
    	...
    	pd_wp_rate = 0.0;
    	pd_wp_price = 0.0;
    	pd_wp_upfront_quantity = 0;
    	pd_wp_upfront_rate = 0.0;
    	pd_wp_upfront_price = 0.0;
    	...
    	bh2_pepm_rate = 0.0; 
    	bh2_pepm_price = 0.0; 
    	bh2_earlyterm_quantity = 0;
    	bh2_earlyterm_rate = 0.0; 
    	bh2_earlyterm_price = 0.0; 
