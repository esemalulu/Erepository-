       ...
       var arrResult = nlapiSearchRecord('customrecord_liv_contracts','customsearch_liv_contracts', arrFilter, null);
        ...
        if(arrResult)
        {
         ...
         //    combprogramdesc = combprogramdesc + ' - ' + convertdate(billing_date, 'MONTHYY') + ' ('+grouping+' )'; 
             ...
             var combdmamt = bundleprice * (nvl(combactivemembers,0))  ; 
             var combhtnamt =  htnbundleprice * (nvl(combactivemembers,0))  ; 
              ...
              if (categorytext == 'Bundled')
              {
               ...
               if (nvl(flattermprice,0) == 0)
                ...
                // var totalbillingamt = (nvl(programamt,0))+ (nvl(replamt,0)) + (nvl(referralamt,0)) + (nvl(earlytermtotal,0))+(nvl(htnprogramamt,0))+(nvl(htnreplamt,0))+(nvl(htnupfrontamt,0))+(nvl(htnreferralamt,0))+(nvl(htnearlytermtotalamt,0))+(nvl(combprogramamt,0))+(nvl(combearlytermtotalamt,0));
                 var totalbillingamt = (nvl(programamt,0))+ (nvl(replamt,0)) + (nvl(referralamt,0)) + (nvl(earlytermtotal,0))+(nvl(htnprogramamt,0))+(nvl(htnreplamt,0))+(nvl(htnupfrontamt,0))+(nvl(htnreferralamt,0))+(nvl(htnearlytermtotalamt,0))+(nvl(combprogdiscamt,0))+(nvl(bundledreferralamt,0));
             ...
             else
             {
              ...
              // nlapiSetFieldValue('custrecord_bs2_cb_program_desc', combprogramdesc );
            //  nlapiSetFieldValue('custrecord_bs2_ns_cb_program_amt', combprogramamt );
            //  nlapiSetFieldValue('custrecord_bs2_ns_cb_term_item_desc',combearlytermdesc );
            // nlapiSetFieldValue('custrecord_bs2_ns_termination_desc',earlytermdesc );
