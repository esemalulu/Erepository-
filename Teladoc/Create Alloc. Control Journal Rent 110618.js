//last update date: 11/06/18 Create Alloc. Control Journal for Rent NS-235


function CreateAllocCtrlJournalRent()
{
 try
 {
	 // Search Stat journal for the same month
       ...
       var p_subsidiary = nlapiGetContext().getSetting('SCRIPT', 'custscript_rent_alloc_subsidiary');
       var p_date = nlapiGetContext().getSetting('SCRIPT', 'custscript_rent_alloc_date');
       var p_period = nlapiGetContext().getSetting('SCRIPT', 'custscript_rent_alloc_period');
       var p_department = nlapiGetContext().getSetting('SCRIPT', 'custscript_rent_alloc_dept');
       var p_location = nlapiGetContext().getSetting('SCRIPT', 'custscript_rent_alloc_loc');
       var p_allocinacct = nlapiGetContext().getSetting('SCRIPT', 'custscript_rent_alloc_in_acct');
       var p_allocoutacct = nlapiGetContext().getSetting('SCRIPT', 'custscript_rent_alloc_out_acct');
       var p_jebatchnumber = nlapiGetContext().getSetting('SCRIPT', 'custscript_rent_alloc_je_batch_number');
       ...
       nlapiLogExecution('DEBUG','param-location: '+p_location) ;
       ...
       searchFilter[3] = new nlobjSearchFilter('location', null, 'is', p_location  ); 
       
       
	   var searchresults = nlapiSearchRecord('transaction','customsearch_liv_alloc_control_je_rent', searchFilter , null);
       ...
       { 
           ...
           { 
               ...
               var location = results.getValue(columns[3]);  
			  ...
			  // Create allocation control journal
			  	try
		        {
		          ...
		          {	  
                    ...
                    jerecord.setFieldText('approvalstatus','Approved');
                    jerecord.setFieldValue('approved',true);
                    jerecord.setFieldValue('approvedSpecified',true);
	              ...
	              //Lines
	                ...
	                //jerecord.setCurrentLineItemValue('line', 'location', location);
              ...
              var strRecID = nlapiSubmitRecord(jerecord, true, true);
         	  nlapiLogExecution('DEBUG','Journal ID :'+strRecID) ;
