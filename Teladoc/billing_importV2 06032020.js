          ...
          for (var i = 0;  i < arrResult.length; i++)
           {
               ...
               //BH2
               var bh2pppmprice = results.getValue(columns[112]);   //custrecord_liv_cm_bh2_pppm	  -  BH 2.0 PPPM
               var bh2pmpmprice = results.getValue(columns[113]);   //custrecord_liv_cm_bh2_pmpm	  -  BH2 PMPM
               var bh2termfee = results.getValue(columns[114]);     // custrecord_liv_cm_bh2_termination_fee	  - BH2 Termination Fee
               var bh2contractterms = results.getValue(columns[115]); // custrecord_liv_cm_bh2_contract_term - BH2 Term in Months
			   var bh2programitem = results.getValue(columns[116]);  // custrecord_liv_cm_bh2_program_item  -  BH2 Program Item
			   var bh2replacementitem = results.getValue(columns[117]);  // custrecord_liv_cm_bh2_termination_item  -  BH2 Termination Item
			   var bh2terminationitem = results.getValue(columns[118]);  // custrecord_liv_cm_bh2_replacement_item  -  BH2 Replacement Item
         ...
         //    combprogramdesc = combprogramdesc + ' - ' + convertdate(billing_date, 'MONTHYY') + ' ('+grouping+' )';
             ...
             // NS-106
              ...
              if (categorytext == 'Upfront')
              {
                 ...
                 var totalbillingamt = (nvl(programamt,0))+ (nvl(replamt,0)) + (nvl(referralamt,0)) + (nvl(upfrontamt,0))+(nvl(htnprogramamt,0))+(nvl(htnreplamt,0))+(nvl(htnreferralamt,0))+(nvl(htnupfrontamt,0))+(nvl(combprogdiscamt,0))+(nvl(wmreferralamt,0))+(nvl(dppreferralamt,0));

              }+(nvl(bundledreferralamt,0))
              ...
              if (categorytext == 'Bundled')
              {
               ...
               if (nvl(flattermprice,0) == 0)
                ...
                var totalbillingamt = (Math.abs(parseFloat(nvl(rstotalbillingamt,0)))) + (nvl(wmreferralamt,0))+(nvl(dppreferralamt,0))+(nvl(bundledreferralamt,0))+(nvl(referralamt,0));
