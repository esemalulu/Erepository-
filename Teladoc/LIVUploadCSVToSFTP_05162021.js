...
function (sftp, file, record, search, runtime, email){
	function execute(scriptContext) {
		...
		var csvFile = runtime.getCurrentScript().getParameter("custscript_csv_file");
		...
		log.debug({title: 'execute', details: 'csvFile: '+ csvFile});
		...
		try{
			...
			var fileToUpload = file.load(csvFile); 
			log.debug({title: 'execute', details: 'fileToUpload = '+fileToUpload});
            ...
            var recipientEmail = ['ram.gorantla@teladochealth.com','anil.sharma@teladochealth.com'];
            email.send({
                author: authorId,
                recipients: recipientEmail,
                subject: 'Uploaded CSV file to SFTP successfully. File: '+fileToUpload.name,
                body: 'Hi,\n'+
