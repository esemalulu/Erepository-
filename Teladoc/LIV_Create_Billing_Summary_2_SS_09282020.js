    ...
    function calculateData(scriptContext, recIds){  	
    	...
    	//try{
	    	for(var j=0;j<recIds.length;j++){
		    	...
		    	total_bill_price = dbt_active_price + dbt_upfront_price + dbt_repl_price + dbt_earlyterm_price + dbt_wp_price + dbt_earlyterm_wp_price +
		    						htn_active_price + htn_upfront_price + htn_repl_price + htn_earlyterm_price + htn_wp_price + htn_earlyterm_wp_price +
		    						wm_active_price + wm_repl_price + wm_earlyterm_price + 
		    						pd_active_price + pd_earlyterm_price + pd_wp_price + pd_earlyterm_wp_price
		    						bh2_active_price + bh2_pepm_price;// + 
