/* NS-345 Last Updated: 03/10/19 - user event script for Revenue Snapshot custom record */
 ...
 if ( (type == 'create') &&  (nlapiGetContext().getExecutionContext() == 'csvimport'))
  {
       ...
       var compositeid = nlapiGetFieldValue('custrecord_liv_rev_composite_id');
       ...
       var quantity = nlapiGetFieldValue('custrecord_liv_rev_quantity');
       var unitprice = nlapiGetFieldValue('custrecord_liv_rev_unit_price');
       var amount = nlapiGetFieldValue('custrecord_liv_rev_amount');
       var docno = nlapiGetFieldValue('custrecord_liv_rev_document_number');
      ...
      */
        ...
        //Backfille customer name for Retrofit conversion journals
     
        if ( (docno == 'Retrofit 2018-04 TB Activity Upload') || (docno == 'Retrofit 2018-05 Activity Upload'))
       {
           btcompanynametmp =  getDataCnvCustomer(compositeid) ;
       }
      ...
      //Default B2C Sales
      if (accounttext == '41149 B2C Revenue')
      {
          btcompanynamedisp = 'B2C Sales';
          stcompanyname = 'B2C Sales';
      ...
      if (accounttext == '41130 Direct to Consumer')
      {
          btcompanynamedisp = 'D2C';
          stcompanyname = 'D2C';
  ...
  // Search for Contracts
          ...
          for (var i = 0;  i < arrResult.length; i++)
           {
               ...
               var dmprice            = results.getValue(columns[6]);
               ...
               var htnprice          = results.getValue(columns[58]);
    ...
    if (nvl(stcompanyname,0) !=0 )
    {
      ...
      if ( (nvl(unitprice,0) == 0)	&& (nvl(quantity,0) != 0 ) )
      {
          unitprice = (amount / quantity);
          nlapiSetFieldValue('custrecord_liv_rev_unit_price', unitprice);
      }
      else
      {
          if ( (nvl(unitprice,0) == 0) &&  (accounttext == '41101 Monthly Subscription Fee') )
          {
              unitprice = dmprice ;
              nlapiSetFieldValue('custrecord_liv_rev_unit_price', unitprice);
          }     
          if ( (nvl(unitprice,0) == 0) &&  (accounttext == '41129 Bundled - Legacy (4200)') )
          {
              unitprice = dmprice ;
              nlapiSetFieldValue('custrecord_liv_rev_unit_price', unitprice);
          }     
          if ( (nvl(unitprice,0) == 0) && (accounttext == '41106 Monthly Subscription Fee - Hypertension') )
          {
              unitprice = htnprice ;
              nlapiSetFieldValue('custrecord_liv_rev_unit_price', unitprice);
          }     
...
function getDataCnvCustomer(compositeid) 
{
     var col = new Array();
     var arrFilter = new Array(); 
   
     nlapiLogExecution('DEBUG', 'ACTIVITY', 'Search Data Conversion Customer');
     nlapiLogExecution('DEBUG','Composite ID value:'+ compositeid) ;
     
	 arrFilter[0] = new nlobjSearchFilter('custrecord_liv_dccl_composite_id', null, 'is',  compositeid );
	 
     col[0] = new nlobjSearchColumn('custrecord_liv_dccl_customer_name_text');
  
     
     var results = nlapiSearchRecord('customrecord_liv_data_conv_customer_lkp', null, arrFilter, col);
     
     for ( var i = 0; results != null && i < results.length; i++ )
     {
        var res = results[i];
        var customernametxt = (res.getValue('custrecord_liv_dccl_customer_name_text'));
  
        nlapiLogExecution('DEBUG','Customer Name Text:'+ customernametxt) ;
