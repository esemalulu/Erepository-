...
function(email, record, runtime, search, format, task) {
	...
	var bh_active_quantity = 0;
	var bh_active_quantity_bundle = 0;
	var bh_active_rate = 0.0; 
	var bh_active_price = 0.0; 
	var bh_pepm_quantity = 0;
	var bh_pepm_rate = 0.0; 
	var bh_pepm_price = 0.0; 
	...
	var comb_dbt_bh_quantity = 0;
	var comb_dbt_bh_rate = 0.0; 
	var comb_dbt_bh_price = 0.0; 
	
	var comb_htn_bh_quantity = 0;
	var comb_htn_bh_rate = 0.0; 
	var comb_htn_bh_price = 0.0; 
	
	var comb_wm_bh_quantity = 0;
	var comb_wm_bh_rate = 0.0; 
	var comb_wm_bh_price = 0.0; 
	...
	var comb_htn_dbt_bh_quantity = 0;
	var comb_htn_dbt_bh_rate = 0.0; 
	var comb_htn_dbt_bh_price = 0.0; 
    ...
    function calculateData(scriptContext, recIds){  	
    	...
    	//try{
	    	for(var j=0;j<recIds.length;j++){
		    	...
		    	if(rs_sku == 'bh_pppm'){
		    		//log.debug('bh_pppm');
		    		bh_active_quantity = bh_active_quantity + rs_quantity;
					bh_active_rate = rs_rate;
					bh_active_price = bh_active_quantity * rs_rate;
					isRowProcessed = true;
		    	}	
		    	//Behavioral Health PEPM (Per Employee Per Month)
		    	if(rs_sku == 'bh_pepm'){
		    		//log.debug('bh_pepm');
		    		bh_pepm_quantity = bh_pepm_quantity + rs_quantity;
					bh_pepm_rate = rs_rate;
					bh_pepm_price = bh_pepm_quantity * rs_rate;
					isRowProcessed = true;
		    	}	
		    	...
		    	if(rs_sku == 'dbt_bh_pppm'){
		    		//log.debug('DBT - BH');
		    		comb_dbt_bh_rate = rs_rate;
					comb_dbt_bh_quantity = comb_dbt_bh_quantity + rs_quantity;
					comb_dbt_bh_price = comb_dbt_bh_price + rs_price;
					isRowProcessed = true;
					if(rs_dbt == 1){
		    			//log.debug('rs_dbt == 1');
		    			dbt_active_quantity = dbt_active_quantity + rs_quantity;
		    			...
		    			if (dbt_active_rate == 0.0)
	    				{
		    				if (rs_bh == 1)
	    					{
		    					dbt_active_rate = comb_dbt_bh_rate - bh_active_rate;
	    					}
		    			}
						dbt_active_price = dbt_active_price + (rs_quantity*dbt_active_rate);
		    		}
		    		if(rs_bh == 1){
		    			//log.debug('rs_bh == 1');
		    			bh_active_quantity = bh_active_quantity + rs_quantity;
		    			bh_active_quantity_bundle = bh_active_quantity_bundle + rs_quantity;
		    			if (bh_active_rate == 0.0)
	    				{
		    				if (rs_dbt == 1)
	    					{
		    					bh_active_rate = comb_dbt_bh_rate - dbt_active_rate;
	    					}
		    			}
						bh_active_price =bh_active_price + (rs_quantity*bh_active_rate);
		    		}		    	
		    	}
		    	...
		    	if(rs_sku == 'htn_bh_pppm'){
		    		//log.debug('HTN - BH');
		    		comb_htn_bh_rate = rs_rate;
					comb_htn_bh_quantity = comb_htn_bh_quantity + rs_quantity;
					comb_htn_bh_price = comb_htn_bh_price + rs_price;
					isRowProcessed = true;
					if(rs_htn == 1){
		    			//log.debug('rs_htn == 1');
		    			htn_active_quantity = htn_active_quantity + rs_quantity;
		    			...
		    			if (htn_active_rate == 0.0)
	    				{
		    				if (rs_bh == 1)
	    					{
		    					htn_active_rate = comb_htn_bh_rate - bh_active_rate;
	    					}
		    			}
						htn_active_price = htn_active_price + (rs_quantity*htn_active_rate);
		    		}
		    		if(rs_bh == 1){
		    			...
		    			if (bh_active_rate == 0.0)
	    				{
		    				if (rs_htn == 1)
	    					{
		    					bh_active_rate = comb_htn_bh_rate - htn_active_rate;
	    					}
		    			}
						bh_active_price =bh_active_price + (rs_quantity*bh_active_rate);
		    		}		    	
		    	...
		    	if(rs_sku == 'wm_bh_pppm'){
		    		//log.debug('WM - BH');
		    		comb_wm_bh_rate = rs_rate;
					comb_wm_bh_quantity = comb_wm_bh_quantity + rs_quantity;
					comb_wm_bh_price = comb_wm_bh_price + rs_price;
					isRowProcessed = true;
					if(rs_wm == 1){
		    			//log.debug('rs_wm == 1');
		    			wm_active_quantity = wm_active_quantity + rs_quantity;
		    			...
		    			if (wm_active_rate == 0.0)
	    				{
		    				if (rs_bh == 1)
	    					{
		    					wm_active_rate = comb_wm_bh_rate - bh_active_rate;
	    					}
		    			}
						wm_active_price = wm_active_price + (rs_quantity*wm_active_rate);
		    		}
		    		if(rs_bh == 1){
		    			...
		    			if (bh_active_rate == 0.0)
	    				{
		    				if (rs_wm == 1)
	    					{
		    					bh_active_rate = comb_wm_bh_rate - wm_active_rate;
	    					}
		    			}
						bh_active_price =bh_active_price + (rs_quantity*bh_active_rate);
		    		}		    	
		    	...
		    	if(rs_sku.match(/htn/g) && rs_sku.match(/dbt/g) && rs_sku.match(/bh/g) && rs_sku.match(/pppm/g)){
		    		...
		    		comb_htn_dbt_bh_quantity = comb_htn_dbt_bh_quantity + rs_quantity;
					comb_htn_dbt_bh_rate = rs_rate;
					comb_htn_dbt_bh_price = comb_htn_dbt_bh_quantity + rs_price;
					isRowProcessed = true;
		    		...
		    		if(rs_htn == 1){
		    			...
		    			if (htn_active_rate == 0.0)
	    				{
		    				if (rs_dbt == 1 && rs_bh == 1)
	    					{
		    					if(dbt_active_rate > 0.0 && bh_active_rate > 0.0)
		    					{
		    						htn_active_rate = comb_htn_dbt_bh_rate - (dbt_active_rate + bh_active_rate);
		    					}		    					
	    					}
		    			}
						htn_active_price = htn_active_price + (rs_quantity*htn_active_rate);
		    		...
		    		if(rs_dbt == 1){
		    			...
		    			if (dbt_active_rate == 0.0)
	    				{
		    				if (rs_htn == 1 && rs_bh == 1)
	    					{
		    					if(htn_active_rate > 0.0 && bh_active_rate > 0.0)
		    					{
		    						dbt_active_rate = comb_htn_dbt_bh_rate - (htn_active_rate + bh_active_rate);	
		    					}	    					
	    					}
		    			}
						dbt_active_price = dbt_active_price + (rs_quantity*dbt_active_rate);
		    		...
		    		if(rs_bh == 1){
		    			...
		    			if (bh_active_rate == 0.0)
	    				{
	    					...
	    					{
		    					if(htn_active_rate > 0.0 && dbt_active_rate > 0.0){
		    						bh_active_rate = comb_htn_dbt_bh_rate - (htn_active_rate + dbt_active_rate);	
		    					}
	    					}
		    			}
						bh_active_price = bh_active_price + (rs_quantity*bh_active_rate);
		    		...
		    		/*if(rs_bh_ptm == 1){
		    			//log.debug('rs_wm_ptm == 1');
		    			bh_earlyterm_quantity = bh_earlyterm_quantity + rs_quantity;
						bh_earlyterm_price = bh_earlyterm_price + (rs_quantity*bh_earlyterm_rate);
		    		}*/
		    	}
    ...
    function createBillingSummary2Record(){
    	...
    	//BH2
    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_bh2_participants_qty', value: parseInt(bh_active_quantity)});//BH2 PARTICIPANTS QTY
    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_bh2_program_price', value: parseFloat(bh_active_rate)});//BH2 PROGRAM PRICE
    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_bh2_program_amt', value: parseFloat(bh_active_price)});//BH2 PROGRAM AMOUNT
    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_bh2_pepm_participants_qty', value: parseInt(bh_pepm_quantity)});//BH2 PEPM PARTICIPANTS QTY
    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_bh2_pepm_program_price', value: parseFloat(bh_pepm_rate)});//BH2 PROGRAM PEPM PRICE
    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_bh2_pepm_program_amt', value: parseFloat(bh_pepm_price)});//BH2 PROGRAM PEPM AMOUNT
    	...
    	//BH2 Combinations
    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_dmbh_active_members_qty', value: parseFloat(comb_dbt_bh_quantity)});//Combined Active Memebers Qty (DM/BH)
    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_dmbh_program_amt', value: parseFloat(comb_dbt_bh_price)});//Combined Program Amount (DM/BH) 
    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_htnbh_active_members_qty', value: parseFloat(comb_htn_bh_quantity)});//Combined Active Memebers Qty (HTN/BH) 
    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_htnbh_program_amt', value: parseFloat(comb_htn_bh_price)});//Combined Program Amount (HTN/BH) 
    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_wmbh_active_members_qty', value: parseFloat(comb_wm_bh_quantity)});//Combined Active Memebers Qty (WM/BH)
    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_wmbh_program_amt', value: parseFloat(comb_wm_bh_price)});//Combined Program Amount (WM/BH) 
    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_htndmbh_active_members_qt', value: parseFloat(comb_htn_dbt_bh_quantity)});//Combined Active Memebers Qty (HTN/DM/BH)
    	billlingSummary2Record.setValue({ fieldId: 'custrecord_bs2_htndmbh_program_amt', value: parseFloat(comb_htn_dbt_bh_price)});//Combined Program Amount (HTN/DM/BH)
    ...
    function resetVariables(){
    	...
    	bh_active_quantity = 0;
    	bh_active_quantity_bundle = 0;
    	bh_active_rate = 0.0; 
    	bh_active_price = 0.0; 
    	bh_pepm_quantity = 0;
    	bh_pepm_rate = 0.0; 
    	...
    	comb_dbt_bh_quantity = 0;
    	comb_dbt_bh_rate = 0.0; 
    	comb_dbt_bh_price = 0.0; 

    	comb_htn_bh_quantity = 0;
    	comb_htn_bh_rate = 0.0; 
    	comb_htn_bh_price = 0.0; 

    	comb_wm_bh_quantity = 0;
    	comb_wm_bh_rate = 0.0; 
    	comb_wm_bh_price = 0.0; 

    	comb_htn_dbt_bh_quantity = 0; 
    	comb_htn_dbt_bh_rate = 0.0; 
    	comb_htn_dbt_bh_price = 0.0; 
