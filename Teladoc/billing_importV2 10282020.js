...
/* Last Update: 10/07/19 - Add WM Referral */
/* Last Update: 01/16/2020 - Commented Amount fields and added BH2 
 * Last Update: 05/11/20202 - Added logic to trigger for Scheduled Script*/
...
function beforeSubmit(type)
{
  ...
  if ( (type == 'create') &&  ((nlapiGetContext().getExecutionContext() != 'csvimport') && (nlapiGetContext().getExecutionContext() == 'scheduled')))
  {
       // Get the value of the Discount item from parameter
       var discountitem = nlapiGetContext().getSetting('SCRIPT', 'custscript_bs2_discount_item');
...
function processRecords(type, discountitem)
{
      ...
      var dbtwpqty = nlapiGetFieldValue('custrecord_bs2_wp_meter_qty') ;
      var grouping = nlapiGetFieldValue('custrecord_bs2_grouping') ;
      var recordtype = nlapiGetFieldValue('custrecord_bs2_record_type') ;
      ...
      var htnwpqty = nlapiGetFieldValue('custrecord_bs2_ns_htn_wp_meter_qty') ;
      ...
      var dppwpqty = nlapiGetFieldValue('custrecord_bs2_ns_dpp_wp_meter_qty') ;

      //BH2
      var bh2participantsqty = nlapiGetFieldValue('custrecord_bs2_bh2_participants_qty');
      var bh2pepmparticipantsqty = nlapiGetFieldValue('custrecord_bs2_bh2_pepm_participants_qty') ;
      ...
      var dmhtnwmactivemembers = nlapiGetFieldValue('custrecord_bs2_dmhtnwm_activdte_members_qty');
      
      //Get cumulative active members qty
      var activememberssum = nlapiGetFieldValue('custrecord_bs2_active_members_qty_sum');

	  /* -------------+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-------------------- */
	  // Capture Pricing numbers - New
	  var bundleprice = nvl(nlapiGetFieldValue('custrecord_bs2_ns_pppm_price'),0);  // DBT PPPM Price
	  var upfrontprice = nvl(nlapiGetFieldValue('custrecord_bs2_ns_upfront_price'),0); // DBT UPFRONT Price
	  var replacementprice = nvl(nlapiGetFieldValue('custrecord_bs2_ns_replace_price'),0);  // DBT REPLACEMENT Price
	  var terminationprice = nvl(nlapiGetFieldValue('custrecord_bs2_ns_termination_price'),0); //DBT Early Term Price
	  var dbtwpprice = nvl(nlapiGetFieldValue('custrecord_bs2_ns_wp_price'),0); //DBT WP Price
	  
	  var htnbundleprice = nvl(nlapiGetFieldValue('custrecord_bs2_ns_htn_pppm_price'),0);  // HTN PPPM Price
	  var htnreplprice = nvl(nlapiGetFieldValue('custrecord_bs2_ns_htn_replace_price'),0);  // HTN REPLACEMENT Price
	  var htnupfrontprice = nvl(nlapiGetFieldValue('custrecord_bs2_ns_htn_upfront_price'),0); // HTN Upfront Price
	  var htnearlytermprice = nvl(nlapiGetFieldValue('custrecord_bs2_ns_htn_term_price'),0); // HTN Early Term Price
	  var htnwpprice = nvl(nlapiGetFieldValue('custrecord_bs2_ns_htn_wp_price'),0); //HTN WP Price
	  
	  var wmprice = nvl(nlapiGetFieldValue('custrecord_bs2_ns_wm_pppm_price'),0);  // WM PPPM Price
	  var wmreplprice = nvl(nlapiGetFieldValue('custrecord_bs2_ns_wm_replace_price'),0); // WM REPLACEMENT Price
	  var wmtermprice = nvl(nlapiGetFieldValue('custrecord_bs2_ns_wm_term_price'),0);  // WM Early Term Price

	  var dppprice = nvl(nlapiGetFieldValue('custrecord_bs2_ns_dpp_pppm_price'),0);  // DPP PPPM Price
	  var dppreplprice = nvl(nlapiGetFieldValue('custrecord_bs2_ns_dpp_replace_price'),0); // DPP REPLACEMENT Price
	  var dpptermprice = nvl(nlapiGetFieldValue('custrecord_bs2_ns_dpp_term_price'),0); //DPP Early Term Price
	  var dppwpprice = nvl(nlapiGetFieldValue('custrecord_bs2_ns_dpp_wp_price'),0); //DPP WP Price
	  
	  //BH2
	  var bh2price =  nvl(nlapiGetFieldValue('custrecord_bs2_bh2_program_price'),0);  // BH2 PROGRAM PRICE
	  var bh2pepmprice = nvl(nlapiGetFieldValue('custrecord_bs2_bh2_pepm_program_price'),0);  // BH2 PEPM PROGRAM PRICE
	  
	  var bundleddiscount = nvl(nlapiGetFieldValue('custrecord_bs2_cb_pppm_discount_price'),0);  // Combined Bundle discount

	  var bundleddiscountamt = nvl(nlapiGetFieldValue('custrecord_bs2_bundle_discount_amt'),0);  // NS Bundled Discount Amount
	  var rstotalbillingamt = nvl(nlapiGetFieldValue('custrecord_bs2_ns_total_billing_amt'),0);  // RS Total Amount

	  //Admin Fee
      var dbtAdminQty = nvl(nlapiGetFieldValue('custrecord_bs2_ns_dbt_admin_members'),0);
      var dbtAdminRate = nvl(nlapiGetFieldValue('custrecord_bs2_ns_referral_fee'),0);
      var dbtAdminAmt = dbtAdminQty * dbtAdminRate;
      
      var htnAdminQty = nvl(nlapiGetFieldValue('custrecord_bs2_ns_htn_admin_members'),0);
      var htnAdminRate = nvl(nlapiGetFieldValue('custrecord_bs2_ns_htn_admin_fee'),0);
      var htnAdminAmt = htnAdminQty * htnAdminRate;
      
      var wmAdminQty = nvl(nlapiGetFieldValue('custrecord_bs2_ns_wm_admin_members'),0);
      var wmAdminRate = nvl(nlapiGetFieldValue('custrecord_bs2_ns_wm_admin_fee'),0);
      var wmAdminAmt = wmAdminQty * wmAdminRate;
      
      var dppAdminQty = nvl(nlapiGetFieldValue('custrecord_bs2_ns_dpp_admin_members'),0);
      var dppAdminRate = nvl(nlapiGetFieldValue('custrecord_bs2_ns_dpp_admin_fee'),0);
      var dppAdminAmt = dppAdminQty * dppAdminRate;
      
      var htndbtAdminQty = nvl(nlapiGetFieldValue('custrecord_bs2_ns_htn_dbt_admin_members'),0);
      var htndbtAdminRate = nvl(nlapiGetFieldValue('custrecord_bs2_ns_htn_dbt_admin_fee'),0);
      var htndbtAdminAmt = htndbtAdminQty * htndbtAdminRate;
      
      var wmdbtAdminQty = nvl(nlapiGetFieldValue('custrecord_bs2_ns_wm_dbt_admin_members'),0);
      var wmdbtAdminRate = nvl(nlapiGetFieldValue('custrecord_bs2_ns_wm_dbt_admin_fee'),0);
      var wmdbtAdminAmt = wmdbtAdminQty * wmdbtAdminRate;
      
      var wmhtnAdminQty = nvl(nlapiGetFieldValue('custrecord_bs2_ns_wm_htn_admin_members'),0);
      var wmhtnAdminRate = nvl(nlapiGetFieldValue('custrecord_bs2_ns_wm_htn_admin_fee'),0);
      var wmhtnAdminAmt = wmhtnAdminQty * wmhtnAdminRate;
      
      var wmhtndbtAdminQty = nvl(nlapiGetFieldValue('custrecord_bs2_ns_wm_htn_dbt_admin_membe'),0);
      var wmhtndbtAdminRate = nvl(nlapiGetFieldValue('custrecord_bs2_ns_wm_htn_dbt_admin_fee'),0);
      var wmhtndbtAdminAmt = wmhtndbtAdminQty * wmhtndbtAdminRate;
      
      var wm2htnAdminQty = nvl(nlapiGetFieldValue('custrecord_bs2_ns_wm2_htn_admin_members'),0);
      var wm2htnAdminRate = nvl(nlapiGetFieldValue('custrecord_bs2_ns_wm2_htn_admin_fee'),0);
      var wm2htnAdminAmt = wm2htnAdminQty * wm2htnAdminRate;
      
      var wm2htndbtAdminQty = nvl(nlapiGetFieldValue('custrecord_bs2_ns_wm2_htn_dbt_admin_memb'),0);
      var wm2htndbtAdminRate = nvl(nlapiGetFieldValue('custrecord_bs2_ns_wm2_htn_dbt_admin_fee'),0);
      var wm2htndbtAdminAmt = wm2htndbtAdminQty * wm2htndbtAdminRate;
	  /* -------------+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-------------------- */
      ...
      nlapiLogExecution('DEBUG','PCCodeID value:'+ pccodeID) ;
       ...
       var arrResult = nlapiSearchRecord('customrecord_liv_contracts','customsearch_liv_contracts', arrFilter, null);
        ...
        if(arrResult)
        {
          for (var i = 0;  i < arrResult.length; i++)
           {
               ...
               //var bundleprice       = results.getValue(columns[6]);  //getting from billing record only
               var pppmdiscvol       = results.getValue(columns[7]);
               //var upfrontprice      = results.getValue(columns[8]);  //getting from billing record only
               var repldeviceresp    = results.getValue(columns[9]);
               var replmeterfoccap   = results.getValue(columns[10]);
               //var replacementprice  = results.getValue(columns[11]);  //getting from billing record only
               //var terminationprice  = results.getValue(columns[12]);  //getting from billing record only
               ...
               //var htnbundleprice      = results.getValue(columns[58]);  //getting from billing record only
               var htnitem             = results.getValue(columns[59]);
               var htnitemdesc         = results.getValue(columns[60]);
               //var htnreplprice        = results.getValue(columns[61]);  //getting from billing record only
               var htnreplitem         = results.getValue(columns[62]);
               var htnreplitemdesc     = results.getValue(columns[63]);
               //var htnupfrontprice     = results.getValue(columns[64]);  //getting from billing record only
               var htnupfrontitem      = results.getValue(columns[65]);
               var htnupfrontitemdesc  = results.getValue(columns[66]);
               //var htnearlytermprice   = results.getValue(columns[67]);  //getting from billing record only
               var htnearlytermitem    = results.getValue(columns[68]);
               var htnearlytermitemdesc = results.getValue(columns[69]);
               //var bundleddiscount     = results.getValue(columns[70]);   //getting from billing record only
               ...
               //WM
               //var wmprice      = results.getValue(columns[84]);  //getting from billing record only
               ...
               //var wmreplprice   = results.getValue(columns[91]); //getting from billing record only
               var wmreplprogitem   = results.getValue(columns[92]);
               var wmreplprogitemdesc   = results.getValue(columns[93]);
               //var wmtermprice   = results.getValue(columns[94]); //getting from billing record only
               ...
               //DPP
               //var dppprice      = results.getValue(columns[98]); //getting from billing record only
               ...
               //var dppreplprice   = results.getValue(columns[105]); //getting from billing record only
               var dppreplprogitem   = results.getValue(columns[106]);
               var dppreplprogitemdesc   = results.getValue(columns[107]);
               //var dpptermprice   = results.getValue(columns[108]); //getting from billing record only
               ...
               //WP 
               var wpitem  = results.getValue(columns[112]); 		 //WP Item
               var wpitemdesc   = results.getValue(columns[113]);	 //WP Item Desc
               var htnwpitem  = results.getValue(columns[114]); 	 //HTN WP Item
               var htnwpitemdesc   = results.getValue(columns[115]);	 //HTN WP Item Desc
               var dppwpitem  = results.getValue(columns[116]);		 //DPP WP Item
               var dppwpitemdesc   = results.getValue(columns[117]); //DPP WP Item Desc
               
               //BH2                
               var bhprogitem  = results.getValue(columns[118]);			//BH2 Program Item
               var bhprogitemdesc   = results.getValue(columns[119]); 		//BH2 Program Item Desc
               var bhpepmprogitem  = results.getValue(columns[120]);	 	//BH2 PEPM Item
               var bhpepmprogitemdesc   = results.getValue(columns[121]); 	//BH2 PEPM Item Desc
               
               //WP Early Termination
               var wptermitem  = results.getValue(columns[122]); 		 //WP Termination Item
               var wptermitemdesc   = results.getValue(columns[123]);	 //WP Termination Item Desc
               var htnwptermitem  = results.getValue(columns[124]); 	 //HTN WP Termination Item
               var htnwptermitemdesc   = results.getValue(columns[125]);	 //HTN WP Termination Item Desc
               var dppwptermitem  = results.getValue(columns[126]);		 //DPP WP Termination Item
               var dppwptermitemdesc   = results.getValue(columns[127]); //DPP WP Termination Item Desc
           	
               var fetchPartnerAdminFeeFromRS   = results.getValue(columns[128]); //Fetch Partner Admin Fee from RS
               var fetchPartnerReferralFeeFromRS   = results.getValue(columns[129]);////Fetch Partner Referral Fee from RS
               
               /*
               var bh2pppmprice = results.getValue(columns[122]);   //custrecord_liv_cm_bh2_pppm	  -  BH 2.0 PPPM
               var bh2pepmprice = results.getValue(columns[123]);   //custrecord_liv_cm_bh2_pepm	  -  BH2 PEPM
               var bh2termfee = results.getValue(columns[124]);     // custrecord_liv_cm_bh2_termination_fee	  - BH2 Termination Fee
               var bh2contractterms = results.getValue(columns[125]); // custrecord_liv_cm_bh2_contract_term - BH2 Term in Months
			   var bh2programitem = results.getValue(columns[126]);  // custrecord_liv_cm_bh2_program_item  -  BH2 Program Item
			   var bh2replacementitem = results.getValue(columns[127]);  // custrecord_liv_cm_bh2_termination_item  -  BH2 Termination Item
			   var bh2terminationitem = results.getValue(columns[128]);  // custrecord_liv_cm_bh2_replacement_item  -  BH2 Replacement Item
			   */
               // If Record Type == 'Sales Tax' then take price values from Contract record.

               if (recordtype == 'Sales Tax')
               {
                 bundleprice       = results.getValue(columns[6]);
                 upfrontprice      = results.getValue(columns[8]);
                 replacementprice  = results.getValue(columns[11]);
               	 terminationprice  = results.getValue(columns[12]);
                 htnbundleprice      = results.getValue(columns[58]);
                 htnreplprice        = results.getValue(columns[61]);
                 htnupfrontprice     = results.getValue(columns[64]);
                 htnearlytermprice   = results.getValue(columns[67]);
                 bundleddiscount     = results.getValue(columns[70]);
                 wmprice      = results.getValue(columns[84]);
                 wmreplprice   = results.getValue(columns[91]);
                 wmtermprice   = results.getValue(columns[94]);
                 dppprice      = results.getValue(columns[98]);
                 dppreplprice   = results.getValue(columns[105]);
                 dpptermprice   = results.getValue(columns[108]);
               }
               ...
               nlapiSetFieldValue('custrecord_bs2_ns_wp_item',wpitem);
               nlapiSetFieldValue('custrecord_bs2_ns_wp_desc',wpitemdesc);
               nlapiSetFieldValue('custrecord_bs2_ns_wp_term_item',wptermitem);
               nlapiSetFieldValue('custrecord_bs2_ns_wp_term_desc',wptermitemdesc);
               ...
               nlapiSetFieldValue('custrecord_bs2_ns_htn_wp_item',htnwpitem);
               nlapiSetFieldValue('custrecord_bs2_ns_htn_wp_item_desc',htnwpitemdesc);
               nlapiSetFieldValue('custrecord_bs2_ns_htn_wp_term_item',htnwptermitem);
               nlapiSetFieldValue('custrecord_bs2_ns_htn_wp_term_item_desc',htnwptermitemdesc);
               ...
               nlapiSetFieldValue('custrecord_bs2_ns_dpp_wp_item',dppwpitem);
               nlapiSetFieldValue('custrecord_bs2_ns_dpp_wp_item_desc',dppwpitemdesc);
               nlapiSetFieldValue('custrecord_bs2_ns_dpp_wp_term_item',dppwptermitem);
               nlapiSetFieldValue('custrecord_bs2_ns_dpp_wp_term_item_desc',dppwptermitemdesc);
               
			   //BH2 
               nlapiSetFieldValue('custrecord_bs2_bh2_program_item',bhprogitem);//BH2 PROGRAM ITEM
               nlapiSetFieldValue('custrecord_bs2_bh2_program_item_desc',bhprogitemdesc);//BH2 PROGRAM DESCRIPTION
               nlapiSetFieldValue('custrecord_bs2_bh2_pepm_program_item',bhpepmprogitem);//BH2 PEPM ITEM
               nlapiSetFieldValue('custrecord_bs2_bh2_pepm_prog_item_desc',bhpepmprogitemdesc);//BH2 PEPM ITEM DESC
               
               /*nlapiSetFieldValue('custrecord_bs2_bh2_program_item', bh2programitem);
			   nlapiSetFieldValue('custrecord_bs2_bh2_repl_item', bh2replacementitem);
			   nlapiSetFieldValue('custrecord_bs2_bh2_term_item', bh2terminationitem)*/
         ...
         //    combprogramdesc = combprogramdesc + ' - ' + convertdate(billing_date, 'MONTHYY') + ' ('+grouping+' )';
             ...
             var dbtwpamt = dbtwpprice * nvl(dbtwpqty,0) ;
             ...
             var htnwpamt = htnwpprice * nvl(htnwpqty,0) ;
             ...
             var wmreplamt = wmreplprice *  nvl(wmreplmeters,0) ;
              ...
              //DPP 092519
             var dppprogramamt = dppprice * (nvl(dppactivemembers,0))  ;
             var dppreferralamt = (dppadminfee * (nvl(dpponlyactivemembers,0)))*-1 ;
             var dppwpamt = dppwpprice * nvl(dppwpqty,0) ;

             //BH2
             nlapiLogExecution('DEBUG','','bh2price : '+bh2price) ;
             nlapiLogExecution('DEBUG','','bh2participantsqty : '+bh2participantsqty) ;
             nlapiLogExecution('DEBUG','','bh2pepmprice : '+bh2pepmprice) ;
             nlapiLogExecution('DEBUG','','bh2pepmparticipantsqty : '+bh2pepmparticipantsqty) ;
             var bh2programamt = bh2price * (nvl(bh2participantsqty,0))  ;
             var bh2pepmprogramamt = bh2pepmprice * (nvl(bh2pepmparticipantsqty,0))  ;
             ...
             //var bh2
              ...
              nlapiLogExecution('DEBUG','dbtwpamt : '+dbtwpamt) ;
              nlapiLogExecution('DEBUG','htnwpamt : '+htnwpamt) ;
              nlapiLogExecution('DEBUG','dppwpamt : '+dppwpamt) ;
              nlapiLogExecution('DEBUG','','bh2programamt : '+bh2programamt) ;
              nlapiLogExecution('DEBUG','','bh2pepmprogramamt : '+bh2pepmprogramamt) ;
             ...
             // NS-106
              ...
              if (categorytext == 'Upfront')
              {
                 ...
                 /*var totalbillingamt = (nvl(programamt,0))+ (nvl(replamt,0)) + (nvl(referralamt,0)) + (nvl(upfrontamt,0))+
                 				(nvl(htnprogramamt,0))+(nvl(htnreplamt,0))+(nvl(htnreferralamt,0))+(nvl(htnupfrontamt,0))+
                 				(nvl(combprogdiscamt,0))+(nvl(wmreferralamt,0))+(nvl(dppreferralamt,0))+(nvl(dbtwpamt,0))+
                 				(nvl(htnwpamt,0))+(nvl(dppwpamt,0))+(nvl(bh2programamt,0))+(nvl(bh2pepmprogramamt,0));*/
                 
                 /*var totalbillingamt = parseFloat(nvl(programamt,0))+ parseFloat(nvl(htnprogramamt,0))+ parseFloat(nvl(wmprogramamt,0))+ parseFloat(nvl(dppprogramamt,0))+ parseFloat(nvl(bh2programamt,0))+parseFloat(nvl(bh2pepmprogramamt,0)) +
                 parseFloat(nvl(dbtwpamt,0))+parseFloat(nvl(htnwpamt,0))+parseFloat(nvl(dppwpamt,0))+
                 parseFloat(nvl(replamt,0)) + parseFloat(nvl(htnreplamt,0))+parseFloat(nvl(wmreplamt,0))+
                 parseFloat(nvl(upfrontamt,0))+parseFloat(nvl(htnupfrontamt,0))+					
                 parseFloat(nvl(referralamt,0))+parseFloat(nvl(htnreferralamt,0))+parseFloat(nvl(wmreferralamt,0))+parseFloat(nvl(dppreferralamt,0))+
                 parseFloat(nvl(bundledreferralamt,0))+parseFloat(nvl(combprogdiscamt,0));*/
                 
                 combprogdiscamt = bundleddiscountamt; 
                 
                 nlapiLogExecution('DEBUG','Upfront: total billing amt : '+totalbillingamt);
                 
                 var totalbillingamt = 0.0;
          	   	 if(fetchPartnerAdminFeeFromRS == 'T'){
          	   		 
          	   	 nlapiLogExecution('DEBUG','Upfront: rstotalbillingamt : '+rstotalbillingamt);
          	     nlapiLogExecution('DEBUG','Bundled: dbtAdminAmt : '+dbtAdminAmt);
                 nlapiLogExecution('DEBUG','Upfront: htnAdminAmt : '+htnAdminAmt);
                 nlapiLogExecution('DEBUG','Upfront: wmAdminAmt : '+wmAdminAmt);
                 nlapiLogExecution('DEBUG','Upfront: dppAdminAmt : '+dppAdminAmt);
                 nlapiLogExecution('DEBUG','Upfront: htndbtAdminAmt : '+htndbtAdminAmt);
                 nlapiLogExecution('DEBUG','Upfront: wmdbtAdminAmt : '+wmdbtAdminAmt);
                 nlapiLogExecution('DEBUG','Upfront: wmhtnAdminAmt : '+wmhtnAdminAmt);
                 nlapiLogExecution('DEBUG','Upfront: wmhtndbtAdminAmt : '+wmhtndbtAdminAmt);
                 nlapiLogExecution('DEBUG','Upfront: wm2htnAdminAmt : '+wm2htnAdminAmt);
                 nlapiLogExecution('DEBUG','Upfront: wm2htndbtAdminAmt : '+wm2htndbtAdminAmt);
                 nlapiLogExecution('DEBUG','Upfront: combprogdiscamt : '+combprogdiscamt);
          	   		 
          	   		 totalbillingamt = parseFloat(rstotalbillingamt) +
          	   		 	(parseFloat(nvl(dbtAdminAmt,0))) + (parseFloat(nvl(htnAdminAmt,0))) + 
          	   		 	(parseFloat(nvl(wmAdminAmt,0))) + (parseFloat(nvl(dppAdminAmt,0))) + 
          	   		 	(parseFloat(nvl(htndbtAdminAmt,0))) + (parseFloat(nvl(wmdbtAdminAmt,0))) + 
          	   		 	(parseFloat(nvl(wmhtnAdminAmt,0))) + (parseFloat(nvl(wmhtndbtAdminAmt,0))) + 
          	   		 	(parseFloat(nvl(wm2htnAdminAmt,0))) + (parseFloat(nvl(wm2htndbtAdminAmt,0))) + 
          	   		 	(parseFloat(nvl(combprogdiscamt,0)));
          	   	 }
          	   	 else{
          	   		 totalbillingamt = parseFloat(rstotalbillingamt) +
          	   		 (parseFloat(nvl(referralamt,0))+parseFloat(nvl(htnreferralamt,0))+parseFloat(nvl(wmreferralamt,0))+parseFloat(nvl(dppreferralamt,0))+
              ...
              if (categorytext == 'Bundled')
              {
               ...
               if (nvl(flattermprice,0) == 0)
                ...
                if (nvl(dppearlytermtotal,0) != 0)
                {
                    ...
                    var dppearlytermdesc = 'DPP Early Termination : ' ;
                    nlapiLogExecution('DEBUG','Inside (nvl(dppearlytermtotal,0) != 0) ');
                    nlapiLogExecution('DEBUG','dppearlytermprice : '+dpptermprice);

                    var dppearlyterm1amt = nvl(dppearlytermtotal,0) * nvl(dpptermprice,0) ;

                    if (nvl(dppearlytermtotal,0) != 0) { dppearlytermdesc = dppearlytermdesc + dppearlytermtotal + ' member(s).' };


                    dppearlytermtotalamt = dppearlyterm1amt;
                ...
                //var totalbillingamt = (nvl(programamt,0))+ (nvl(replamt,0)) + (nvl(referralamt,0)) + (nvl(earlytermtotal,0))+(nvl(htnprogramamt,0))+(nvl(htnreplamt,0))+(nvl(htnupfrontamt,0))+(nvl(htnreferralamt,0))+(nvl(htnearlytermtotalamt,0))+(nvl(combprogdiscamt,0))+(nvl(bundledreferralamt,0))+(nvl(wmprogramamt,0))+(nvl(wmreplamt,0))+(nvl(wmearlytermtotalamt,0))+(nvl(dppprogramamt,0))+(nvl(dppearlytermtotalamt,0))+(nvl(dmwmdiscpctamt,0))+(nvl(wmreferralamt,0))+(nvl(dppreferralamt,0));
                
                nlapiLogExecution('DEBUG','','rstotalbillingamt : '+rstotalbillingamt);
                nlapiLogExecution('DEBUG','','programamt : '+programamt);
                nlapiLogExecution('DEBUG','','htnprogramamt : '+htnprogramamt);
                nlapiLogExecution('DEBUG','','wmprogramamt : '+wmprogramamt);
                nlapiLogExecution('DEBUG','','dppprogramamt : '+dppprogramamt);
                nlapiLogExecution('DEBUG','','bh2programamt : '+bh2programamt);
                nlapiLogExecution('DEBUG','','bh2pepmprogramamt : '+bh2pepmprogramamt);
                nlapiLogExecution('DEBUG','','dbtwpamt : '+dbtwpamt);
                nlapiLogExecution('DEBUG','','htnwpamt : '+htnwpamt);
                nlapiLogExecution('DEBUG','','dppwpamt : '+dppwpamt);
                nlapiLogExecution('DEBUG','','replamt : '+replamt);
                nlapiLogExecution('DEBUG','','wmreplamt : '+wmreplamt);
                nlapiLogExecution('DEBUG','','earlytermtotal : '+htnearlytermtotal);
                nlapiLogExecution('DEBUG','','wmearlytermtotal : '+wmearlytermtotal);
                nlapiLogExecution('DEBUG','','dppearlytermtotal : '+dppearlytermtotal);
                nlapiLogExecution('DEBUG','','referralamt : '+referralamt);
                nlapiLogExecution('DEBUG','','htnreferralamt : '+htnreferralamt);
                nlapiLogExecution('DEBUG','','wmreferralamt : '+wmreferralamt);
                nlapiLogExecution('DEBUG','','dppreferralamt : '+dppreferralamt);
                nlapiLogExecution('DEBUG','','bundledreferralamt : '+bundledreferralamt);
                nlapiLogExecution('DEBUG','','combprogdiscamt : '+combprogdiscamt);
                
                /*var totalbillingamt = (Math.abs(parseFloat(nvl(rstotalbillingamt,0)))) + (nvl(wmreferralamt,0))+
        		(nvl(dppreferralamt,0))+(nvl(bundledreferralamt,0))+(nvl(referralamt,0));*/
                
                /*var totalbillingamt = (Math.abs(parseFloat(nvl(rstotalbillingamt,0)))) + (nvl(wmreferralamt,0))+
                		(nvl(dppreferralamt,0))+(nvl(bundledreferralamt,0))+(nvl(referralamt,0))+(nvl(dbtwpamt,0))+
                		(nvl(htnwpamt,0))+(nvl(dppwpamt,0))+(nvl(bh2programamt,0))+(nvl(bh2pepmprogramamt,0));*/
                
                /*var totalbillingamt = (nvl(wmreferralamt,0))+ (nvl(dppreferralamt,0))+(nvl(bundledreferralamt,0))+
                						(nvl(referralamt,0))+(nvl(dbtwpamt,0))+ (nvl(htnwpamt,0))+(nvl(dppwpamt,0))+
                						(nvl(bh2programamt,0))+(nvl(bh2pepmprogramamt,0));*/
                
                /*var totalbillingamt = parseFloat(nvl(programamt,0))+ parseFloat(nvl(htnprogramamt,0))+ parseFloat(nvl(wmprogramamt,0))+ parseFloat(nvl(dppprogramamt,0))+ parseFloat(nvl(bh2programamt,0))+parseFloat(nvl(bh2pepmprogramamt,0)) +
                parseFloat(nvl(dbtwpamt,0))+parseFloat(nvl(htnwpamt,0))+parseFloat(nvl(dppwpamt,0))+
                parseFloat(nvl(replamt,0)) + parseFloat(nvl(htnreplamt,0))+parseFloat(nvl(wmreplamt,0))+
                parseFloat(nvl(earlytermtotal,0))+parseFloat(nvl(htnearlytermtotal,0))+ parseFloat(nvl(wmearlytermtotal,0))+ parseFloat(nvl(dppearlytermtotal,0))+ 
                parseFloat(nvl(referralamt,0))+parseFloat(nvl(htnreferralamt,0))+parseFloat(nvl(wmreferralamt,0))+parseFloat(nvl(dppreferralamt,0))+
                parseFloat(nvl(bundledreferralamt,0))+parseFloat(nvl(combprogdiscamt,0));*/
                combprogdiscamt = bundleddiscountamt; 
                nlapiLogExecution('DEBUG','Bundled: total billing amt : '+totalbillingamt);
                
                var totalbillingamt = 0.0;
         	   	 if(fetchPartnerAdminFeeFromRS == 'T'){
         	   		 
                     nlapiLogExecution('DEBUG','Bundled: rstotalbillingamt : '+rstotalbillingamt);
                     nlapiLogExecution('DEBUG','Bundled: dbtAdminAmt : '+dbtAdminAmt);
                     nlapiLogExecution('DEBUG','Bundled: htnAdminAmt : '+htnAdminAmt);
                     nlapiLogExecution('DEBUG','Bundled: wmAdminAmt : '+wmAdminAmt);
                     nlapiLogExecution('DEBUG','Bundled: dppAdminAmt : '+dppAdminAmt);
                     nlapiLogExecution('DEBUG','Bundled: htndbtAdminAmt : '+htndbtAdminAmt);
                     nlapiLogExecution('DEBUG','Bundled: wmdbtAdminAmt : '+wmdbtAdminAmt);
                     nlapiLogExecution('DEBUG','Bundled: wmhtnAdminAmt : '+wmhtnAdminAmt);
                     nlapiLogExecution('DEBUG','Bundled: wmhtndbtAdminAmt : '+wmhtndbtAdminAmt);
                     nlapiLogExecution('DEBUG','Bundled: wm2htnAdminAmt : '+wm2htnAdminAmt);
                     nlapiLogExecution('DEBUG','Bundled: wm2htndbtAdminAmt : '+wm2htndbtAdminAmt);
                     nlapiLogExecution('DEBUG','Bundled: combprogdiscamt : '+combprogdiscamt);
             ...
             else
             {
              ...
              //Admin Fee and Amount
              nlapiLogExecution('DEBUG', 'VALUE', 'fetchPartnerAdminFeeFromRS: '+fetchPartnerAdminFeeFromRS);
              if(fetchPartnerAdminFeeFromRS == 'F'){
            	  nlapiLogExecution('DEBUG', 'VALUE', 'Admin fee used from the Contract.');
            	  nlapiSetFieldValue('custrecord_bs2_ns_referral_fee',Math.abs(parseFloat(adminfee)));
                  nlapiSetFieldValue('custrecord_bs2_ns_referral_fee_amt', referralamt );  //NS Admin Fee Amount
                  nlapiSetFieldValue('custrecord_bs2_ns_htn_admin_fee',Math.abs(parseFloat(htnadminfee)));
                  nlapiSetFieldValue('custrecord_bs2_ns_htn_admin_fee_amt', htnreferralamt );  // NS HTN Admin Fee Amt
                  nlapiSetFieldValue('custrecord_bs2_ns_wm_admin_fee', wmadminfee );
                  nlapiSetFieldValue('custrecord_bs2_ns_wm_admin_fee_amt', wmreferralamt );  //NS WM Admin Fee Amt
                  nlapiSetFieldValue('custrecord_bs2_ns_dpp_admin_fee', dppadminfee );
                  nlapiSetFieldValue('custrecord_bs2_ns_dpp_admin_fee_amt', dppreferralamt );  //NS DPP Admin Fee Amt
                  
                  nlapiSetFieldValue('custrecord_bs2_ns_bundled_admin_fee',Math.abs(parseFloat(bundledadminfee))); //NS-246
                  nlapiSetFieldValue('custrecord_bs2_ns_bundled_admin_fee_amt', bundledreferralamt ); // Bundled Admin Fee Amt //NS-246
              }
              else
              {
            	  nlapiLogExecution('DEBUG', 'VALUE', 'Admin fee used from the RS Table.');
              }
              
              
              ///nlapiSetFieldValue('custrecord_bs2_ns_program_amt', programamt );  //NS Livongo Program Amt
              ///nlapiSetFieldValue('custrecord_bs2_ns_upfront_amt', upfrontamt ); //NS Upfront Amount
              ///nlapiSetFieldValue('custrecord_bs2_ns_replacement_amt', replamt ); // NS Replacement Amt
              ///nlapiSetFieldValue('custrecord_bs2_ns_termination_amt', earlytermtotal );  //NS Termination Amt

              ///nlapiSetFieldValue('custrecord_bs2_ns_termination_desc',earlytermdesc );
              nlapiSetFieldValue('custrecord_bs2_ns_total_billing_amt',totalbillingamt );  // NS Total Billing Amount
              nlapiSetFieldValue('custrecord_bs2_ns_calc_billing_amt',rstotalbillingamt );  // This is new col to capture total amount calculated in NS

              nlapiSetFieldValue('custrecord_bs2_ns_ffs_lancets_amt',ffs_lancetamt );  // NS FFS Lancets Amount
              nlapiSetFieldValue('custrecord_bs2_ns_ffs_strips_amt', ffs_stripamt );  //NS FFS Test Strips Amount
              nlapiSetFieldValue('custrecord_bs2_ns_ffs_lancing_amt',ffs_lancingamt );  // NS FFS Lancing Device Amt
              nlapiSetFieldValue('custrecord_bs2_ns_ffs_control_sol_amt',ffs_controlsolamt );  // NS FFS Control Solution Amount

              
              ///nlapiSetFieldValue('custrecord_bs2_ns_htn_program_amt', htnprogramamt );  // NS HTN Program Amount
              ///nlapiSetFieldValue('custrecord_bs2_htn_upfront_amt', htnupfrontamt );  // NS HTN Upfront Amt
              ///nlapiSetFieldValue('custrecord_bs2_ns_htn_replacement_amt', htnreplamt );  // NS HTN Replacement Amt
              ///nlapiSetFieldValue('custrecord_bs2_ns_htn_term_amt', htnearlytermtotalamt );  // NS HTN Termination Amt

              //nlapiSetFieldValue('custrecord_bs2_ns_cb_term_amt', combearlytermtotalamt ); -- no field in Netsuite
              nlapiSetFieldValue('custrecord_bs2_htn_program_desc', htnitemdesc );

              nlapiSetFieldValue('custrecord_bs2_ns_htn_term_item_desc',htnearlytermdesc );

              ///nlapiSetFieldValue('custrecord_bs2_cb_pppm_discount_amt', combprogdiscamt ); // Bundled Discount Amount (DM/HTN)
              ...
              //NS-287
              //nlapiSetFieldValue('custrecord_bs2_dmhtn_program_amt', combprogramamt );  //Combined Program Amount (DM/HTN)
              nlapiSetFieldValue('custrecord_bs2_cb_bundled_discount_pct', bundleddiscountpct);

              //WM
              
              
              ///nlapiSetFieldValue('custrecord_bs2_ns_wm_program_amt', wmprogramamt );  //NS WM Program Amount
              ///nlapiSetFieldValue('custrecord_bs2_ns_wm_replacement_amt', wmreplamt );  //NS WM Replacement Amt
              ///nlapiSetFieldValue('custrecord_bs2_ns_wm_term_amt', wmearlytermtotalamt );  //NS WM Termination Amt
              nlapiSetFieldValue('custrecord_bs2_wm_program_desc', wmprogitemdesc );
              nlapiSetFieldValue('custrecord_bs2_ns_wm_term_item_desc',wmearlytermdesc );

              //DPP 092519
              
              ///nlapiSetFieldValue('custrecord_bs2_ns_dpp_program_amt', dppprogramamt );  //NS DPP Program Amount
              ///nlapiSetFieldValue('custrecord_bs2_ns_dpp_term_amt', dppearlytermtotalamt );  //NS DPP Termination Amt
              nlapiSetFieldValue('custrecord_bs2_dpp_program_desc', dppprogitemdesc );
              nlapiSetFieldValue('custrecord_bs2_ns_dpp_term_item_desc',dppearlytermdesc );

              // Multi product conditions discount
              ///nlapiSetFieldValue('custrecord_bs2_dmwm_program_amt', dmwmamt );  //Combined Program Amount (DM/WM)
			  ///nlapiSetFieldValue('custrecord_bs2_dpphtn_program_amt', dpphtnamt );  //Combined Program Amount (DPP/HTN)
			  ///nlapiSetFieldValue('custrecord_bs2_dmhtnwm_program_amt', dmhtnwmamt);  //Combined Program Amount (DM/HTN/WM)
			  //New
			  ///nlapiSetFieldValue('custrecord_bs2_dmwm_discount_amt', dmwmdiscpctamt ); //Bundled Discount Amount (DM/WM)
			  ///nlapiSetFieldValue('custrecord_bs2_dpphtn_discount_amt', dpphtndiscpctamt );  //Bundled Discount Amount (DPP/HTN)
			  ///nlapiSetFieldValue('custrecord_bs2_dmhtnwm_discount_amt', dmhtnwmdiscpctamt );  //Bundled Discount Amount (DPP/HTN/WM)
       ...
       else
       {
              ...
              //Check Context =Scheduled and Then
              if(nlapiGetContext().getExecutionContext() == 'scheduled'){
            	  var err = nlapiCreateError('CONTRACT_NOT_FOUND','Unable to find NetSuite Customer Contract.');
                  throw err;
              }
