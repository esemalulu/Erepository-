...
//last update date: 06/06/18 Add parameters
//last update date: NS-217
...
function CreateCCJournal()
{
 try
 {
   ...
   var today = new Date(); 
    ...
    var ccaccount = nlapiGetContext().getSetting('SCRIPT', 'custscript_liv_cc_account');
    var fromdate = nlapiGetContext().getSetting('SCRIPT', 'custscript_liv_cc_post_date_from');
    var todate = nlapiGetContext().getSetting('SCRIPT', 'custscript_liv_cc_post_date_to');
    var psubsidiary = nlapiGetContext().getSetting('SCRIPT', 'custscript_liv_cc_subsidiary');
    var pjebatchnumber = nlapiGetContext().getSetting('SCRIPT', 'custscript_liv_cc_je_batch_number'); //NS-217
      
    nlapiLogExecution('DEBUG','param-from date: '+fromdate) ;
    nlapiLogExecution('DEBUG','param-to date: '+todate) ;
    nlapiLogExecution('DEBUG','param-ccaccount: '+ccaccount) ;
    ...
    arrFilter[2] = new nlobjSearchFilter('custrecord_liv_cc_post_date', null, 'within', fromdate, todate  ); 
    arrFilter[3] = new nlobjSearchFilter('custrecord_liv_cc_control_account', null, 'is',  ccaccount ); 
    arrFilter[4] = new nlobjSearchFilter('custrecord_liv_cc_ns_subsidiary', null, 'is',  psubsidiary ); 
    ...
    for (var i = 0;  i < arrResult.length; i++)
      {
             ...
             { 
	            ...
	            // JE Header
	         	...
	         	record.setFieldValue('custbody_liv_je_batch_number', pjebatchnumber); //NS-217
	         	record.setFieldText('custbody_liv_journal_entry_type','Routine'); //NS-217
            ...
            // If the script's remaining usage points are bellow 1,000 ...       
             ...
             var updateid = updateCCtrxs(fromdate, todate, ccaccount) ; //update je internal id in cc trxs
...
function updateCCtrxs(fromdate, todate, ccaccount)
/********************************************************************************************/
   ...
   var today = new Date(); 
  	...
  	if(arrResult)
    {
        for (var i = 0;  i < arrResult.length; i++)
      {
        ...
        // jeFilter[1] = new nlobjSearchFilter('mainline', null, 'is',  'T'); //NS-217
         ...
         if (jeResult)
         {
            ...
            {
              var jerecord = jeResult[0]; //NS-217
