...
function (sftp, file, record, search, runtime, email, task){
	function execute(scriptContext) {
		...
		var sftpBillingFilenameId = runtime.getCurrentScript().getParameter("custscript_sftp_billing_filename");
		...
		log.debug({title: 'execute', details: 'sftpBillingFilenameId: '+ sftpBillingFilenameId});
		
		var sftpBillingFilename = '';
    	if(sftpBillingFilenameId == 1)
    		sftpBillingFilename = 'Billing_Snapshot';
    	else if(sftpBillingFilenameId == 2)
    		sftpBillingFilename = 'CC_Val';
        else if(sftpBillingFilenameId == 3)
    		sftpBillingFilename = 'ns_bill';
		...
		try{
			...
			var filename = sftpBillingFilename + "_" + formattedDate+'.csv'; // eg. Billing_Snapshot_2020_08_01 or CC_Val_2020_09_27.csv
