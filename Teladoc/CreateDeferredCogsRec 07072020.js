...
//last update date: fix clientcode issue
//last update date: NS-229 aggregate journals at client code level
//last update date: add contract number
//07/07/2020: searchAllRecord function added
...
function CreateDeferredCogsRec()
{
 try
 {
       ...
       var p_savedsearchid = nlapiGetContext().getSetting('SCRIPT', 'custscript_def_cogs_saved_search_id');
...
/********************************************************************************************/
{
	nlapiLogExecution('DEBUG', 'ACTIVITY', 'createRec Started...');
	    ...
	    var amorttemplate = p_amorttemplate;
			   ...
			   var amorttype = customrecord.getValue(columns[18]);
			   var contractnumber = customrecord.getValue(columns[19]);
			   
			   var accountnum = accounttext.substr(0,5);
			   var jename = 'DFC Amort. '+amorttype+'-'+period+'-'+clientcode ; //NS-229
			   jename = jename.substring(0, 44) ;
		...
		try
		{
	        ...
	        //record.setFieldText('custrecord_liv_dfc_client_code',clientcode);  //102418
	        record.setFieldValue('custrecord_liv_dfc_client_code',clientcode);  //102418
	        ...
	        record.setFieldValue('custrecord_liv_dfc_amount', parseFloat(amount)) ;
	        ...
	        record.setFieldValue('custrecord_liv_dfc_contract_number', contractnumber) ;
