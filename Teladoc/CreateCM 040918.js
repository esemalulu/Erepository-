//last updated date: 09/18/17 Create CM for Admin Fee
//last updated date: 10/24/17 NS-62 add subsidiary / currency fields
//last update date: 091817 - add Create CM for Fees
//last update date: 102417 - NS-62 - add subsidiary / currency
//last update date: 040918 - NS-140 - add client code custom corecord.setCurrentLineItemText('item', 'custcol_liv_invoice_client_code', clientcode);l field to invoice line
...
function CreateCM()
{
 try
 {
  ...
  arrFilter[0] = new nlobjSearchFilter('custrecord_liv_bs_ns_cm_internal_id', null, 'isempty',  'null' ); 
  arrFilter[1] = new nlobjSearchFilter('custrecord_liv_bs_ns_billable', null, 'is',  'T' ); 
  arrFilter[2] = new nlobjSearchFilter('custrecord_liv_bs_ns_create_cm_fee', null, 'is',  'T' ); 
  ...
  arrColumns.push(new nlobjSearchColumn('custrecord_liv_bs_ns_cm_customer'));
...
/********************************************************************************************/
{
	nlapiLogExecution('DEBUG', 'ACTIVITY', 'createCMRec Started...');
        ...
        var cmcustomerid = customrecord.getValue('custrecord_liv_bs_ns_cm_customer');
        ...
        // Referral Fee
        var billing_date = customrecord.getValue('custrecord_liv_bs_date');
        ...
        var clientcode = customrecord.getValue('custrecord_liv_bs_grouping');
		...
		try
		{
	     ...
	     {  
            ...
            if (strRecID)
            {
                ...
                currentrecord.setFieldValue('custrecord_liv_bs_ns_cm_internal_id', strRecID);
                currentrecord.setFieldValue('custrecord_liv_bs_ns_cm_creation_date', today);
                currentrecord.setFieldValue('custrecord_liv_bs_ns_cm_error_msg', ' ');
         ...
         {
                var currentrecord = nlapiLoadRecord('customrecord_liv_billing_summary',recordid, {recordmode: 'dynamic'});
                currentrecord.setFieldValue('custrecord_liv_bs_ns_error_msg', 'Total Credit Amount is zero. CM not created.');
                var strCustomID = nlapiSubmitRecord(currentrecord, true, true);
		...
		}catch(error)
		{
						if (error.getDetails != undefined) 
					   {
							nlapiLogExecution('DEBUG', 'CM creation error for billing summary internal id: '+recordid, error.getCode() + ': ' + error.getDetails());
							 var currentrecord = nlapiLoadRecord('customrecord_liv_billing_summary',recordid, {recordmode: 'dynamic'});
                             currentrecord.setFieldValue('custrecord_liv_bs_ns_cm_error_msg', error.getCode() + ': ' + error.getDetails());
					   ...
					   {    
							nlapiLogExecution('DEBUG', 'Unexpected CM creation error for billing summary internal id : '+recordid, error.toString());
							 var currentrecord = nlapiLoadRecord('customrecord_liv_billing_summary',recordid, {recordmode: 'dynamic'});
                             currentrecord.setFieldValue('custrecord_liv_bs_ns_cm_error_msg', error.toString());
                             nlapiSubmitRecord(currentrecord, true, true);
					   }
		}
