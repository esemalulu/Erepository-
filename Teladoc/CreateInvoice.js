function CreateInvoice()
{
 try
 {
  ...
  arrColumns.push(new nlobjSearchColumn('custrecord_liv_bs_client_code'));
  arrColumns.push(new nlobjSearchColumn('custrecord_liv_bs_ns_customer'));
  ...
  arrColumns.push(new nlobjSearchColumn('custrecord_liv_bs_ns_invoice_number'));
  ...
  }catch(error) 
  {
	   ...
	   {
		   nlapiLogExecution('DEBUG', 'Process Error for Billing Internal ID : '+recordid, error.getCode() + ': ' + error.getDetails());
		   throw nlapiCreateError('Create Invoice','',recordid,error.getCode(), error.toString());
	   }
	   else 
	   {    	
			nlapiLogExecution('DEBUG', 'Unexpected Error for Billing Internal ID : '+recordid, error.toString());
			throw nlapiCreateError('Create Invoice','',name,'Undefined Error Code', error.toString());
	   }
  }
} // end function CreateInvoice

function createInvoiceRec(customrecord)
/********************************************************************************************/
...
/********************************************************************************************/
{
	nlapiLogExecution('DEBUG', 'ACTIVITY', 'createInvoiceRec Started...');
		...
		}catch(error)
		{
						if (error.getDetails != undefined) 
					   {
							nlapiLogExecution('DEBUG', 'Invoice creation error for : '+recordid, error.getCode() + ': ' + error.getDetails());
							 var currentrecord = nlapiLoadRecord('customrecord_liv_billing_summary',recordid, {recordmode: 'dynamic'});
					   ...
					   {    
							nlapiLogExecution('DEBUG', 'Unexpected invoice creation error for : '+recordid, error.toString());
							 var currentrecord = nlapiLoadRecord('customrecord_liv_billing_summary',recordid, {recordmode: 'dynamic'});
	...
	nlapiLogExecution('DEBUG', 'ACTIVITY', 'createInvoiceRe Ended Sucessfully');
}
