...
function CreateMyAdvICJournal()
{
 try
 {
       ...
       var fromdate = nlapiGetContext().getSetting('SCRIPT', 'custscript_liv_my_from_date');
       var todate = nlapiGetContext().getSetting('SCRIPT', 'custscript_liv_my_to_date');
       var icaracct = nlapiGetContext().getSetting('SCRIPT', 'custscript_liv_my_ic_ar_acct');
       var icapacct = nlapiGetContext().getSetting('SCRIPT', 'custscript_liv_my_ic_ap_acct');
       var jename = nlapiGetContext().getSetting('SCRIPT', 'custscript_liv_my_ic_jename');
       var acctg_period = nlapiGetContext().getSetting('SCRIPT', 'custscript_liv_my_ic_period');
       var tranid = nlapiGetContext().getSetting('SCRIPT', 'custscript_liv_my_ic_tranid');
       ...
       nlapiLogExecution('DEBUG','param-tranid: '+tranid) ;
   ...
   arrFilter[0] = new nlobjSearchFilter('tranid', null, 'is', tranid  ); 
     ...
     //  arrFilter[0] = new nlobjSearchFilter('trandate', null, 'within', fromdate, todate  ); 
       ...
       //arrFilter[2] = new nlobjSearchFilter('tranid', null, 'is', tranid  ); 
  
	
	   var searchresults = nlapiSearchRecord('transaction','customsearch_liv_my_je_reclass', arrFilter , null);
       ...
       { 
			  ...
			  // Create intercompany journal
			  	try
		        {
			      ...
			      //  var icrecord = nlapiCreateRecord('intercompanyjournalentry', {recordmode:'dynamic'});
					...
					var jememohdr = jename ;
				    var jememo =  name+' '+docno ;
				    var externalid = 'IC'+'.'+jename;
					
				
				    nlapiLogExecution('DEBUG','jename: '+jename) ;
				    nlapiLogExecution('DEBUG','externalid: '+externalid) ;
				    nlapiLogExecution('DEBUG','tranid: '+jename) ;
				    nlapiLogExecution('DEBUG','trandate: '+trandate) ;
				    nlapiLogExecution('DEBUG','memo: '+jememohdr) ;
				    nlapiLogExecution('DEBUG','subsidiary: '+fromsubsidiary) ;
				     nlapiLogExecution('DEBUG','tosubsidiary: '+tosubsidiary) ;
				...
				//    icrecord.setFieldValue('postingperiod', period);    
					...
					icrecord.setCurrentLineItemValue('line', 'entity', 84566);  // 01-04 myStrength
				   ...
				   icrecord.setCurrentLineItemValue('line', 'entity', 84567); // 04-01 Livongo
			       icrecord.setCurrentLineItemValue('line', 'department', department); 
                   icrecord.setCurrentLineItemValue('line', 'memo',  jememo);
                   icrecord.setCurrentLineItemValue('line', 'credit', Math.abs(parseFloat(amount)));
                   icrecord.commitLineItem('line'); 
			        ...
			        else // z!= 0
			        {
			           ...
			           if (amount >= 0)
					   {
				         ...
				         // add journal line - from subsidiary (Debit)
					      ...
					      jerecord.setCurrentLineItemValue('line', 'entity', 84566);  // 01-04 myStrength
				          ...
				          jerecord.setCurrentLineItemValue('line', 'entity', 84567); // 04-01 Livongo
			              jerecord.setCurrentLineItemValue('line', 'department', department); 
                          jerecord.setCurrentLineItemValue('line', 'memo',  jememo);
                          jerecord.setCurrentLineItemValue('line', 'credit', Math.abs(parseFloat(amount)));
                          jerecord.commitLineItem('line'); 
		...
		/*	  
                   ...
                   // Search vendor bill to update
			          ...
			          nlapiLogExecution('DEBUG','Start Search/Update record') ;
                ...
                */   
                ...
                }catch(error)
		        {
					   ...
					   {
							nlapiLogExecution('DEBUG', 'Intercompany journal creation error : '+docno, error.getCode() + ': ' + error.getDetails());
							
						
					   }
					   else 
					   {    
							nlapiLogExecution('DEBUG', 'Unexpected Intercompany journal creation : '+docno, error.toString());
