 ...
 *
 * Script Id: customscript_coupa_vendor
 *
 * Deployment Id: customdeploy_coupa_vendor
 *
 * Version    Date            Author            Remarks
 * 1.0                                     Initial Development
 * 1.1      Jun 25, 2020  Yogesh Jagdale   Bug Fix; NIB-233
 *
 ********************************************************************************/
...
if (oidcHeader) {
	...
	var param_APIKey = nlapiGetContext().getSetting('SCRIPT','custscript_vendor_apikey');	//NIB# 331 Use API_KEY header if OIDC parameters are not available
...
function userEventAfterSubmit(type) {
	var companyFaxNum = "", companyMobileNum = "";
	...
	if (context
		...
		if (type == 'edit' || type == 'xedit') {
			 ...
			 * nlapiGetNewRecord() to return all the newly updated fields, be aware that only the fields which have been
			 * updated through an xedit event (inline edited or mass updated) will be returned. In many cases, this is
			 * only one or two fields.
			 */
			if (newrecord.getFieldValue(custfieldincludeonly) != 'T' && oldrecord.getFieldValue(custfieldincludeonly) == 'F') {
				nlapiLogExecution('AUDIT', 'Skipping Vendor update - as DO NOT INCLUDE in COUPA set',
					'Vendor = ' + newrecord.getFieldValue('entityid') + ' VendorId = ' + nlapiGetRecordId());
				return;
			}
		}
	...
	for (var k = 0; k < 1; k++) {
        ...
        + "</email>";
			...
			var skipPOMethod = context.getSetting('SCRIPT', 'custscript_vendor_skip_po_method_update');
			nlapiLogExecution("DEBUG", "skipPOMethod: " + skipPOMethod, "isCreateMode: " + isCreateMode);
			if (skipPOMethod == "T" && isCreateMode) {
				postData = postData + "<po-method>" + out_pomethod + "</po-method>";
			} else if (!skipPOMethod || skipPOMethod == "F") {
				postData = postData + "<po-method>" + out_pomethod + "</po-method>";
			}
			...
			var isDefaultShipping = '', defShipAddFound = false;
			...
			if (context.getSetting('SCRIPT',
				...
				if (contentgrpvalue) {
					...
					if (supplierId != "" && supplierId != null) {
						...
						var blankURL = nlapiGetContext().getSetting('SCRIPT', 'custscript_vendor_url') + '/api/suppliers/' + supplierId;
...
function getCoupaSupplier(supplierName, supplierNumber, url, header, supplier) {
	var getResponse = '';
	var nameUrl = url + '/api/suppliers?name=' + supplierName.replace(/ /g, '%20').replace(/&/g, '%26').replace(/#/g, '%23').replace(/\+/g, '%2B');
	var numberUrl = url + '/api/suppliers?number=' + supplierNumber.replace(/ /g, '%20');
...
function createRTA(record) {
	try {
		...
		if (supplierID == "" || supplierID == null || supplierID == undefined) {
			...
			supplierID = getCoupaSupplier(record.getFieldValue('entityid'), record.getId(), nlapiGetContext().getSetting('SCRIPT', 'custscript_vendor_url'), headers, record);
		}
		if (supplierID == "" || supplierID == null || supplierID == undefined) {
		...
		if (verifyCount) {
			...
			var base_url = nlapiGetContext().getSetting('SCRIPT', 'custscript_vendor_url');
...
function clearEveryoneContentGrp(supplierId) {
	...
	if (supplierId != "" && supplierId != null) {
		// Need to clear out Everyone Content Group
		var everyoneUrl = nlapiGetContext().getSetting('SCRIPT', 'custscript_vendor_url') + '/api/suppliers/' + supplierId + '/business_groups/1/remove';
		var everyonePayload = '<supplier><content-groups></content-groups></supplier>';
...
function syncContentGroups(record, externalid) {
	...
	var contentGroupMap;
	var url = nlapiGetContext().getSetting('SCRIPT', 'custscript_vendor_url') + '/api/suppliers/' + externalid;
...
function syncSubsAsCustomField(record, externalid) {
	// Setting up Headers
	var headers = new Array(), contentResponse = "";
