...
function userEventAfterSubmit(type) {
	...
	if (type == 'delete') {
		...
		nlapiLogExecution('DEBUG', 'TYPE IS DELETE')
		var recordSubsidiary = record.getFieldValue('subsidiary');
		var deleted_recordName = record.getFieldValue('name');
		if (currentType == 'account') {
			var deleted_recordName = record.getFieldValue('acctname');
			var accntNumber = record.getFieldValue('acctnumber');
		} else {
			recordName = record.getFieldValue('name');
		}
		nlapiLogExecution('DEBUG', 'TYPE IS DELETE', 'Subs ' + recordSubsidiary + ' name ' + deleted_recordName);
	}
	...
	var param_useExternalID = context.getSetting('SCRIPT',
		'custscript_coupa_accs_extid');
	var accntNumber = "";
	if (currentType == 'account') {
	...
	var customNameField = context.getSetting('SCRIPT',
		'custscript_coupa_accs_namefield');
	if (customNameField != '' && customNameField != null) {
		recordName = record.getFieldValue(customNameField);
	}
	nlapiLogExecution('AUDIT', 'Account Details ', 'Name = ' + recordName +
		' Type = ' + currentType);

	// Setting up Headers
	var oidcHeader = getAPIHeader('text/xml');
	var headers = new Array();
	if (oidcHeader) {
		headers = oidcHeader;
	} else {
		var param_APIKey = context.getSetting('SCRIPT','custscript_coupa_accs_apikey');	//NIB# 331 Use API_KEY header if OIDC parameters are not available
		headers['Accept'] = 'text/xml';
		headers['X-COUPA-API-KEY'] = param_APIKey;
	}

	var url = nlapiGetContext().getSetting('SCRIPT',
			'custscript_coupa_accs_url') +
		'/api/lookup_values';
	...
	if (nlapiGetContext().getSetting('SCRIPT',
			'custscript_coupa_accs_cstmfldincludeonly')) {
		var custfieldincludeonly = nlapiGetContext().getSetting('SCRIPT',
			'custscript_coupa_accs_cstmfldincludeonly');
	...
	try {
		...
		var param_ExternalID = record.getFieldValue('externalid');

		nlapiLogExecution('DEBUG', 'external ID  ', param_ExternalID);

		var recID = '';
		if (param_useExternalID == 'T') {
			recID = param_ExternalID;
		} else {
			recID = recordID;
		}
		nlapiLogExecution('DEBUG', 'record ID  ', recID);
		nlapiLogExecution('DEBUG', 'currentType ', currentType);

		var lookupList = getLookupList(recID, currentType, recordSubsidiary, deleted_recordName);
		...
		nlapiLogExecution('DEBUG', 'Payload', postData);
		if (currentType == 'account') {
			postData = postData + "<external-ref-num>" +
				record.getFieldValue('acctnumber') +
				"</external-ref-num>";
		} else {
			nlapiLogExecution('DEBUG', 'IS External ID? = ', param_useExternalID);
			if (param_useExternalID == 'T') {
				nlapiLogExecution('DEBUG', 'External ID = ', param_ExternalID);
				postData = postData + "<external-ref-num>" + param_ExternalID +
					"</external-ref-num>";
			} else {
		...
		var parents = [];
		var parentFieldId = nlapiGetContext().getSetting('SCRIPT', 'custscript_parent_field_id');
		if (parentFieldId != null && parentFieldId != '') {
			// get field from record
			var parent = record.getFieldValue(parentFieldId);
			// if spaces, or special char, loop
			if (parent.indexOf(' ') > -1) {
		...
		postToCoupa(postData, lookupList[0], parents, url, headers, type, recordName,
			accntNumber);

	} // try statement
	catch (error) {
		nlapiLogExecution('ERROR', 'Error in userEventAfterSubmit()', JSON.stringify(error));
		if (error instanceof nlobjError) {
			...
			nlapiSendEmail(-5, nlapiGetContext().getSetting('SCRIPT',
					'custscript_coupa_accs_notify_email'), +'Dynamic Account Integration:Processing Error',
			...
			nlapiLogExecution('ERROR', 'Process Error In Try!', 'Error = ' +
				error);
			nlapiSendEmail(-5, nlapiGetContext().getSetting('SCRIPT',
					'custscript_coupa_accs_notify_email'), +'Dynamic Account Integration:Processing Error', 'Error = ' +
				error);
		}
	}
 ...
 */
function getLookupList(recID, recType, recordSubsidiary, deleted_recordName) {
	nlapiLogExecution('DEBUG', 'in getLookupList', recType + ' recID ' + recID + ' deleted rec subsidiary ' + recordSubsidiary + ' deleted rec ' + deleted_recordName);

	var subList = getSearchResults(recID, recType, recordSubsidiary, deleted_recordName);
	...
	var lookupNamePrefix = nlapiGetContext().getSetting('SCRIPT',
		'custscript_lookup_name_prefix');
	if (lookupNamePrefix != '' && lookupNamePrefix != null) {
		prefix = lookupNamePrefix;
	}

	// Search for subsidiaries not included in Coupa, and remove subsidiaries
	// from lookupList that are in this set
	var subcustincludeonly = nlapiGetContext().getSetting('SCRIPT',
		'custscript_coupa_accs_dynamic_sub_includ');
	var inCoupaFilter = new Array();
	inCoupaFilter[0] = new nlobjSearchFilter(subcustincludeonly, null, 'is',
		'T');
	var subColumn = new nlobjSearchColumn('namenohierarchy');
	var resultList = nlapiSearchRecord('subsidiary', null, inCoupaFilter,
		subColumn);
	...
	subArray = coupaInclude.filter(function (x) {
		if (subArray.indexOf(x + ", ") > -1) {
			...
			return true;
		} else if (subArray.indexOf(x + ",") > -1) {		//NIB-304 Added support for subsidiaries if Multi-select field present on Segment
			return true;
		} else if (subArray.indexOf("," + x) > -1) {		//NIB-304 Added support for subsidiaries if Multi-select field present on Segment
			return true;
 ...
 */
function getSearchResults(id, type, recordSubsidiary, deleted_recordName) {
	nlapiLogExecution('DEBUG', 'in getSearchResults', type + ' recID ' + id);
	if (nlapiGetContext().getSetting('SCRIPT',
			'custscript_coupa_accs_one_subsidiary') == 'T') {
	...
	//modified NS Enhacements
	var context = nlapiGetContext();
	var param_useExternalID = context.getSetting('SCRIPT',
		'custscript_coupa_accs_extid');
	var searchField = 'internalid';
	if (param_useExternalID == 'T') {
		searchField = 'externalid';
	}

	var idFilter = new nlobjSearchFilter(searchField, null, 'is', id);
	nlapiLogExecution('DEBUG', 'in getSearchResults', type + ' recID ' + id + ' recName ' + deleted_recordName);
	nlapiLogExecution('DEBUG', 'in SUBSIDIARY ', recordSubsidiary + ' recID ' + id + ' recName ' + deleted_recordName);
	...
	if (type == 'account') {
		...
		if (searchResults == null || searchResults == ' ') {
			var accName = deleted_recordName
			nlapiLogExecution('DEBUG', 'in Account ', accName);

			var sub = getSubsidiary(recordSubsidiary);
			nlapiLogExecution('DEBUG', 'executing IF in accountign - deleted sub internal ID is', recordSubsidiary);
		} else {
			if(!searchResults[0].getValue('name')){
				
				accName = searchResults[0].getValue('displayname');
				nlapiLogExecution('DEBUG', 'Account name getvalue(displayname)', accName);

			}else{
				//NIB# 345: From 2021.1 Name search column will only return the name of the account and displayname will return the Account Number along with the Account Name
				searchResults = nlapiSearchRecord("account",null, idFilter, 
					[
				...
				if(searchResults && searchResults.length>0){
					accName = searchResults[0].getValue('displayname');
					nlapiLogExecution('AUDIT', 'Account searched on the basis of displayname instead of name according to NS 2021.1 change. Account Name: ', accName);
				}
			}
	...
	if (type == 'department') {
		nlapiLogExecution('DEBUG', 'type == department', "DEPARTMENT");

		var searchResults = nlapiSearchRecord('department',
			'customsearch_coupa_accs_deptsearch', idFilter, null);

		nlapiLogExecution('DEBUG', 'type == department', searchResults);
		if (searchResults == null || searchResults == ' ') {
			nlapiLogExecution('DEBUG', 'executing IF - deleted sub internal ID is', recordSubsidiary);
			var sub = getSubsidiary(recordSubsidiary);
			nlapiLogExecution('DEBUG', 'executing IF - deleted rec name is', deleted_recordName);
			var deptHierarchicalName = deleted_recordName;
		} else {
			var sub = searchResults[0].getValue('subsidiary');
			var deptHierarchicalName = searchResults[0].getValue('name');
		}

		nlapiLogExecution('DEBUG', 'type == department', "After If statment");

		if (sub.indexOf('More..') > -1) {
			nlapiLogExecution('DEBUG', 'type == department', "INDEOX OF > -1");
	...
	if (type == 'location') {
		...
		if (searchResults == null || searchResults == ' ') {
			nlapiLogExecution('DEBUG', 'executing IF - deleted subsidairy  is', recordSubsidiary);
			var sub = getSubsidiary(recordSubsidiary);
			nlapiLogExecution('DEBUG', 'executing IF - deleted record name is', deleted_recordName);
			var locHierarchicalName = deleted_recordName;
		} else {
			var sub = searchResults[0].getValue('subsidiary');
	...
	var customNameField = context.getSetting('SCRIPT',
		'custscript_coupa_accs_namefield');
	try {
		nlapiLogExecution('DEBUG', 'Get Seach result ', "Try/catch");
		filters[0] = [searchField, 'is', id];
		if (customNameField != '' && customNameField != null) {
			columns[0] = new nlobjSearchColumn(customNameField);
		} else {
			columns[0] = new nlobjSearchColumn('name');
		}
		columns[1] = new nlobjSearchColumn('subsidiarynohierarchy');
		searchResults = nlapiSearchRecord(type, null, filters, columns);
		sub = searchResults[0].getText('subsidiarynohierarchy');
	} catch (error) {
		if (error.getCode() == 'SSS_INVALID_SRCH_COL') {
			try {
				filters[0] = [searchField, 'is', id];
				if (customNameField != '' && customNameField != null) {
					columns[0] = new nlobjSearchColumn(customNameField);
				...
				searchResults = nlapiSearchRecord(type, null, filters, columns);
				sub = searchResults[0].getText('subsidiary');
			} catch (error) {
				if (error.getCode() == 'SSS_INVALID_SRCH_COL') {
					var subsidiaryColumn = context.getSetting('SCRIPT',
						'custscript_coupa_accs_subfield');
					filters[0] = [searchField, 'is', id];
					nlapiLogExecution('AUDIT', 'In search', 'about to search for subsidiaryColumn');
					if (customNameField != '' && customNameField != null) {
						columns[0] = new nlobjSearchColumn(customNameField);
					} else {
						columns[0] = new nlobjSearchColumn('name');
					}
					columns[1] = new nlobjSearchColumn(subsidiaryColumn);
					searchResults = nlapiSearchRecord(type, null, filters, columns);
					sub = searchResults[0].getText(subsidiaryColumn);
				} else {
					throw error;
				}
			}
	...
	var recordHierarchicalName;
	if (customNameField != '' && customNameField != null) {
		recordHierarchicalName = searchResults[0].getValue(customNameField);
	} else {
		recordHierarchicalName = searchResults[0].getValue('name');
	}
	if (sub.indexOf('More..') > -1) {
		sub = parentSubsidiarySearch(type, recordHierarchicalName, id);
	}
	nlapiLogExecution('AUDIT', 'Subsidiary found: ', sub + ' for ' + type +
		': ' + recordHierarchicalName);
	return [recordHierarchicalName, sub];

}

function parentSubsidiarySearch(type, name, objectId) {
	nlapiLogExecution('DEBUG', 'objectId in parentSubsidiarySearch', objectId);
	var context = nlapiGetContext();	//NIB-333 Context was not defined
	var param_useExternalID = context.getSetting('SCRIPT','custscript_coupa_accs_extid');
	nlapiLogExecution('DEBUG', 'param_useExternalID: ', param_useExternalID);
    if(param_useExternalID == 'T') {
       var internalId = getRecordIdByExternalId(type, objectId);
       objectId = internalId ? internalId : objectId;
    }
...
function getSubsidiary(recordSubsidiary) {
	nlapiLogExecution('DEBUG', ' getSubsidiary looking for subsidiary with internal ID: ' + recordSubsidiary);
	var internalid = recordSubsidiary;
	var idFilter = new Array();
	idFilter[0] = new nlobjSearchFilter('internalid', null, 'is', internalid);
	var column = new nlobjSearchColumn('namenohierarchy');
	var subList = "";
	var subs = nlapiSearchRecord('subsidiary', null, idFilter, column);
	for (var result in subs) {
	...
	subList = subList.slice(0, -2);
	nlapiLogExecution('DEBUG', 'getSubsidiary subsidiary name is ' + subList);
	return subList;
 ...
 * @param name
 */
function postToCoupa(payload, lookups, parents, url, headers, type, name, glNum) {
	for (var i = 0; i < lookups.length; i++) {
		...
		var numLoops = parents.length;
		if (numLoops == 0) {
			numLoops = 1;
		}
		for (var k = 0; k < numLoops; k++) {
			var partialPayload = payload;
			partialPayload = partialPayload + "<lookup><name>" + "<![CDATA[" + lookup_name + "]]>" +
			"</name></lookup>";
			if (parents.length > 0) {
				partialPayload = partialPayload + "<parent><external-ref-num>" + parents[k] + '</external-ref-num></parent>';
			}
			partialPayload = partialPayload + "</lookup-value>";
			var existsInCoupa = false;
			var coupaLookupValueId = "";
			var lookupName = encodeURIComponent(lookup_name);
			if (type == 'edit' || type == 'delete') {
				...
				if (param_useExternalID != null && param_useExternalID == 'T') {
					//recordID = record.getFieldValue('externalid');
					prevrecord = nlapiGetOldRecord();
					recordID = prevrecord.getFieldValue('externalid');
				}

				var GETurl = url + '?external-ref-num=' + recordID +
					'&[lookup][name]=' + lookupName;
				if (parents.length > 0) {
					GETurl = GETurl + '&[parent][external-ref-num]=' + parents[k];
				}
				...
				nlapiLogExecution('DEBUG', 'GETurl response code is = ',
					getResponse.getCode());
				if (getResponse.getCode() == '200') {
					existsInCoupa = true;
					var responseXML = nlapiStringToXML(getResponse.getBody());
					coupaLookupValueId = nlapiSelectValue(responseXML,
						'lookup-values/lookup-value/id');
					nlapiLogExecution('DEBUG', 'LookupValue ID is = ',
						coupaLookupValueId);
				} else {
					nlapiLogExecution("DEBUG", "Failure to retrieve lookupvalue",
						"Response Code: " + getResponse.getCode() +
			...
			if (existsInCoupa) {
				...
				nlapiLogExecution('DEBUG', 'within PUT payload = ', partialPayload);
				...
				nlapiLogExecution('DEBUG', 'within POST payload = ', partialPayload);
...
/**
 * Description: search for the internal Id of the segment by External Id
 * NIB#: NIB-307
 * @param type string
 * @param externalId string
 * @return {array} responseArray
 */
function getRecordIdByExternalId(type, externalId) {
	var internalId = ""
	if (externalId) {
		var idFilter = new nlobjSearchFilter('externalid', null, 'is', externalId);
		var searchResults = nlapiSearchRecord(type, null, idFilter, null);
		if (searchResults && searchResults.length > 0) {
			internalId = searchResults[0].getId();
		}
	}
	nlapiLogExecution('AUDIT', 'Returning internal Id: ' + internalId,' for external id' + externalId);
	return internalId;
}