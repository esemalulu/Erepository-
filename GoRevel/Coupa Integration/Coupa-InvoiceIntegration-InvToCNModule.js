...
/*******************************************************************************
 *
 * Name: Coupa-InvoiceIntegration-InvToCNModule.js
 *
 * Script Type: SuiteScript 2.1 Module
 *
 * Description: This module handle the import functionality for creating Vendor Credit from Coupa Invoice
 *
...
define(['N/email', 'N/https', 'N/record', 'N/runtime', 'N/search', 'N/url', './Coupa-UtilityFunctionsModule'],
    ...
    (email, https, record, runtime, search, url, utility) => {
        ...
        const createVendorCreditFromInvoice = (invoicePayload, configuration) => {
            try {
                if (utility.validateDocument(invoicePayload)) {
                    ...
                    let taxAmount = utility.flipSign(utility.parseFloatOrZero(invoicePayload['tax-amount']));
                    ...
                    for (let ic in invoiceCharges) {
                        ...
                        charge.total = utility.flipSign(utility.parseFloatOrZero(invoiceCharges[ic].total));
                        ...
                        for (let tl in taxLines) {
                            let taxLine = {};
                            taxLine.amount = utility.flipSign(utility.parseFloatOrZero(taxLines[tl].amount));
                            chargeTaxTotal += utility.flipSign(utility.parseFloatOrZero(taxLines[tl].amount));
                    ...
                    let totalMiscAmount = utility.flipSign(utility.parseFloatOrZero(invoicePayload["misc-amount"]));
                    let totalHandlingAmount = utility.flipSign(utility.parseFloatOrZero(invoicePayload["handling-amount"]));
                    let totalShippingAmount = utility.flipSign(utility.parseFloatOrZero(invoicePayload["shipping-amount"]));

                    let linesNetTotal = utility.flipSign(utility.parseFloatOrZero(invoicePayload["total-with-taxes"])) - (taxAmount + totalMiscAmount + totalHandlingAmount + totalShippingAmount);
                    log.debug({
                    ...
                    for (let il in invoiceLines) {
                        ...
                        if (!configuration.sendTaxCode && invoiceLines[il] && invoiceLines[il] ['account']) {                                             //Get Line detail JSON for non-split condition
                            ...
                            invoiceLinesArray.push(utility.getNonSplitAccountExpenseLine(invoiceLines[il], tempObject, configuration, true, headerTaxArray, lineLevelTaxationFlag));
                            ...
                            let tempArray = utility.getSplitAccountExpenseLine(invoiceLines[il], tempObject, configuration, true, headerTaxArray, lineLevelTaxationFlag);
                         ...
                         *****************************************/
                            ...
                            if (!configuration.useCoupaChargeDistribution) {
                                tempObject.taxAmount = utility.formatCurrency(utility.calculateProportion(utility.flipSign(invoiceLines[il]['total']), linesNetTotal, taxAmount));
                                tempObject.totalShippingAmount = utility.formatCurrency(utility.calculateProportion(utility.flipSign(invoiceLines[il]['total']), linesNetTotal, utility.getChargeTotal(chargesArray, 'InvoiceShippingCharge')));
                                tempObject.totalHandlingAmount = utility.formatCurrency(utility.calculateProportion(utility.flipSign(invoiceLines[il]['total']), linesNetTotal, utility.getChargeTotal(chargesArray, 'InvoiceHandlingCharge')));
                                tempObject.totalMiscAmount = utility.formatCurrency(utility.calculateProportion(utility.flipSign(invoiceLines[il]['total']), linesNetTotal, utility.getChargeTotal(chargesArray, 'InvoiceMiscCharge')));
                            } else {
                                tempObject.taxAmount = utility.formatCurrency(utility.flipSign(utility.parseFloatOrZero(invoiceLines[il]['tax-distribution-total'])));
                                tempObject.totalShippingAmount = utility.formatCurrency(utility.flipSign(utility.parseFloatOrZero(invoiceLines[il]['shipping-distribution-total'])));
                                tempObject.totalHandlingAmount = utility.formatCurrency(utility.flipSign(utility.parseFloatOrZero(invoiceLines[il]['handling-distribution-total'])));
                                tempObject.totalMiscAmount = utility.formatCurrency(utility.flipSign(utility.parseFloatOrZero(invoiceLines[il]['misc-distribution-total'])));
                            }
                            tempObject["invoiceLineID"] = "Invoice: " + invoicePayload.id + " - " + (utility.parseFloatOrZero(il) + 1);
                            let tempArray = utility.getNonSplitAccExpnsLineWithTaxCode(invoiceLines[il], tempObject, configuration, chargesArray, true, headerTaxArray);
                            invoiceLinesArray.push(...tempArray);
                        } else if (configuration.sendTaxCode && invoiceLines[il] && !invoiceLines[il] ['account']) {                                                                                                  //Get Line detail JSON for split condition
                            log.debug({
                                title: 'Invoice ID: ' + invoicePayload.id + ' Line#' + (utility.parseFloatOrZero(il) + 1),
                            ...
                            for (let acc in invoiceLines[il]['account-allocations']) {
                                ...
                                if (!configuration.useCoupaChargeDistribution) {
                                    tempObject.taxAmount = utility.formatCurrency(utility.calculateProportion(utility.flipSign(invoiceLines[il]['account-allocations'][acc]['amount']), linesNetTotal, taxAmount));
                                    tempObject.totalShippingAmount = utility.formatCurrency(utility.calculateProportion(utility.flipSign(invoiceLines[il]['account-allocations'][acc]['amount']), linesNetTotal, utility.getChargeTotal(chargesArray, 'InvoiceShippingCharge')));
                                    tempObject.totalHandlingAmount = utility.formatCurrency(utility.calculateProportion(utility.flipSign(invoiceLines[il]['account-allocations'][acc]['amount']), linesNetTotal, utility.getChargeTotal(chargesArray, 'InvoiceHandlingCharge')));
                                    tempObject.totalMiscAmount = utility.formatCurrency(utility.calculateProportion(utility.flipSign(invoiceLines[il]['account-allocations'][acc]['amount']), linesNetTotal, utility.getChargeTotal(chargesArray, 'InvoiceMiscCharge')));
                                } else {
                                    tempObject.taxAmount = utility.formatCurrency(utility.flipSign(utility.parseFloatOrZero(invoiceLines[il]['account-allocations'][acc]['tax-distribution-total'])));
                                    tempObject.totalShippingAmount = utility.formatCurrency(utility.flipSign(utility.parseFloatOrZero(invoiceLines[il]['account-allocations'][acc]['shipping-distribution-total'])));
                                    tempObject.totalHandlingAmount = utility.formatCurrency(utility.flipSign(utility.parseFloatOrZero(invoiceLines[il]['account-allocations'][acc]['handling-distribution-total'])));
                                    tempObject.totalMiscAmount = utility.formatCurrency(utility.flipSign(utility.parseFloatOrZero(invoiceLines[il]['account-allocations'][acc]['misc-distribution-total'])));
                                }
                                tempObject["invoiceLineID"] = "Invoice: " + invoicePayload.id + " - " + (utility.parseFloatOrZero(il) + 1);
                                let tempArray = utility.getSplitAccExpnsLineWithTaxCode(invoiceLines[il], invoiceLines[il]['account-allocations'][acc], tempObject, configuration, chargesArray, true, headerTaxArray);
                                invoiceLinesArray.push(...tempArray);
                            }
                        }
                    }
                     ...
                     *
                     *****************************************/
                    vendorCreditRecord = utility.setExpenseSublist(configuration, vendorCreditRecord, chargesArray, invoiceLinesArray, invoicePayload, true);
