...
define(
    [ 'N/email', 'N/https', 'N/search', 'N/runtime', 'N/record', 'N/error',
        'N/xml', 'N/config', './Coupa - OpenIDConnect 2.0' ],
    function(email, https, search, runtime, record, error, xml, config, oidc) {
        ...
        function map(context) {
            ...
            //get checkbox param for bank record mapping
            var skip_bank_record  = scriptRef
            .getParameter('custscript_coupa_sim_bank_sync');
            ...
            var getUrl = coupa_url + '/api/supplier_information/'+ obj.id;
            log.debug('Retrieve SIM record', getUrl);

            var response = https.get({
                url : getUrl,
                headers : obj.requestHeader
            });

            if (response.code == 200) {
                // good response
                log.audit('Retrieve SIM record', "Successfully retrieved SIM record id " + obj.id);
            } else if (resonse.code == 404) {
                log.audit('Retrieve SIM record', 'Failure to retrieve SIM record id ' + obj.id);
            } else {
                // bad response
            ...
            try {
                ...
                try {
                    ...
                    log.debug('Phone Number Skipped','Error retrieving Contact\'s Phone, no phone number will be set');
                }
                ...
                var subsidiaryArray = [];
                var subMap = {};
                var numLines = ns_vendor_record.getLineCount({
                    sublistId: 'submachine'
                });

                for (var k = 0; k < numLines; k++) {
                	var subsidiary = ns_vendor_record.getSublistValue({
                		sublistId: 'submachine',
                		fieldId: 'subsidiary',
                		line: k
                	});
                	subMap[subsidiary] = subsidiary;
                }

                var customFieldsNode = xml.XPath.select({
                	node : sim_records[0],
                	xpath : 'custom-fields'
                });

                subsidiaryArray = getSubsididaryList(customFieldsNode[0]);

                for (var subs = 0; subsidiaryArray && subs < subsidiaryArray.length; subs++) {
                	if (subMap[subsidiaryArray[subs]] == undefined) {
                		ns_vendor_record.selectNewLine({
                			sublistId: 'submachine'
                		});
                		ns_vendor_record.setCurrentSublistValue({
                			sublistId: 'submachine',
                			fieldId: 'subsidiary',
                			value: subsidiaryArray[subs]
                		});
                		ns_vendor_record.commitLine({
                			sublistId: 'submachine'
                		});
                	}
                }
                ...
                for (var a = 0; a < address_records.length; a++) {
                    ...
                    log.debug("Bank Record Mapping",
                        ...
                        + c_address_id);
                       log.debug("bank sync checkbox is set to", skip_bank_record  ) ;
                       

                    if((skip_bank_record  == "false" || skip_bank_record  == false) || (skip_bank_record == null || skip_bank_record == ''))  {
                        ...
                        if (scriptRef
                            ...
                            if (bank_type == 'customrecord_pp_ach_account') {
                                ...
                                var bankData = {
                                    ...
                                    custrecord_pp_ach_is_primary : true, // always set the last bank record as primary 
                                    name : c_supp_name,
                                    externalid : "CoupaAddress_"
                                        .concat(c_address_id)
                                };
                   ...
                   log.debug("bank_record is set to ", skip_bank_record  + " skipped bank mapping ")
        ...
        function getCustomFields(custom_field_mappings, xml_node, ns_record) {
            ...
            for (var y = 0; y < mapping_pairs.length; y++) {
                if(mapping_pairs[y]) {
                    ...
                    var custFieldtype=mappings[1].split(":");
                    mappings[1]=custFieldtype[0];
                    var xml_element;
                    try {
                        if (custFieldtype.length>1)
                        {
                            if(custFieldtype[1].toUpperCase() == "LOOKUP")
                            {
                            ...
                            else if (custFieldtype[1].toUpperCase() == "TEXT")
                            {
        ...
        function validate() {
            ...
            if (coupa_url == null || default_coupa_sub == null) {
                throw errorObj;
            }
        }
		...
		function getSubsididaryList(customFieldNode) {
			var subsidiaryArray = [];
			var scriptRef = runtime.getCurrentScript();
			var subsidiaryMapping = scriptRef.getParameter('custscript_coupa_sim_subs_multiselect');
			//verify if script parameter is available and contains the multi-select field id
			if (subsidiaryMapping && subsidiaryMapping.length > 0) {
				var multiSelectField = xml.XPath.select({
					node: customFieldNode,
					xpath: subsidiaryMapping
				});

				var subsidiariesArray = xml.XPath.select({
					node: multiSelectField[0],
					xpath: subsidiaryMapping
				});

				for (var s = 0; subsidiariesArray && s < subsidiariesArray.length; s++) {
					var subsidiaryID = xml.XPath.select({
						node: subsidiariesArray[s],
						xpath: 'external-ref-code'
					})[0].textContent
					if (subsidiaryID) {
						log.debug("Adding subsidiaryID to the Aray: ", subsidiaryID);
						subsidiaryArray.push(subsidiaryID);
					}
				}
			} else{
				log.audit("Multi-select subsidiary skipped", "Multi-select subsidiary custom field Ind not found in script parameter");
			}
			log.audit("subsidiaryArray returned from getSubsididaryList(): ", JSON.stringify(subsidiaryArray));
