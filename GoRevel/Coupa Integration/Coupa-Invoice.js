...
var emailNotifier = context.getSetting('SCRIPT', 'custscript_coupa_inv_error_email_sender') ? context.getSetting('SCRIPT', 'custscript_coupa_inv_error_email_sender') : -5;
...
if (oidcHeader) {
	...
	var param_APIKey = context.getSetting('SCRIPT','custscript_coupa_inv_apikey');	//NIB# 331 Use API_KEY header if OIDC parameters are not available
    ...
    if (response.getCode() == '200') {
        ...
        for (var i = 0; i < invoiceHeaderNodes.length; i++) {
            ...
            var entityid = nlapiSelectValue(invoiceHeaderNodes[i],
                'supplier/number');
            var paymentChannel = nlapiSelectValue(invoiceHeaderNodes[i], 'payment-channel');
            var skipVcPo = nlapiGetContext().getSetting('SCRIPT','custscript_coupa_inv_skip_bill_for_vc_po');
            ...
            if (invoiceexists == 'false')
                try {
                    if(skipVcPo == 'T' && paymentChannel == 'coupapay_virtual_card_po'){	//NIB-311:  skipInvoices with paymentChannel as coupapay_virtual_card_po to avoid double payment if Skip Virtual Card PO CB is checked.
                         nlapiLogExecution('AUDIT', 'Skipping Vendor Bill creation','Skipping vendor bill creation  & not marking invoice ' + externalid + ' number: ' + tranid + ' exported as it is for payment channel: "Coupa Pay - Virtual card for PO"');
                         continue;
                    }
                  ...
                  if (existingBill.getFieldValue('paymenthold')){
                    ...
                    if(nlapiSubmitRecord(existingBill, true)) {
                      ...
                      nlapiSendEmail(
                        ...
                        try {
                            var paymentHold = nlapiLookupField('vendorbill', invoiceexists, 'paymenthold'); 
                            if (paymentHold == 'F' && nlapiSelectValue(invoiceHeaderNodes[i], 'payment-channel') == 'coupapay_invoice_payment') {
                                nlapiSubmitField('vendorbill', invoiceexists, 'paymenthold', 'T');  // NIB#:330
                                 Setexportedtotrue(nlapiSelectValue(invoiceHeaderNodes[i], 'id'));
                            }
                        } catch (err) {
                            nlapiLogExecution('ERROR', 'Error while setting Invoice on Hold', err.message);
                        }
...
function CreateVendorBillorVendorCredit(invoice) {
    ...
    var invoiceNumber = nlapiSelectValue(invoice, 'invoice-number');
    var invoiceID = nlapiSelectValue(invoice, 'id');
    var invoiceEntityID = nlapiSelectValue(invoice, 'supplier/number');
    var mixedSignLines = (positiveFlag != negativeFlag ? false : true);
    var totalAmountDue = nlapiSelectValue(invoice, 'total-with-taxes');

    nlapiLogExecution('DEBUG', 'totalAmountDue: ', totalAmountDue);
    ...
    if (creditMemoOption == 1) {
        ...
        if (credit == true) {
            if (mixedSignLines) {
                nlapiLogExecution("AUDIT", "INVALID_INVOICE_TYPE", "Coupa NetSuite P2P Bundle does not support integrating Invoices/Credit Notes with mixed (Positive & Negative) invoice lines.\n\n Coupa Invoice ID : " + invoiceID + " Invoice Number : " + invoiceNumber + " Vendor : " + invoiceEntityID);
                return;
            }
            CreateVendorCredit(invoice);
        }
    } else if (creditMemoOption == 2) {
        if (invoicetotal >= 0 || totalAmountDue >= 0) {
...
function CreateVendorCredit(invoice) {
    ...
    var invoiceLineNodes = new Array();​
    invoiceLineNodes = nlapiSelectNodes(invoiceLine, 'invoice-line');
    var isInvToCredit = false;
    var allNegativeFlag = true;
    ...
    var totalAmountDue = nlapiSelectValue(invoice, 'total-with-taxes');
    nlapiLogExecution('DEBUG', 'totalAmountDue', totalAmountDue);
    if(totalAmountDue < 0 && invoiceType == 'Credit Note'){
        isInvToCredit = true;
    }
    else {
        if (invoiceType == 'Credit Note') {
    ...
    if(shippingamount > 0 || handlingamount > 0 || taxamount > 0 || miscamount > 0){
        allNegativeFlag = false;
    }
    var sendTaxCodeParam = nlapiGetContext().getSetting('SCRIPT', 'custscript_coupa_inv_send_taxcode') == 'T' ? true : false;
    if(isInvToCredit && (!allNegativeFlag || sendTaxCodeParam)){
      if(shippingamount < 0 || shippingamount > 0){
        shippingamount = shippingamount * (-1);
      }
      if(handlingamount < 0 || handlingamount > 0){
        handlingamount = handlingamount * (-1);
      }
      if(taxamount < 0 || taxamount > 0){
        taxamount = taxamount * (-1);
      }      
      if(miscamount < 0 || miscamount > 0){
        miscamount = miscamount * (-1);
      }
    }
    nlapiLogExecution('AUDIT', 'isInvToCredit: ' + isInvToCredit, ' shippingamount: ' + shippingamount + ' handlingamount: ' + handlingamount + ' taxamount: ' + taxamount + ' miscamount: ' + miscamount);
         ...
         {			 
 			...
 			else 
 			{
				...
				if (shippingamount != '' && shippingamount != null && shippingamount != 0.0)
				{
					...
					if (nlapiSelectValue(chargeLineNodes[cnt], 'tax-lines/tax-line/amount') != '' && nlapiSelectValue(chargeLineNodes[cnt], 'tax-lines/tax-line/amount') != null && !isInvToCredit)
 					{
                        totalheaderlinecharge = parseFloat(totalheaderlinecharge) + parseFloat(nlapiSelectValue(chargeLineNodes[cnt], 'tax-lines/tax-line/amount') ? nlapiSelectValue(chargeLineNodes[cnt], 'tax-lines/tax-line/amount') : 0);
 					} else {
                        totalheaderlinecharge = parseFloat(totalheaderlinecharge) - parseFloat(nlapiSelectValue(chargeLineNodes[cnt], 'tax-lines/tax-line/amount') ? nlapiSelectValue(chargeLineNodes[cnt], 'tax-lines/tax-line/amount') : 0);
                    }
				}				
    ...
    if(isInvToCredit && (!allNegativeFlag || sendTaxCodeParam)) {
        if(shippingChargeTaxAmount < 0 || shippingChargeTaxAmount > 0){
            shippingChargeTaxAmount = shippingChargeTaxAmount * (-1);
        }
        if(handlingChargeTaxAmount < 0 || handlingChargeTaxAmount > 0){
            handlingChargeTaxAmount = handlingChargeTaxAmount * (-1);
        }
        if(miscChargeTaxAmount < 0 || miscChargeTaxAmount > 0){
            miscChargeTaxAmount = miscChargeTaxAmount * (-1);
        }      
    }
    ...
    headertaxamount = headertaxamount ? parseFloat(headertaxamount) : 0;
   ...
   if (nlapiSelectValue(supplierNode, 'number'))
    ...
    for (var x = 0; x < invoiceLineNodes.length; x++) {
		...
		if (nlapiGetContext().getSetting('SCRIPT',
                'custscript_coupa_inv_send_taxcode') == 'F' &&  !isInvToCredit) {
            linetaxtotalamount = parseFloat(nlapiSelectValue(invoiceLineNodes[x],
                'tax-amount')) + parseFloat(linetaxtotalamount);
                }

        else if((nlapiGetContext().getSetting('SCRIPT',
            'custscript_coupa_inv_send_taxcode') == 'F' && isInvToCredit &&  parseFloat(nlapiSelectValue(invoiceLineNodes[x],
            'tax-amount'))<0)){
            linetaxtotalamount = parseFloat(nlapiSelectValue(invoiceLineNodes[x], 'tax-amount')) - parseFloat(linetaxtotalamount);
        }
    ...
    for (var x = 0; x < invoiceLineNodes.length; x++) {
        ...
        if (splitaccounting == 'TRUE') {
            for (var i = 0; i < accountallocations.length; i++) {
                ...
                if ((i == 0) && (x == 0)){//MD
                    if (nlapiGetContext().getSetting('SCRIPT',
                            ...
                            'custscript_coupa_inv_send_taxcode') == 'T'))
						{
                                ...
                                if (adjlineamount > totalamount) {
                                    headerlinecharge = (parseFloat(adjlineamount) / (parseFloat(totalamount) + parseFloat(headertaxamount))) * totalheaderlinecharge;
                                } else {
                ...
                if(isInvToCredit && (adjlineamount > 0 || adjlineamount < 0)){
                    nlapiLogExecution('DEBUG', 'Take the Inverse of ', adjlineamount);
                    adjlineamount = adjlineamount * (-1);
                }
...
// set accounts payable account if passed as parameter to the script
        ...
        //var today = new Date();   //commented NIB# 370
        var today = nlapiStringToDate(nlapiDateToString(new Date(), "datetimetz")); //NIB# 370 Returns DayLight Saving Time based on Customer's Timezone
        ...
        nlapiLogExecution('DEBUG', 'is Negative Cutoff Day: ' + isNegativeCutoffDay, 'cutoffday: ' + parseFloat(cutoffday));
        ...
        nlapiLogExecution('DEBUG', 'Nmonth: '+Nmonth, 'Nday: ' + Nday + 'Nyear: ' + Nyear);
        var maxDays = daysInMonth(Nmonth,Nyear);
        var thresholdDay = maxDays - Math.abs(parseFloat(cutoffday));
        if (isNegativeCutoffDay && today.getDate() > thresholdDay) {
            if (today.getFullYear() < parseInt(Nyear)) {
                if (parseInt(Nmonth) > 0) {
                    postingPeriod = getMonthShortName(parseInt(Nmonth) - 1) + ' ' + (Nyear);
                } else if (today.getMonth() == 11) {
                    postingPeriod = getMonthShortName('0') + ' ' + (parseInt(Nyear));
                } else {
                    postingPeriod = getMonthShortName(today.getMonth()) + ' ' + (today.getFullYear());
                }
            } else if (today.getMonth() == 11 && Nyear == today.getFullYear()) {
                postingPeriod = getMonthShortName('0') + ' ' + (parseInt(Nyear) + 1);
            } else if (Nyear == today.getFullYear() && (parseInt(today.getMonth()) + 1) < parseInt(Nmonth)) {
                postingPeriod = getMonthShortName(parseInt(Nmonth) - 1) + ' ' + (Nyear);
            } else if (Nyear == today.getFullYear()) {
                postingPeriod = getMonthShortName(parseInt(today.getMonth()) + 1) + ' ' + (Nyear);
            }
        } else if (today.getDate() < cutoffday) {
            var nDate = nlapiSelectValue(invoice, 'invoice-date').split('T');
            var datesplit = nDate[0].split('-');
            var Nyear = datesplit[0];
            // var Nday = datesplit[2];
            ...
            if (Nmonth < today.getMonth() && Nyear == today.getFullYear())
                postingPeriod = getMonthShortName(today.getMonth() - 1) + ' '
                    ...
                    nlapiLogExecution('DEBUG', 'datetime mnonth check"',
...
nlapiLogExecution('ERROR','ELI SUB CHECK1', subsidiaryId);
                    ...
                    if (dynamicAccounting == 'T') 
                    {
                      	...
                      	else
                        {
                            record.setFieldValue('subsidiary', subsidiaryId);
//-------------------------------------------------------------------------------------------------------------------------------------
							if(subsidiaryId == 2) //Revel Transit Inc
                            {
								record.setFieldValue('location', '116');
                            }

							if(subsidiaryId == 3) //Revel Leasing Corp
                            {
								record.setFieldValue('location', '435');
                            }

							if(subsidiaryId == 4) //Electric Peppers LLC
                            {
								record.setFieldValue('location', '436');
                            }
	//-------------------------------------------------------------------------------------------------------------------------------------
    ...
    try {
        ...
        //var todaysactualDate = new Date();
        //nlapiLogExecution('DEBUG', 'todaysactualDate based on NS time',todaysactualDate);

        //var today = getTodayCompanyDate();   //commented NIB# 370
        var today = nlapiStringToDate(nlapiDateToString(new Date(), "datetimetz")); //NIB# 370 Returns DayLight Saving Time based on Customer's Timezone
        nlapiLogExecution('DEBUG', 'Todays date/time based on companytimeZone',today);
        var postingPeriod = getMonthShortName(today.getMonth()) + ' '
            + today.getFullYear();
        var cutoffday = 5;
        if (nlapiGetContext().getSetting('SCRIPT', 'custscript_coupa_inv_cutoffdate'))
        ...
        if (nlapiGetContext().getSetting('SCRIPT', 'custscript_coupa_inv_deptseg')) {
            var dept = nlapiSelectValue(
              ...
              if (dynamicAccounting == 'T') {   
                  ...
                  if( param_useExternalID != null  && param_useExternalID == 'T'){
                      var objInternalID = getInternalIDByExternalId(dept,'department');
                      nlapiLogExecution('DEBUG','Setting department internal ID step 1 == ', objInternalID);
                      record.setCurrentLineItemValue('expense', 'department', objInternalID);
                  }
                  else {
                      record.setCurrentLineItemValue('expense', 'department', dept);
          ...
          if (nlapiGetContext().getSetting('SCRIPT', 'custscript_coupa_inv_classseg')) {
            var clss = nlapiSelectValue(
              ...
              if (dynamicAccounting == 'T') {
                  if( param_useExternalID != null  && param_useExternalID == 'T'){
                      var objInternalID = getInternalIDByExternalId(clss,'classification');
                      nlapiLogExecution('DEBUG','Setting classinternal ID step 1 == ', objInternalID);
                      record.setCurrentLineItemValue('expense', 'class', objInternalID);
                  }
                  else {
                      record.setCurrentLineItemValue('expense', 'class', clss);
                  ...
                  if( param_useExternalID != null  && param_useExternalID == 'T'){
                      var objInternalID = getInternalIDByExternalId(clss[1],'classification');
                      nlapiLogExecution('DEBUG','Setting class internal ID step 1 == ', objInternalID);
                      record.setCurrentLineItemValue('expense', 'class', objInternalID);
                  }
                  else {
                      record.setCurrentLineItemValue('expense', 'class', clss[1]);
          ...
          if (nlapiGetContext().getSetting('SCRIPT', 'custscript_coupa_inv_locseg')) {
            var locId = nlapiSelectValue(
              ...
              if (dynamicAccounting == 'T') {
                  if( param_useExternalID != null  && param_useExternalID == 'T'){
                      var objInternalID = getInternalIDByExternalId(locId,'location');
                      nlapiLogExecution('DEBUG','Setting location internal ID step 1 == ', objInternalID);
                      record.setCurrentLineItemValue('expense', 'location', objInternalID);
                  } else {
                      record.setCurrentLineItemValue('expense', 'location', locId);
                  }
            ...
            if (nlapiGetContext().getSetting('SCRIPT',
              ...
              if (nlapiGetContext().getSetting('SCRIPT',
                ...
                if (custfield[4]) {
                  ...
                  if (custfield[2] == 'Segment') {
                      if (isSplit) {
                          valuetoinsert = nlapiSelectValue(accountNode, custfield[0]);
                      } else {
                          valuetoinsert = nlapiSelectValue(invoiceline, 'account/' + custfield[0]);
                      }
                  }
                }
...
function vendorBillExists(tranid, externalid, entity) {
    nlapiLogExecution('DEBUG','in vendorBillExists', 'tranid = ' + tranid + ' externalid = ' + externalid + ' entity = ' + entity);
    var filters = new Array();
    if (tranid.indexOf("'") > -1) { // Check if the Transaction Id contains an apostrophe to prevent Formula Error
        filters[0] = new nlobjSearchFilter('tranid', null, 'is', tranid);
    } else {
        filters[0] = new nlobjSearchFilter('formulatext', null, 'is', '1');
        filters[0].setFormula("CASE WHEN TO_CHAR({tranid})= TO_CHAR('" + tranid + "') THEN 1 ELSE 0 END");
    }
...
function getNetsuiteAccountId(accountnumber) {
    ...
    filters[1] = new nlobjSearchFilter('isinactive', null, 'is', 'F');  // NIB# 408 - Filter out inactive accounts.
...
//to get todays date based on company pref
function getTodayCompanyDate(){
    ...
    //nlapiLogExecution('DEBUG', 'companyDateTime',new Date(companyDateTime));
...
{
    nlapiLogExecution('DEBUG', 'getInternalIDByExternalId ', 'externalId: ' + JSON.stringify(externalId) + ' recordType: ' + recordType);
    idFilter = new nlobjSearchFilter('externalid', null, 'is', externalId);
    var columns = new Array();
    columns[0] = new nlobjSearchColumn('internalid');
    var savedSearch = '';
    if (externalId == "" || externalId == null || externalId == undefined) {    //NIB-459 - If externalid is undefined or null return "" instead of searching internal id
        nlapiLogExecution('AUDIT', 'returning internal id "" of ' + recordType + ' based on external ID: ' + externalId);
        return "";
    }
...
/**
 * This method returns the number od days in a month by by passing month & year
 * @method
 * @param month
 * @param year
 * @return {number}
