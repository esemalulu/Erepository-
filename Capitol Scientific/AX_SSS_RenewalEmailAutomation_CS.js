 ...
 * This script is customized for Capitol Scientific
 * @author olivier.ahad@it-ration.com
 * @return null
 **/
 ...
 * mod req: 11/12/2012 - log/assign phone calls to salesRepEmail
 * @return nothing
 **/
function setupFollowUpCall(company, daysAfter, tranID, salesRepEmail){
    ...
    try {
        ...
        //Mod Req: phone Calls should be set to salesrep
        if (salesRepEmail) {
        	phoneRecord.setFieldValue('assigned', salesRepEmail);
        }
        
        phoneRecord.setFieldValue('company', company);
        phoneRecord.setFieldValue('transaction', tranID);
        
        var phoneId = nlapiSubmitRecord(phoneRecord); // 10 Units
        
        nlapiLogExecution('debug','Logged new followup call: '+phoneId, 'Logged for SalesRep ID: '+salesRepEmail);
    } 
    catch (e) {
...
function sendEmail(recordType, customerId, address, emailTemplate, returnAddress, addTransaction, tranID){
    try {
        ...
        //Mod Request: 11/12/2012 - Always attach email to Transaction
        if(tranID) {
        ...
        nlapiLogExecution('audit', 'email info', 'TO ' + address + ' FROM ' + returnAddress + '\n' + mailInfo.getName() + '\n' + mailInfo.getValue());
...
function sendEmailWithFailedTransaction(returnAddress, ruleName, listOfFailedTransaction){
    ...
    try {
		...
		nlapiLogExecution('audit', 'failed transactions email info', 'TO ' + returnAddress + ' FROM ' + returnAddress + '\n' + subject + '\n' + body);
...
function processRecord(recordType, tranId, salesRepEmail, savedSearch, companyID, customerEmail, template, returnAddress, addTransaction, followup, callDelay, contactEmail, ruleName, listOfFailedTransaction, customerSalesrep, defaultEmailAuthor){
    ...
    verifyMetering(200, null); // 20 Units
    try {
        ...
        if (contactEmail != null && contactEmail != '') {
            ...
            if (followup == 'T') {
            	//Mod Req 11/12/2012 - phone calls should be logged/assigned to salesrep
                setupFollowUpCall(companyID, callDelay, tranId, salesRepEmail); // 20 Units
            }
        }
        else {
...
function findSavedSearchRecords(savedSearch, template, address, addTransaction, followup, callDelay, ruleName, defaultEmailAuthor){
    ...
    try {
        ...
        searchColumns[4] = new nlobjSearchColumn('salesrep');
        ...
        if (results != null && results != '' && results.length > 0) {
            ...
            for (var i = 0; i < results.length; i++) {
                ...
                var salesrepEmail = results[i].getValue('salesrep');
