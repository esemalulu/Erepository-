    ...
    switch (datain.call) {
        ...
        case "get_shipping_order":
            ...
            break;
             ...
             case "get_return_email":

            return get_return_email(datain.id);
            break;
        ...
        case "test":
         ...
         case "getItemDetailDMDWR":

            return getItemDetailDMDWR(datain.id);
            break;
            case "getItemFullfilmentInternalIDFromName":

            return getItemFullfilmentInternalIDFromName(datain.id);
            break;
...
function get_return_email(){
	var Returnrec = nlapiLoadRecord('returnauthorization', 125069);
	 var ParsedReturnrec = parseRecord(Returnrec);

	//Sales Order Information
    var salesOrderInternalID = ParsedReturnrec.createdfrom.internalid;
       ...
       //  var email = nlapiLookupField("customer", customerId, "email");
        ...
        for (var i = 0; i < ParsedReturnrec.item.length; i++) {
          ...
          var currentItemParsedRecord=parseRecord(currentItemRecord);
				 lineItemInfo.push(item_title + checkIfUndefined(currentItemParsedRecord.displayname) + "</item_title>");
        ...
        try {
            ...
            payment_method = payment_method + checkIfUndefined(salesOrderParsedRecord.paymentmethod.name) + "  " + checkIfUndefined(salesOrderParsedRecord.ccnumber) + "</payment_method>";
   ...
   var response = sendExactTargetEmail(jsonOut, "07082016_return_received_trigger");
    return response;

}


function getItemDetailDMDWR(id) {
    try {
        var record = nlapiLoadRecord("inventoryitem", id);
        var JSONout={};
        //var ItemName = checkIfUndefined(record.salesdescription);


            var ItemName = nlapiLookupField("inventoryitem", id, "displayname");



nlapiLogExecution('DEBUG','ItemName = '+ItemName);
        //var urlAPIgee = "http://initium-commerce-dev.apigee.net/pimapi/items";
        var paylod={"item":{
            "ItemName" :ItemName,
            "ItemID" :id
} }
...
function getSalesOrderStatustest(internalID) {
...
function getItemFullfilmentInternalIDFromName(itemName) {

    var arrSearchFilter = new nlobjSearchFilter('displayname', null, 'is', itemName);
  ...
  //  var itemInternalID = nlapiLookupField("inventoryitem", itemName, "internalid");
...
function getitemfullfilmentEmail(transID) {
    ...
    try {
        ...
        for (var i = 0; i < itemFullfilmentParsedRecord.item.length; i++) {
          ...
          var currentItemParsedRecord=parseRecord(currentItemRecord);
				 lineItemInfo.push(item_title + checkIfUndefined(currentItemParsedRecord.displayname) + "</item_title>");
				 	lineItemInfo.push(item_qty +checkIfUndefined(itemFullfilmentParsedRecord.item[i].quantity) + "</item_qty>");
        ...
        try {
            ...
            payment_method = payment_method + checkIfUndefined(salesOrderParsedRecord.paymentmethod.name) + "  " + checkIfUndefined(salesOrderParsedRecord.custbody_cc_number_last4) + "</payment_method>";
...
function getSalesOrderEmail(id) {
    ...
    try {
        ...
        for (var i = 0; i < parsedRecord.item.length; i++) {
           ...
           if(checkIfUndefined(parsedRecord.item[i].custcol_item_type.name)=="Inventory Item")
            ...
            lineItemInfo.push(item_title + checkIfUndefined(parsedRecord.item[i].custcol_searsitemname) + "</item_title>");
        ...
        try {
            ...
            payment_method = payment_method + checkIfUndefined(parsedRecord.paymentmethod.name) + "  " + checkIfUndefined(parsedRecord.custbody_cc_number_last4) + "</payment_method>";
            ...
            payment_amount = OrderTotal + "</payment_amount>";
        }
    ...
    //var response = sendExactTargetEmail(jsonOut, "08122016_order_confirmation_trigger");
    return jsonOut;
    ...
    for (j = 0; j < internalIDs.length; j++) {
        ...
        try {
            ...
            var tempLocName=recordObj.locations[k].location_display;
                if (tempLocName == "Calgary SCI : Available to Sell" || tempLocName =="Calgary NLC : Available to Sell") {
                
               

                    totalQuantity =totalQuantity + recordObj.locations[k].quantityavailable;
                }
            }
        } catch (exception) {
...
function getCustomerList() {
    var filters = [];
    var columns = [];
    var result = nlapiSearchRecord("customer", map.donor_list_nsid, filters, columns);
    var list = [];
    for (var i = 0; result != null && i < result.length; i++) {
        var item = {},
            row = cloneObject(result[i]);
        for (var k in map.donor_list) {
            item[k] = row.columns[map.donor_list[k]];
        }
        list.push(item);
    }
    return list;
