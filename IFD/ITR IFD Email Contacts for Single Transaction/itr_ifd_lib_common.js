...
define (['N/log', 'N/record', 'N/search', 'N/runtime', 'N/file'],
	function (log, record, search, runtime, file) {
		...
		common.searchRecords = function(stRecordType, stSearchID, arrSearchFilters, arrSearchColumns) {
			...
			try {
				...
				log.debug(FUNC_NAME, 'total search result length: ' + arrSearchResults.length);
			} catch (error) {
		...
		common.searchRecFilExp = function(stRecordType, stSearchID, arrSearchFilters, arrSearchColumns) {
			...
			try {
				...
				log.debug(DEBUG_IDENTIFIER, 'total search result length: ' + objSearchResults.length);
			}
			catch (error) {
				var errorStr = 'Type: ' + error.type + ' | ' + 'Name: ' + error.name +' | ' + 'Error Message: ' + error.message;
				log.debug(DEBUG_IDENTIFIER, ' Error on Catch : ' + JSON.stringify(errorStr));
			}
			finally {
		...
		common.getScriptParameter = function(parameterName) {
			...
			if (common.isNullOrEmpty(paramValue)) {
				log.error(FUNC_NAME, 'A script parameter:' + parameterName + ': is null/empty. Please Review script deployment');
				log.error(FUNC_NAME, '==================== END Script====================');
				return null;
			}
			else {
		...
		common.filterContactEmailFlag = function (recType){
			var contactEmailFlag = '';
			switch(recType) {
				case 'invoice' : 
					contactEmailFlag = 'custentity_ifd_contact_email_invoice';				   		
					break;

                case 'salesorder' : 
					contactEmailFlag = 'custentity_ifd_contact_email_salesorder';				   		
					break;
					
				case  'creditmemo' :
					contactEmailFlag = 'custentity_ifd_contact_email_creditmemos';				   							
					break;	
					
				case 'purchaseorder' : 
					contactEmailFlag = 'custentity_ifd_contact_email_purchase_or';	
					break;	
								
					
				case 'vendorcredit' : 
					contactEmailFlag = 'custentity_ifd_contact_email_bill_credit';	
					break;	
				
				case 'returnauthorization' : 
					contactEmailFlag = 'custentity_ifd_contact_email_ras';
					break;
			}
				
			var emailFlag = search.createFilter({
				name: contactEmailFlag,
		...
		common.getTransactionAttachments = function (transactionId, attachmentSearchId) {
			var attachments = [];

			if (transactionId && attachmentSearchId) {
				var attachmentSearch = search.load({id: attachmentSearchId});
				attachmentSearch.filters = (attachmentSearch.filters) ? attachmentSearch.filters : [];

				attachmentSearch.filters.push(search.createFilter({
					name: 'custrecord_ifd_parent_transaction',
					operator: search.Operator.ANYOF,
					values: transactionId
				}));

				var attachmentSearchResults = attachmentSearch.run().getRange({start: 0, end: 50});
				if (attachmentSearchResults && attachmentSearchResults.length > 0) {
					for (var index = 0; index < attachmentSearchResults.length; index++) {
						attachments.push(
							file.load({
								id: attachmentSearchResults[index].getValue({name: 'custrecord_ifd_file_attachment'})
							})
						);
					}
				}
