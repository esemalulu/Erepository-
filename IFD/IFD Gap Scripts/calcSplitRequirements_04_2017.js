 ...
 * 1.01       24 Oct 2016     Rafe Goldbach, SuiteLaunch llc
 *
 */
/**
...
function calcSplits(request, response) {
    ...
    if (itemsResult) {
        ...
        var report = 'Indianhead Foodservice Distributor,,,,Split Replenishment Report,,,,Date: ' + today + '\n\n Split Item, Description, Master Item, Split PF, Split QOH, PF Max, Cases to Split, Master Cases Avail., Master Pick Face, Per Case Split, Actual Cases Split\n\n';
        ...
        for (var j = 0; j < max; j++) {
            ...
            //var splitBin =  itemsResult[j].getText(columns[10]);
            var splitBin =  itemsResult[j].getText(columns[4]);
            ...
            if (casesToSplit > 0) {
                ...
                for (var k = 0; k < counter; k++) {
                   ...
                   // report+='result item='+resultItem+' master Item '+masterItem+'\n';
                    if (resultItem == masterItemText && masterAvail > 0) {                       
                        //nlapiLogExecution('debug', 'masterPF', masterPF);
                        //nlapiLogExecution('debug', 'counter', counter);
                        report += itemText + c +descritpion+c+ masterItemText + c + splitBin + c + splitQoh + c + pickFaceMax + c + casesToSplit + c + masterAvail + c + masterPF+c+splitConversion+c+'_____________________\n\n';
