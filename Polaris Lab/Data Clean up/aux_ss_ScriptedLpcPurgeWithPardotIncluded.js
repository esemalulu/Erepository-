...
var LAST_PROC_CUSTOMER_PARAM = 'custscript_lastproc_49pulist';
//Last processed ID for Backup and Delete process
var LAST_PROC_CUSTOMER_PARAM2 = 'custscript_lastproc_50bkdel';

//Static Values 
// - Folder ID to store data clean up related files
var PURGE_FOLDER_ID = '444620';
//	- Primary Notifier: Send to Elliott Shultz
var PURGE_PRIME_NOTIF = '108049';
//	- Comma Separated list of additional emails.
var PURGE_ADDITIONAL_NOTIF = ['polaris@audaxium.com'];
...
var customerRecHeader = 'Status, Log, Internal ID, Entity ID, Stage, Status, Company Name, First Name, Last Name, Email, Phone, Bill Address 1, Bill Address 2, Bill Zip, Ship Address 1, Ship Address 2, Ship Zip, '+
						'Inactive, Sales Rep, Lead Source\n';
var customerRecBody = '';
...
function pardotUnlistList() {
	...
	try {
		...
		var flt = [new nlobjSearchFilter('custentity_datacleanup_tobedeleted',null, 'is','T'),
		...
		for (var i=0; rslt && i < rslt.length; i++) {
			...
			if (rslt[i].getValue('email')) {
				var lvl2aexpflt = [
						              ['custentity_datacleanup_tobedeleted', 'is','F'],
						              'and',
						              [
			...
			var lvl2flt = [new nlobjSearchFilter('custentity_datacleanup_tobedeleted', null, 'is', 'T'),
		...
		if (csvPardotToUnlistBody) {
			...
			csvFileObj.setFolder(PURGE_FOLDER_ID);
			...
			var msg = 'Attached is CSV to send to Pardot to process mass sales rep unassignment. <br/><br/>'+
					  fileName+' also stored under AUX: Scripted L/P/C Purge Log Folder';
			//Send it to Internal Team ONLY 
			//nlapiSendEmail(-5, 'joe.son@audaxium.com', sbj, msg, null, null, null, csvFileObj);
		}
...
function backupAndDeleteCleanupRecords() {
	...
	if (emailCsvBody) {
		...
		emailCsvFileObj.setFolder(PURGE_FOLDER_ID);
	...
	if (activityCsvBody) {
		...
		activityCsvFileObj.setFolder(PURGE_FOLDER_ID);
	...
	if (mktSubCsvBody) {
		...
		mktsubCsvFileObj.setFolder(PURGE_FOLDER_ID);
	...
	if (noteCsvBody) {
		...
		unoteCsvFileObj.setFolder(PURGE_FOLDER_ID);
	...
	if (fileCsvBody) {
		...
		fileCsvFileObj.setFolder(PURGE_FOLDER_ID);
	...
	if (ctCsvBody) {
		...
		ctCsvFileObj.setFolder(PURGE_FOLDER_ID);
	...
	if (customerRecBody) {
		...
		customerRecCsvFileObj.setFolder(PURGE_FOLDER_ID);
