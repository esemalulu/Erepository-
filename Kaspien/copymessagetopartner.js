 ...
 * @returns {Void}
 */
function copymessagetopartner(type){
	...
	try {
		...
		if (type == 'create' || type == 'edit') {
			...
			if (customerId) {
				...
				filters[0] = new nlobjSearchFilter('recipient',null,'is',customerId);
	
				// Search for contacts that have company = customer internal id 
				var searchRecords = nlapiSearchRecord('message',null,filters,columns);
				...
				for (var i in searchRecords) {
					...
					nlapiAttachRecord('message', recId, 'vendor', vendorId);       
