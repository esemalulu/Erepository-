function autoReminderForPartner() {
	var recLimit = 99;
	try {
		var poSearchResults = nlapiSearchRecord('purchaseorder', 'customsearch_po_second_round_followup_3');
		if(poSearchResults !== null && poSearchResults.length > 0) {
			...
			for (var i = 0; i < recLimit; i++) {
				...
				for(var j = 0; j < internalIds.length; j++){
					...
					var sysMaxDate = poRec.getFieldValue('custbody_auto_email_sent_on_date_fup3');
				...
				for(var index=0; index < validInternalIds.length; index++) {
					nlapiSubmitField('purchaseorder', validInternalIds[index], ['custbody_auto_email_sent_2round_fup3', 'custbody_auto_email_sent_dt_2rnd_fup3'], ['T' ,nlapiDateToString(new Date(), 'date')]);
				}
			}
