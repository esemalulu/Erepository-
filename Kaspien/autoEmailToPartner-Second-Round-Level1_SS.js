function autoReminderForPartner() {
	var recLimit = 99;
	try {
		var poSearchResults = nlapiSearchRecord('purchaseorder', 'customsearch_po_second_round_followup_1');
		if(poSearchResults !== null && poSearchResults.length > 0) {
			...
			for (var i = 0; i < recLimit; i++) {
				...
				for(var j = 0; j < internalIds.length; j++){
					...
					filters[2] = new nlobjSearchFilter("field","systemNotes","noneof",["custbody_auto_email_sent_2round_fup1", "custbody_auto_email_sent_2round_fup2","custbody_auto_email_sent_2round_fup3"]);
					...
					var sysMaxDate = poRec.getFieldValue('custbody_auto_email_sent_on_date_fup1');
				...
				for(var index=0; index < validInternalIds.length; index++) {
					nlapiSubmitField('purchaseorder', validInternalIds[index], ['custbody_auto_email_sent_2round_fup1', 'custbody_auto_email_sent_dt_2rnd_fup1'], ['T' ,nlapiDateToString(new Date(), 'date')]);
				}
			}
