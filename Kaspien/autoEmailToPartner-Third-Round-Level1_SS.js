function autoReminderForPartner() {
	var recLimit = 99;
	try {
		var poSearchResults = nlapiSearchRecord('purchaseorder', 'customsearch_po_third_round_followup_1');
		if(poSearchResults !== null && poSearchResults.length > 0) {
			...
			for (var i = 0; i < recLimit; i++) {
				...
				for(var j = 0; j < internalIds.length; j++){
					...
					var sysMaxDate = poRec.getFieldValue('custbody_auto_email_sent_dt_2nd_fup1');
				...
				for(var index=0; index < validInternalIds.length; index++) {
					nlapiSubmitField('purchaseorder', validInternalIds[index], ['custbody_auto_email_sent_3round_fup1', 'custbody_auto_email_sent_dt_3rnd_fup1'], ['T' ,nlapiDateToString(new Date(), 'date')]);
				}
			}
