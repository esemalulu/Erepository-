function autoReminderForPartner() {
	var recLimit = 150;
	try {
		var poSearchResults = nlapiSearchRecord('purchaseorder', 'customsearch_po_followup_1');
		if(poSearchResults !== null && poSearchResults.length > 0) {
			...
			for (var i = 0; i < recLimit; i++) {
				...
				for(var index=0; index < validInternalIds.length; index++) {
					nlapiSubmitField('purchaseorder', validInternalIds[index], ['custbody_auto_email_sent_fup1', 'custbody_auto_email_sent_on_date_fup1'], ['T' ,nlapiDateToString(new Date(), 'date')]);
				}
			}
