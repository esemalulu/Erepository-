function autoReminderForPartner() {
	var recLimit = 99;
	try {
		var poSearchResults = nlapiSearchRecord('purchaseorder', 'customsearch_po_followup_3');
		if(poSearchResults !== null && poSearchResults.length > 0) {
			...
			for (var i = 0; i < recLimit; i++) {
				...
				for(var index=0; index < validInternalIds.length; index++) {
					nlapiSubmitField('purchaseorder', validInternalIds[index], ['custbody_auto_email_sent_fup3', 'custbody_auto_email_sent_on_date_fup3'], ['T' ,nlapiDateToString(new Date(), 'date')]);
				}
			}
