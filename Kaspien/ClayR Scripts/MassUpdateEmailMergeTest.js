...
function massUpdateEmailMerge(recType, recId) {
	...
	try {
		if (recType == 'vendor') {
			...
			var emailTemplate = nlapiGetContext().getSetting('SCRIPT','custscript_email_template2');
			var emailSender = nlapiGetContext().getSetting('SCRIPT','custscript_partner_opt_member2');
			...
			nlapiLogExecution('DEBUG', 'Mass Update Email Merge', 'recType: ' + recType + '; internalId: ' + recId + 
					'; Subject: ' + emailSubject + '; currDateTime: ' + currDateTime + '; emailTemplate: ' + emailTemplate + '; emailSender: ' + emailSender);
