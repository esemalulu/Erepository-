 ...
 */
function validateDelete_rollbackDiscounts(type)
{
	try
	{
	   if(type == 'item')
	   {
		   var stLoggerTitle	= 'validateDelete_rollbackDiscounts';
		   var stNOIPool	= nlapiGetCurrentLineItemValue('item', 'custcol_ifd_noi_pool_budget');
		   
		   if(!Eval.isEmpty(stNOIPool))
		   {
			   nlapiLogExecution('debug', stLoggerTitle, 'stNOIPool = ' + stNOIPool);
			   var boolRemove = confirm('Removing this line would rollback discounts to NOI Pool Budget records. Continue?');
