 ...
 * 1.15       4 Apr 2024      mbigelow         Replace use of Market Cost to Fixed Landed Cost - Weekly
 */
      ...
      if (!NSUtil.isEmpty(arrMatrixPrices)) {
        ...
        for (var intCtr = 0; intCtr < nmArrMatrixLength; intCtr++) {
          ...
          try {
            ...
            var stMatrixProdPriceGroup = objMatrixPricing['custrecord_prod_prc_grp'];
            ...
            if ((stMatrixStatus == stSTATUS_NEW) || (stMatrixRefresh == stINTERVAL_ON_DEMAND)) {
              ...
              buildCatsandLevelsArray(stMatrixId, stMatrixProdPriceGroup, stMatrixPriceLevel, flMatrixMarkupPercent, flMatrixMarkupDollar); // v1.12: Add
              // processItemsByCategory(stMatrixProdPriceGroup, stMatrixPriceLevel, flMatrixMarkupPercent, flMatrixMarkupDollar, stMatrixRecord, stMatrixId, stSTATUS_ERROR, stSTATUS_PROCESSED); // v1.12: Remove
        ...
        if (boBuildCatsPL) {
          ...
          // processItemsByCategory(stMatrixProdPriceGroup, stMatrixPriceLevel, flMatrixMarkupPercent, flMatrixMarkupDollar, stMatrixRecord, stMatrixId, stSTATUS_ERROR, stSTATUS_PROCESSED); // v1.12: Add
 ...
 * @param {String} stMatrixProdPriceGroup - Category to search items in
 ...
 *
 */
// function processItemsByCategory(stMatrixProdPriceGroup, stMatrixPriceLevel, flMatrixMarkupPercent, flMatrixMarkupDollar, stMatrixRecord, stMatrixId, stSTATUS_ERROR, stSTATUS_PROCESSED) { // v1.12: Remove
function processItemsByCategory(arCatPricLev, stMatrixRecord, stSTATUS_ERROR, stSTATUS_PROCESSED) {
  try {
    ...
    arrItemCols.push(new nlobjSearchColumn('class')); // v1.12: Add
    arrItemFilters.push(new nlobjSearchFilter('class', null, 'anyof', arItemCats)); // v1.12: Updated stMatrixProdPriceGroup to arItemCats
    ...
    var arrItems = NSUtils.search('item', null, arrItemFilters, arrItemCols);
    ...
    if (!NSUtil.isEmpty(arrItems)) {
      ...
      nlapiLogExecution('DEBUG', stLoggerTitle + STEP, ' # of Items retrieved : ' + nmArrItems + ' with subcategories : ' + arItemCats); // v1.12: Updated stMatrixProdPriceGroup to arItemCats
      ...
      for (var intItemIdx = 0; intItemIdx < nmArrItems; intItemIdx++) {
        try {
          ...
          stItemClass = objItemRes.getValue('class'); // v1.12: Add
          ...
          var stErrorMessage = 'Last item processed = ' + stItemId
            + ' | Prod Category = ' + stMatrixProdPriceGroup
...
function buildCatsandLevelsArray(stMatrixId, stMatrixProdPriceGroup, stMatrixPriceLevel, flMatrixMarkupPercent, flMatrixMarkupDollar) { // v1.12
  boBuildCatsPL = true;
  ...
  obParamData.cat = stMatrixProdPriceGroup;
  ...
  var obArrData = {};
  obArrData.matrixId = [];
  obArrData.cat = stMatrixProdPriceGroup
  ...
  if (arCatPricLev.length === 0) {
    ...
    for (var idx = 0; idx < arCatPricLev.length; idx++) {
      if (stMatrixProdPriceGroup == arCatPricLev[idx].cat) {
        // Category matches
