...
function Readerboard(params)
{
	...
	html += 'function AddToCart(){var color = document.getElementById("custitem_nrs_letter_color").value; var letters = new String(); var qty = new String(); var cells = document.getElementsByTagName("input"); for(var i = 0; i < cells.length; i++){if(cells[i].className == "txt" && cells[i].value != "" && !isNaN(cells[i].value)){letters += cells[i].id + ","; qty += cells[i].value + ",";}}window.location.href = "http://shopping.netsuite.com/app/site/backend/additemtocart.nl?parent=' + internalId + '&color=" + color + "&letters=" + letters + "&qty=" + qty;}';	
	...
	html += '<tr><td>';
		...
		for(var i = 0; i < letters.length; i++)
		{
			...
			html += '<br/><input type="text" class="txt" id="' + letterValue + '" size="4" onblur="CalculatePrice()" />';
			html += '<div class="itemprice" style="display:none;" id="price' + letterValue + '">' + letterPrice + '</div>';
			html += '</td>';
		}
