...
function Readerboard(params)
{
	...
	//Mod 10/1/2012 by Joe.son@audaxium.com - added json2.js file 
	html += "<script type='text/javascript' src='http://www.nationalreaderboard.com/Json2.js'></script>";
	...
	//Mod: JSON Audaxium. - 9/29/2012
	//Wanted Color drop down to default to empty.
	//This requires AddToCart() function to check to make sure Color is selected
	html += 'function AddToCart(){ \n'+
			'	var color = document.getElementById("custitem_nrs_letter_color").value; \n'+
			'	if (!color) {\n'+
			...
			//added by JSON Audaxium - 10/1/2012
			'	var jsonObj = JSON.parse(document.getElementById("jsonObj").innerHTML);\n'+
			'	for ( var i = 0; i < cells.length; i++) {  \n'+
			'		if (cells[i].className == "txt" && cells[i].value != "" && !isNaN(cells[i].value)) {  \n'+ 
			'			letters += jsonObj[color][cells[i].alt] + "," + cells[i].value + ";";  \n'+
			'		}  \n'+
			'	}  \n'+
			//validation added by Audaxium. When no item and item qty is selected, alert user 
			'   if (!letters) {\n'+
			'		alert("Please enter quantity you would like to order under each letter");\n'+
			'		return false;\n'+
			'	}\n'+
			'	window.location.href = "/app/site/backend/additemtocart.nl?&buyid=multi&multi=" + letters;  \n'+
			'}';
		...
		html += '<tr><td align="left" valign="top">';
			...
			if(searchresults != null && searchresults.length > 0)
			{
				...
				*/
				...
				//Mod 10/1/2012 - joe.son@audaxium.com
				//build JSON object of all related matrix item from search result
				var allRelatedMatrixItems = {};
				...
				for(var i = 0; i < searchresults.length; i++)
				{
					...
					//fill in allRelatedMatrixItems json object
					if (!allRelatedMatrixItems[searchresults[i].getValue('custitem_nrs_letter_color')]) {
						allRelatedMatrixItems[searchresults[i].getValue('custitem_nrs_letter_color')]={};
					}
					allRelatedMatrixItems[searchresults[i].getValue('custitem_nrs_letter_color')][searchresults[i].getText('custitem_nrs_letter')] = searchresults[i].getId();
				...
				html +='<div id="jsonObj" style="display:none;">'+JSON.stringify(allRelatedMatrixItems)+'</div>';
	...
	html += '<tr><td>';
		...
		for(var i = 0; i < letters.length; i++)
		{
			...
			//Mod: JSON Audaxium. - 9/29/2012
			//Wanted 1 to appear just like letter I
			
			html += '<td align="center" valign="bottom">' + ((letterText=='1')?'I':letterText);
			html += '<br/><input type="text" class="txt" id="' + lettersId[letterText] + '" alt="'+letterText+'" size="4" onblur="CalculatePrice()" />';
